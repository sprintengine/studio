...
- Add a post-code-review final review scheduling task before treating the swarm as complete
...
- Owns a review/scheduling document path such as `swarm/<team-slug>/reviews/final-review-schedule-1.md`.
...
- Acceptance must require deciding which final reviews are needed and adding only those task cards with `swarm plan add-task`.
