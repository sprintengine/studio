...
def brand_context_instructions(sectors: list[str]) -> list[str]:
    ...
    return [
        ...
        "",
        "Before creating brand-alignment findings, inspect the workspace knowledge graph for brand guidance when one is present — for example a top-level brand note, a design-system note, or a workspace-theme note under the workspace's knowledge root.",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "Before creating the first task, run `switchboard create --help` to confirm the current schema.",
            ...
            "",
            "Create one inbox task per finding with the local `switchboard` command.",
            "",
            "```bash",
            ...
            "```",
            "Set `creationConfidencePct` to your 0-100 confidence that this is a legitimate, actionable Switchboard task worth tracking.",
            "",
    ...
    return "\n".join(
        [
            ...
            *task_lines,
            ...
            "# Read Contract",
            "",
            "Use project-root-relative paths in all analysis. Read through the local `switchboard` command:",
            "",
            "```bash",
            f"switchboard read-all --workspace {json.dumps(workspace_text)}",
            "switchboard show --workspace <repo> <task-id>",
            "```",
            "",
            ...
            "",
            "```bash",
            f"switchboard comment --workspace {json.dumps(workspace_text)} <task-id> --kind triage --author Architect --author-type agent --author-id watchtower-architect --confidence-pct <0-100> --body '<comment-body>'",
            "```",
            "Set `--confidence-pct` to your 0-100 confidence in the triage recommendation and reasoning.",
            "",
