...
If the real dependency is unavailable, blocked, physically unverified, missing from the codebase, or outside the current task, do not claim the product behavior is done. Mark the task `needs_input` or record a blocker/follow-up, and make the remaining real integration explicit.

Architect task cards and plan acceptance criteria must fail when the feature only works through hardcoded samples, disconnected UI state, fake success paths, mocks, stubs, or documentation of unverified limits.
