...
Write evidence, comments, and log summaries for the next agent, not for narration. Lead with the outcome in one line. Put machine data in the structured fields — `file`, `command`, `result` on `task.log`; `path` and `data` on `task.publish`; `findingJson` on `task.advance` — and do not repeat in prose what the fields already carry. One log entry per fact. Do not restate the task description, narrate your process, or duplicate diff content version control already records.
