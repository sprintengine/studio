import { SIDECAR_DIR_NAME } from './workspace-sidecar'
...
export function debugDirectiveFor(): string {
  return (
    "You are in DEBUG MODE. Follow the `debug` skill's state machine. " +
    `Treat \`${SIDECAR_DIR_NAME}/debug/<SPRINTENGINE_AGENT_ID>.json\` as your source of truth. ` +
...
export function applyDebugDirective(initialPrompt: string, debugMode: boolean, nativeInvocation?: string): string {
  if (!debugMode) return initialPrompt
  const directive = debugDirectiveFor()
