...
test('workspace-sync', async () => {
  ...
  assert.deepEqual(moved.state.workspaceWindows.find((candidate) => candidate.id === 'primary')?.workspaceIds, [
    'ws-two',
  ])
  assert.equal(
  ...
  const assignedAgent = assignSession.state.workspaces.find((candidate) => candidate.id === 'ws-one')?.agents[
    'agent-one'
  ]
  ...
  const assignedClaudeCodeAgent = assignClaudeCodeSession.state.workspaces.find(
    (candidate) => candidate.id === 'ws-one',
