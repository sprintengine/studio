...
export function normalizeSprintEngineProjection(
  ...
  const candidate: SprintEngineState = {
    ...
    ...((): Partial<Pick<SprintEngineState, 'locks'>> => {
      const locks = normalizeProjectionLocks(record.locks)
      return locks ? { locks } : {}
    })(),
    ...((): Partial<Pick<SprintEngineState, 'creation'>> => {
      const creation = normalizeProjectionCreation(runRecord.creation)
      return creation ? { creation } : {}
    })(),
    ...((): Partial<Pick<SprintEngineState, 'runner'>> => {
      const runner = normalizeSprintEngineRunnerPolicy(runRecord.runner)
      return runner ? { runner } : {}
    })(),
    ...((): Partial<Pick<SprintEngineState, 'vcs' | 'useWorktrees'>> => {
      const vcs = normalizeSprintEngineVcs(runRecord.vcs)
      return vcs ? { vcs, useWorktrees: true } : {}
    })(),
    ...((): Partial<Pick<SprintEngineState, 'roleRuntimes'>> => {
