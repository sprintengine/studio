...
export function buildSprintEngineStartupPrompt(
  ...
  const generalLoopBlock = isGeneral
    ? [
      ...
      'Drive every piece of work through the same loop, in order: plan → build → publish → review → advance. Normally you carry a task through to done before claiming new ready work. If a configured independent phase reviewer takes review, stop after the handoff; a reviewer who requests changes stays reserved until reapproval.',
