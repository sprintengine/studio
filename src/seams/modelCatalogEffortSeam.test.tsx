...
test('modelCatalogEffortSeam', async () => {
  ...
  // A level with no model — "the CLI's own default model at extra-high effort" —
  // which the pre-1870 normalizer would have thrown away.
  const PREVIOUS_BUILD_SELECTION = { cli: 'claude-code', model: '', reasoning: 'xhigh' }
  ...
  async function main(): Promise<void> {
    ...
    const { __resetPluginRegistryForTest, __setPluginRegistryForTest } =
      await import('../main/plugin-registry-instance')
    ...
    await check('SEAM: clearing the level clears the flag, and the model survives the clearing', async () => {
      ...
      store().setLastSelectedAgentModel({
        cli: 'claude-code',
        model: 'claude-opus-5',
        reasoning: 'xhigh',
      } as never)
      store().setLastSelectedAgentReasoning('claude-code' as never, null)
      const stored = store().appSettings.lastSelectedAgentModel
      assert.deepEqual(
        stored,
    ...
    // ── Seam 4: a level the launching CLI does not declare ─────────────────────

    await check('SEAM: a level the launching CLI does not declare never reaches its command', async () => {
      // `ultra` is Codex's top level and is NOT one of claude-code's five. A
      // stored selection carrying it (a hand-edited settings file, a CLI changed
      // under a remembered pick) must launch without the flag rather than with a
      // value the CLI would reject.
      await usingBundledRegistry(() => {
        const command = buildAgentShellCommand({
          cli: 'claude-code',
          sessionId: 'seam-session',
    ...
    await check(
      ...
      () => {
        ...
        assert.deepEqual(
          normalizeAppSettings(normalized as never, []).lastSelectedAgentModel,
          PREVIOUS_BUILD_SELECTION,
