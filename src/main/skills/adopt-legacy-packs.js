 ...
 */
export async function adoptLegacySkillPackSources(options) {
    if (await options.store.hasAdoptedLegacyPacks())
        return [];
    ...
    for (const pack of LEGACY_SKILL_PACKS) {
        ...
        if (await options.store.getSource(id))
            continue;
...
function legacySkillPackSource(id, repo) {
...
function isSkillInstalled(workspaceRoot, dirName) {
    return SKILL_PACK_HARNESSES.some((harness) => existsSync(join(workspaceRoot, SKILL_HARNESS_DIR[harness], 'skills', dirName)));
}
