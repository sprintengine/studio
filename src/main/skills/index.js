...
import { BUILTIN_SKILL_SOURCE_ID, CONNECTORS_SKILL_SOURCE_ID, parseSkillFrontmatter, SKILL_ENTRY_FILE, skillSourceMonogram, } from '../../shared/skills';
...
import { fetchSkillRepoFile, fetchSkillRepoTree, parseSkillRepoRef, resolveSkillRepoCommit, SkillFetchError, } from './github-tree';
...
export function createSkillsService(userDataDir, deps, store = createSkillSourceStore(userDataDir)) {
    ...
    const localRootFor = (id) => id === BUILTIN_SKILL_SOURCE_ID ? builtinRoot() : id === CONNECTORS_SKILL_SOURCE_ID ? connectorRoot() : null;
    async function scanFor(sourceId) {
    ...
    return {
        ...
        async getScan(input) {
            ...
            if (!source)
                return { ok: false, message: 'That source is not in your list.' };
            ...
            if (!ref)
                return { ok: false, message: `${source.repo} is not a repository that can be read.` };
            try {
                const github = { ...deps.github, token: await deps.resolveToken() };
        ...
        async syncSource(input) {
            ...
            const copied = workspaceRoot && existsSync(workspaceRoot)
    ...
    async function locateSkill(sourceId, skillId) {
        ...
        if (!skill)
            return { ok: false, message: 'That skill is not in this source.' };
        return { ok: true, source, skill };
    }
    async function resolveSkillFile(sourceId, skillId, path) {
        ...
        if (!file)
            return { ok: false, message: 'That file is not part of this skill.' };
        return { ok: true, source: located.source, skill: located.skill, file };
    }
    async function readSkillBytes(source, skill, file) {
        if (source.kind === 'github') {
            const github = { ...deps.github, token: await deps.resolveToken() };
        ...
        if (!root)
            throw new SkillFetchError(`${source.name} is not available in this build.`);
        return readFile(join(root, ...skill.id.split('/').filter(Boolean), ...file.path.split('/')));
    }
}
async function scanGithubSource(ref, id, github) {
    const commitSha = await resolveSkillRepoCommit(ref, github);
    const tree = await fetchSkillRepoTree(ref, commitSha, github);
    const manifest = tree.entries.some((entry) => entry.type === 'blob' && entry.path === SKILL_MARKETPLACE_MANIFEST_PATH)
    ...
    const skills = await enrichSkills(scanned.skills, (skill) => fetchSkillRepoFile(ref, commitSha, joinRepoPath(skill.id, SKILL_ENTRY_FILE), github).then((bytes) => bytes.toString('utf8')));
    const scan = { ...scanned, skills };
 ...
 */
async function enrichSkills(skills, readEntry) {
...
function githubRefFor(source) {
    const [owner, repo] = source.repo.split('/');
    return { owner: owner ?? '', repo: repo ?? '', ref: '' };
}
function joinRepoPath(skillId, relativePath) {
