...
import { SKILL_ENTRY_FILE, SKILL_REPO_ROOT_GROUP, skillDirName, } from '../../shared/skills';
...
export function scanSkillTree(input) {
    ...
    for (const file of files) {
        ...
        if (isExecutableFile(relativePath, file.mode))
            executableDirs.add(dir);
    }
    const scanned = skillDirs.map((dir) => ({
        id: dir,
 ...
 */
function findSkillDirs(files) {
    const dirs = new Set();
    for (const file of files) {
        const cut = file.path.lastIndexOf('/');
        const base = cut === -1 ? file.path : file.path.slice(cut + 1);
        if (base === SKILL_ENTRY_FILE)
            dirs.add(cut === -1 ? '' : file.path.slice(0, cut));
    }
 ...
 */
function collapseHarnessMirrors(skillDirs) {
    const canonicalByMirrorKey = new Map();
    for (const dir of skillDirs) {
 ...
 */
function harnessMirrorKey(dir) {
    const segments = dir.split('/');
    if (segments.length < 2)
        return dir;
    const root = segments[0];
    if (!root.startsWith('.') && !HARNESS_ROOT_NAMES.includes(root))
        return dir;
 ...
 */
function ownerSkillDir(path, skillDirs) {
...
function isExecutableFile(relativePath, mode) {
 ...
 */
function group(skills, manifest) {
    ...
    const signal = folders.length >= 2 ? 'folders' : 'none';
    if (signal === 'none') {
 ...
 */
function parseManifestSkills(manifest) {
    if (!manifest)
        return null;
    let parsed;
    try {
    ...
    for (const plugin of plugins) {
        ...
        if (typeof name !== 'string' || name.length === 0 || !Array.isArray(skills))
            continue;
        for (const raw of skills) {
            if (typeof raw !== 'string')
                continue;
            const path = raw.replace(/^\.\//, '').replace(/\/+$/, '');
            if (path.length > 0 && !listed.has(path))
                listed.set(path, name);
        }
    }
    return listed.size > 0 ? listed : null;
}
function folderGroup(skillId) {
