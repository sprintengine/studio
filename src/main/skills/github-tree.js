...
import { MARKETPLACE_EXTRA_HOSTS_ENV, isMarketplaceSourceHostAllowed, parseMarketplaceExtraHosts, } from '../../shared/marketplace/source-policy';
 ...
 */
export function parseSkillRepoRef(input) {
...
function validRef(ref) {
    ...
    if (!namePattern.test(ref.owner) || !namePattern.test(ref.repo))
        return null;
...
export async function resolveSkillRepoCommit(ref, options = {}) {
    const target = ref.ref || (await fetchDefaultBranch(ref, options));
    const body = await fetchJson(`https://api.github.com/repos/${ref.owner}/${ref.repo}/commits/${encodeURIComponent(target)}`, options);
...
async function fetchDefaultBranch(ref, options) {
    const body = await fetchJson(`https://api.github.com/repos/${ref.owner}/${ref.repo}`, options);
 ...
 */
export async function fetchSkillRepoTree(ref, commitSha, options = {}) {
    const body = await fetchJson(`https://api.github.com/repos/${ref.owner}/${ref.repo}/git/trees/${encodeURIComponent(commitSha)}?recursive=1`, options);
    if (body.truncated === true) {
        throw new SkillFetchError(`${ref.owner}/${ref.repo} is too large for GitHub to list in one request, so it cannot be scanned completely.`);
    }
    ...
    if (body.tree.length > DEFAULT_SKILL_MAX_TREE_ENTRIES) {
        throw new SkillFetchError(`${ref.owner}/${ref.repo} lists more than ${DEFAULT_SKILL_MAX_TREE_ENTRIES} files, which is too large to scan.`);
    }
...
function normalizeTreeEntry(value) {
    if (!value || typeof value !== 'object')
        return null;
    const raw = value;
    if (typeof raw.path !== 'string' || typeof raw.type !== 'string')
        return null;
    return {
        path: raw.path,
...
export async function fetchSkillRepoFile(ref, commitSha, path, options = {}) {
    const encoded = path.split('/').map(encodeURIComponent).join('/');
    return fetchBytes(`https://raw.githubusercontent.com/${ref.owner}/${ref.repo}/${encodeURIComponent(commitSha)}/${encoded}`, { accept: '*/*' }, options);
}
...
async function fetchBytes(url, headers, options, limit) {
    const parsed = parseAllowedUrl(url);
    const fetcher = options.fetcher ?? defaultFetch;
    const maxBytes = limit?.maxBytes ?? options.maxFileBytes ?? DEFAULT_SKILL_MAX_FILE_BYTES;
    const describeOverflow = limit?.describeOverflow ?? ((bytes) => `A file in this repository is larger than ${bytes} bytes.`);
    ...
    try {
        ...
        if (!response.ok)
            throw new SkillFetchError(githubErrorMessage(response), response.status);
...
function githubErrorMessage(response) {
    if (response.status === 404)
        return 'That repository could not be found, or it is not public.';
...
function parseAllowedUrl(value) {
    ...
    if (url.protocol !== 'https:')
        throw new SkillFetchError('Skill source URL must use HTTPS.');
...
async function defaultFetch(url, init) {
    if (typeof globalThis.fetch !== 'function')
        throw new SkillFetchError('fetch is not available.');
