...
test('scan-plugins', async () => {
  ...
  async function pluginArtworkFields(): Promise<void> {
    ...
    assert.equal(
      pluginIconGlyph('ABCDEFGH'),
      '',
      'eight letters are inside the code-point cap and three times the chip',
    )
  ...
  async function skillsOnlyRepositories(): Promise<void> {
    ...
    const empty = await scanPluginTree({
      entries: [],
      skills: [],
      marketplaceManifest: null,
      readFile: async () => null,
  ...
  async function rootServers(): Promise<void> {
    ...
    assert.equal(
      parseMcpRegistryManifest(
        { name: 'x', packages: [{ registryType: 'oci', identifier: 'ghcr.io/x' }] },
        'server.json',
      ).length,
      0,
    )
  ...
  async function readBudget(): Promise<void> {
    ...
    assert.equal(
      capped.plugins[MAX_SCANNED_PLUGINS].componentsKnown,
      false,
      'the one past the cap is unread, not empty',
    )
  ...
  async function linkedPluginsFollowed(): Promise<void> {
    ...
    assert.deepEqual(
      guidance.readState,
      { status: 'read' },
      'an in-tree plugin was read whole, entry documents included',
    )
    assert.ok(guidance.components.hooks.length >= 3)
  ...
  async function linkedPluginsPartial(): Promise<void> {
    ...
    assert.equal(
      shortfallLine({ total: 0, read: 0, pending: 0, unreadable: 0, pendingReasons: NO_PENDING }, false),
      null,
    )
    assert.equal(
