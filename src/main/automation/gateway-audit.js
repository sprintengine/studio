...
function collectSafeIdentifiers(value, result, depth) {
...
function normalizeConnection(connection) {
    const trim = (value) => value?.trim().slice(0, 256) || undefined;
    return {
        kind: connection.kind === 'studio-agent'
            ? 'studio-agent'
...
function errorCode(error, result) {
    ...
    if (structured && typeof structured === 'object') {
        ...
        if (typeof structured.code === 'string')
            return structured.code.slice(0, 128);
    }
    if (result?.isError) {
        try {
            const parsed = JSON.parse(result.content[0]?.text ?? '');
            if (typeof parsed.error?.code === 'string')
                return parsed.error.code.slice(0, 128);
        }
        catch {
...
function rotateIfNeeded(path, incomingBytes, maxBytes, backups) {
