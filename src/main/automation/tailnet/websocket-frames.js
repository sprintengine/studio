...
export function computeWebSocketAcceptKey(clientKey) {
    return createHash('sha1').update(`${clientKey}${WEBSOCKET_GUID}`).digest('base64');
}
export function createWebSocketFrameDecoder(maxMessageBytes = MAX_WEBSOCKET_MESSAGE_BYTES, role = 'server') {
