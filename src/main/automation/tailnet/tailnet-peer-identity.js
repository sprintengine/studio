...
export function createTailnetPeerResolver(options = {}) {
    ...
    const runWhois = options.runWhois ?? ((address) => whoisViaCli(address, options.timeoutMs ?? DEFAULT_TIMEOUT_MS));
    const cache = new Map();
    return {
 ...
 */
export function normalizeAddress(remoteAddress) {
    if (typeof remoteAddress !== 'string')
        return '';
    const value = remoteAddress.trim().split('%')[0];
    return value.startsWith('::ffff:') ? value.slice('::ffff:'.length) : value;
}
async function whoisViaCli(address, timeoutMs) {
 ...
 */
export function peerNameFromWhois(raw) {
    let parsed;
    try {
    ...
    if (name)
        return name.replace(/\.$/, '').slice(0, 256);
