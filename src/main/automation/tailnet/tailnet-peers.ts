...
export function createTailnetPeerScanner(options: {
  ...
  return {
    async scan({ port }): Promise<TailnetPeerScan> {
      const raw = await runStatus()
      if (raw === null) {
        return {
          tailscaleAvailable: false,
          unavailableReason:
            'Tailscale did not answer on this machine. Install Tailscale and sign in, then scan again.',
          probedPort: port,
          peers: [],
        }
      }
 ...
 */
export function parseTailscaleStatus(raw: string): ParsedStatus {
  let parsed: unknown
  try {
  ...
  const peers: Array<Omit<TailnetPeer, 'studio'>> = []
...
function readNode(value: unknown, isSelf: boolean): Omit<TailnetPeer, 'studio'> | null {
 ...
 */
function firstTailnetAddress(value: unknown): string | null {
  if (!Array.isArray(value)) return null
  const addresses = value.filter((entry): entry is string => typeof entry === 'string' && isTailnetAddress(entry))
 ...
 */
export function probeStudioListener(address: string, port: number): Promise<TailnetPeerStudio | null> {
  return new Promise((resolve) => {
    let settled = false
    let call: ClientRequest | null = null
    const finish = (value: TailnetPeerStudio | null): void => {
      if (settled) return
      settled = true
 ...
 */
export function readHealthPayload(body: string): TailnetPeerStudio | null {
  let parsed: unknown
  try {
  ...
  if (!product || !Number.isInteger(transportVersion) || !Array.isArray(protocolVersions)) return null
  const versions = protocolVersions.filter((entry): entry is string => typeof entry === 'string')
  if (versions.length !== protocolVersions.length) return null
  return {
