...
import { createMcpDispatcher, declaredProtocolVersion, isRecord, jsonRpcErrorResponse, jsonRpcIdOf, unsupportedProtocolVersionMessage, JSONRPC_INTERNAL_ERROR, JSONRPC_INVALID_PARAMS, JSONRPC_INVALID_REQUEST, JSONRPC_PARSE_ERROR, } from '../mcp-dispatch';
import { isAllowedTailnetBindAddress } from './tailnet-interface';
import { TAILNET_HEALTH_PATH, TAILNET_IDENTITY_PATH, TAILNET_MCP_PATH, TAILNET_PAIR_PATH, TAILNET_STREAM_PATH, TAILNET_TERMINAL_PATH, TAILNET_WS_TICKET_PATH, } from './tailnet-routes';
import { createTailnetTerminalStream, terminalAttachScopeFor, } from './tailnet-terminal-stream';
...
import { computeWebSocketAcceptKey, createWebSocketFrameDecoder, encodeCloseFrame, encodePongFrame, encodeTextFrame, MAX_WEBSOCKET_MESSAGE_BYTES, WEBSOCKET_CLOSE_GOING_AWAY, WEBSOCKET_CLOSE_REVOKED, } from './websocket-frames';
...
export function createTailnetGatewayServer(options) {
    const now = options.now ?? (() => Date.now());
    let server = null;
    let unsubscribeRevocations = null;
    ...
    async function start() {
        ...
        if (!isAllowedTailnetBindAddress(options.bindAddress)) {
            throw new Error(`Refusing to bind the tailnet gateway to ${options.bindAddress || '(empty)'}: only a Tailscale address (100.64.0.0/10 or fd7a:115c:a1e0::/48) or loopback is allowed.`);
        }
    ...
    async function stop() {
        ...
        for (const terminal of [...terminalStreams])
            terminal.close(WEBSOCKET_CLOSE_GOING_AWAY, 'Server stopping.');
    ...
    async function handleMessage(parsed, context, device, protocolHeader) {
        ...
        try {
            ...
            if (outcome.kind === 'error')
                return jsonRpcErrorResponse(id, outcome.code, outcome.errorMessage);
    ...
    function gateFor(device) {
        const granted = new Set(device.scopes);
        const scopeAllows = (toolName) => tailnetScopeGrantsAccess(granted, requiredScopeForTool(toolName, options.isMutation(toolName)));
        return {
            ...
            authorizeToolCall: (toolName) => {
                if (isLocalOnlyGatewayTool(toolName)) {
                    return toolError('tailnet_local_only', `"${toolName}" configures who may drive this machine and is served only on its owner-only local socket, never over the tailnet. Run it from an agent on that machine.`);
                }
                return scopeAllows(toolName)
                    ? null
                    : toolError('tailnet_scope_required', `This device is not granted "${requiredScopeForTool(toolName, options.isMutation(toolName))}", which "${toolName}" requires. Re-pair the device with that scope in Settings.`);
            },
        };
    }
    function contextFor(device, peerNode) {
        return {
            metadata: {
                kind: 'remote-tailnet',
                deviceId: device.id,
    ...
    async function handleUpgrade(request, socket, head) {
        ...
        const path = pathOf(request.url);
        if (path !== TAILNET_STREAM_PATH && path !== TAILNET_TERMINAL_PATH)
        ...
        if (!key)
            return rejectUpgrade(socket, 400, 'missing_websocket_key');
        ...
        if (path === TAILNET_TERMINAL_PATH) {
            ...
            if (!options.terminals)
                return rejectUpgrade(socket, 503, 'terminal_streaming_unavailable');
            if (!terminalAttachScopeFor(device.scopes))
                return rejectUpgrade(socket, 403, 'terminal_scope_required');
            const sessionId = query.get('sessionId')?.trim();
            if (!sessionId)
                return rejectUpgrade(socket, 400, 'session_id_required');
            ...
            const registration = { stream: null };
            const stream = createTailnetTerminalStream({
                ...
                onClosed: () => {
                    if (registration.stream)
                        terminalStreams.delete(registration.stream);
                },
                log: options.log,
            });
        ...
        options.devices.recordSeen(device.id, peerNode);
        acceptUpgrade(socket, key);
        const session = { socket, deviceId: device.id, context: contextFor(device, peerNode) };
        streams.add(session);
    ...
    async function handleStreamMessage(session, text) {
    ...
    function closeStreamsFor(deviceId) {
        for (const stream of [...streams]) {
            if (stream.deviceId === deviceId)
                closeStream(stream, WEBSOCKET_CLOSE_REVOKED, 'This device has been revoked.');
        }
        ...
        for (const terminal of [...terminalStreams]) {
            if (terminal.deviceId === deviceId)
                terminal.close(WEBSOCKET_CLOSE_REVOKED, 'This device has been revoked.');
        }
        for (const [ticket, entry] of tickets)
    ...
    function closeStream(session, code, reason) {
        streams.delete(session);
    ...
    function sendStream(session, payload) {
        if (session.socket.destroyed)
            return;
    ...
    return {
        ...
        notifyToolsListChanged: () => {
            for (const stream of streams)
                sendStream(stream, { jsonrpc: '2.0', method: 'notifications/tools/list_changed' });
        },
 ...
 */
function remoteAddressOf(socket) {
