...
export function createTailnetDeviceStore(options) {
    ...
    let devices = readDevices(options.resolveUserDataDir(), options.log);
    let pairing = null;
    function persist() {
    ...
    return {
        ...
        getPairingState: () => pairing && pairing.expiresAtMs > now().getTime()
        ...
        authenticate(bearerToken) {
            ...
            for (const device of devices) {
                if (secretsMatch(presented, device.tokenHash))
        ...
        recordSeen(deviceId, peerNode) {
            ...
            if (!peerChanged && Number.isFinite(previousMs) && seenAtMs - previousMs < LAST_SEEN_PERSIST_INTERVAL_MS)
                return;
            try {
                persist();
            }
...
function readDevices(userDataDir, log) {
    let raw;
    try {
