...
import { fleetTerminalAccess, } from '../../../shared/tailnet-fleet';
import { createTailnetFleetStore } from './tailnet-fleet-store';
import { callRemoteTool, formatTailnetEndpoint, openRemoteTerminalSocket, pairWithMachine, parsePairingUrl, parseTailnetEndpoint, readRemoteIdentity, } from './tailnet-remote-client';
...
export function createTailnetFleetService(options) {
    ...
    async function pair(input) {
        ...
        if (!paired.ok)
            return { ok: false, code: paired.code, message: paired.message };
        ...
        try {
            ...
            return {
                ...
                message: `Paired with ${formatTailnetEndpoint(parsed.endpoint)}, but the credential could not be saved here `
                    + `(${message(error)}). Revoke this device on that machine and pair again.`,
            };
        }
    }
    async function browse(connectionId) {
        const connection = connectionFor(connectionId);
        if (!connection)
            return unknownConnectionBrowse(connectionId);
    ...
    async function readWorkspaces(connection, scopes, gaps) {
    ...
    async function readTerminals(connection, scopes, gaps) {
    ...
    async function listRuns(connectionId, workspaceId) {
        const connection = connectionFor(connectionId);
        if (!connection)
    ...
    async function dial(attachment) {
        ...
        if (attachment.released) {
            if (opened.ok)
                opened.value.close('The pane was closed.');
            return;
        }
        if (!opened.ok) {
    ...
    function handleFrame(attachment, frame) {
        if (attachment.released)
            return;
        ...
        if (type === 'error') {
            ...
            if (!attachment.attachedThisDial)
                attachment.refusalThisDial = text;
            attachment.emit({
                type: 'error',
    ...
    function scheduleRetry(attachment, reason) {
        if (attachment.released || attachment.retryTimer)
            return;
        ...
        attachment.emit(attachment.attempts > OFFLINE_AFTER_ATTEMPTS
    ...
    function finish(attachment, reason) {
        if (attachment.released)
            return;
        attachment.released = true;
        if (attachment.retryTimer)
    ...
    function detachTerminal(attachId) {
    ...
    return {
            ...
            if (typeof connectionId === 'string') {
                ...
                for (const attachment of [...attachments.values()]) {
                    if (attachment.connectionId === connectionId)
                        finish(attachment, 'This machine was removed from your fleet.');
                }
        ...
        createTerminal,
        attachTerminal,
        sendInput(attachId, data) {
            const attachment = typeof attachId === 'string' ? attachments.get(attachId) : undefined;
            if (!attachment || typeof data !== 'string' || !data)
                return;
        ...
        resizeTerminal(attachId, cols, rows) {
            const attachment = typeof attachId === 'string' ? attachments.get(attachId) : undefined;
            if (!attachment || !isPositiveInteger(cols) || !isPositiveInteger(rows))
                return;
        ...
        shutdown() {
            for (const attachment of [...attachments.values()])
                detachTerminal(attachment.attachId);
        },
    };
}
function unknownConnectionBrowse(connectionId) {
    return {
