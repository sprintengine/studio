...
export function readTailnetSettings(userDataDir) {
    const defaults = { enabled: false, port: DEFAULT_TAILNET_LISTENER_PORT };
    let raw;
    try {
...
export function writeTailnetSettings(userDataDir, settings) {
    const body = `${JSON.stringify({ enabled: settings.enabled === true, port: isUsablePort(settings.port) ? settings.port : DEFAULT_TAILNET_LISTENER_PORT }, null, 2)}\n`;
