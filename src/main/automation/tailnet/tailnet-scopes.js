 ...
 */
export function isLocalOnlyGatewayTool(toolName) {
 ...
 */
export function requiredScopeForTool(toolName, isMutation) {
    if (toolName.startsWith('terminal.'))
        return isMutation ? 'terminal:control' : 'terminal:observe';
    return `${toolFamily(toolName)}:${isMutation ? 'operate' : 'read'}`;
}
function toolFamily(toolName) {
    if (toolName.startsWith('backlog.'))
        return 'backlog';
    if (toolName.startsWith('horizon.'))
        return 'horizon';
    ...
    if (toolName.startsWith('sprint.') || toolName.startsWith('sprintengine.'))
        return 'sprint';
    return 'workspace';
}
