...
// on the SAME board the person draws on — a `.excalidraw` file in the project,
// merged per element, with the person always winning a contested shape.
//
...
const SHARED_PROPERTIES = {
  board: {
    type: 'string',
    description:
      'The board to act on: a project-relative path such as "diagrams/architecture.excalidraw", or a bare name, which lands in diagrams/. Omitted, the workspace\'s most recently changed board, or diagrams/canvas.excalidraw when it has none.',
  },
...
export function createCanvasTools(deps: CanvasToolsDeps): McpToolRegistration[] {
  ...
  async function resolveBoard(
    ...
    if (named) {
      ...
      return { workspaceId, ref: { workspaceId, path: normalized.value } }
    }
  ...
  const tools: McpToolRegistration[] = [
    {
      name: 'canvas.list',
      description:
        'List the Canvas boards in this workspace with their element count and when each last changed, most recently changed first. A board is an .excalidraw file in the project, normally under diagrams/, versioned with the repository like any other file. Capped at 200, with a note when the cap was reached. Read-only.',
      inputSchema: {
        type: 'object',
