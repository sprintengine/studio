  ...
  check(
    ...
    async () => {
      ...
      try {
        ...
        assert.equal(
          laptop.devices.listDevices().length,
          1,
          'the reverse device stays, revocable by name on the laptop',
        )
      } finally {
        await laptop.close()
        await mini.close()
  ...
  check(
    ...
    async () => {
      ...
      try {
        ...
        assert.ok(pending)
        assert.ok(
          mini.devices.approvePairRequest({ id: requestId, scopes: ['workspace:read'], code: pending.comparisonCode })
            .ok,
        )
