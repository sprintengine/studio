...
import { createMcpDispatcher, declaredProtocolVersion, isRecord, jsonRpcErrorResponse, jsonRpcIdOf, unsupportedProtocolVersionMessage, JSONRPC_INTERNAL_ERROR, JSONRPC_INVALID_PARAMS, JSONRPC_INVALID_REQUEST, JSONRPC_PARSE_ERROR, TOOLS_LIST_TTL_MS, } from './mcp-dispatch';
...
export function createMcpSocketServer(options) {
    let server = null;
    ...
    function handleConnection(socket) {
        ...
        socket.on('data', (chunk) => {
            ...
            while (newline !== -1) {
                ...
                if (line)
                    pending = pending.then(() => handleLine(socket, line));
    ...
    async function handleLine(socket, line) {
        ...
        if (declared.kind === 'unsupported') {
            ...
            if (isNotification)
                options.log?.(`automation socket refused ${parsed.method}: ${detail}`);
            else
                respond(socket, jsonRpcErrorResponse(id, JSONRPC_INVALID_PARAMS, detail));
            return;
        }
        try {
            const context = connectionContexts.get(socket) ?? { metadata: { kind: 'external-local' } };
            ...
            if (result.kind === 'error') {
                if (!isNotification)
                    respond(socket, jsonRpcErrorResponse(id, result.code, result.errorMessage));
                return;
            }
            if (!isNotification)
                respond(socket, { jsonrpc: '2.0', id, result: result.value });
        }
        catch (error) {
