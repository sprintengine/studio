...
test('browser-tools', async () => {
  ...
  function harness(controlOverrides: Partial<BrowserToolsDeps['control']> = {}): Harness {
    ...
    const control: BrowserToolsDeps['control'] = {
      ...
      type: async (_tabId, target, text, options = {}) => {
        calls.push(
          `type:${target?.ref ?? '-'}:${text}:${options.clear ? 'clear' : ''}${options.submit ? 'submit' : ''}`,
        )
        return { ok: true }
      },
  ...
  async function main(): Promise<void> {
    ...
    await run(
      'click, type and press pass through with their options; a control error becomes a tool error',
      async () => {
        const h = harness()
        h.tabs.set('t1', { workspaceId: 'ws-1', state: tabState('t1', 'http://localhost:5173/') })
    ...
    await run('history rides status, snapshot and browser.actions', async () => {
      ...
      h.tabs.set('t1', {
        workspaceId: 'ws-1',
        state: tabState('t1', 'http://localhost:5173/', { controller: 'human' }),
      })
