import { isSupportedMcpProtocolVersion, negotiateMcpProtocolVersion, SUPPORTED_MCP_PROTOCOL_VERSIONS, } from '../../shared/mcp/protocol';
...
export function createMcpDispatcher(options) {
    return {
        async dispatch(method, params, context, gate) {
            switch (method) {
 ...
 */
export function applyDeclaredConnectionMetadata(established, params) {
    const text = (key) => {
    ...
    if (established.kind !== 'remote-tailnet')
        return declared;
    return {
        ...declared,
        kind: 'remote-tailnet',
 ...
 * error response.
 */
function quoteDeclared(value) {
    const rendered = JSON.stringify(value) ?? String(value);
    return rendered.length > 64 ? `${rendered.slice(0, 64)}…` : rendered;
}
export function jsonRpcErrorResponse(id, code, errorMessage) {
    return { jsonrpc: '2.0', id, error: { code, message: errorMessage } };
}
export function jsonRpcIdOf(value) {
    if (!isRecord(value))
        return null;
