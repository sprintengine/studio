...
import { MARKETPLACE_COMPONENT_KINDS, } from '../../shared/marketplace/manifest';
...
const LAUNCH_PERMISSION_PRESETS = ['default', 'auto_workspace'];
...
const AGENT_BACKED_ACTION_KINDS = ['spawn-agent', 'run-skill-loop'];
...
export function createAutomationTools(backends) {
    const now = backends.now ?? Date.now;
    const sleep = backends.sleep ?? ((ms) => new Promise((resolve) => setTimeout(resolve, ms)));
    async function waitFor(timeoutMs, probe) {
    ...
    function resolveWorkspaceRoot(workspaceId) {
        ...
        if (!workspace.folderPath) {
            return failure('workspace_without_folder', `Workspace "${workspaceId}" has no usable folder path in this app session; open it in the app first.`);
        }
    ...
    function resolveBacklogRoot(args, context) {
        ...
        if (workspace?.folderPath)
            return { root: workspace.folderPath };
        return failure('project_root_required', 'This connection has no resolvable workspace to default from; pass "projectRoot" (the absolute path to the project folder).');
    }
    function connectionWorkspace(context) {
    ...
    function resolveBacklogWorkspace(args, context) {
        if (args.projectRoot !== undefined) {
            const resolved = resolveBacklogRoot(args, context);
            if (!('root' in resolved))
                return resolved;
            const workspace = backends.getWorkspaceSyncSnapshot().state.workspaces.find((candidate) => candidate.folderPath === resolved.root) ?? null;
            if (!workspace?.folderPath) {
                return failure('workspace_not_open', `No open workspace uses the folder "${resolved.root}"; this tool needs that project open in the app.`);
            }
    ...
    function workspaceWindowId(workspaceId) {
    ...
    function agentTerminalSession(workspaceId, agentId, cliSessionId) {
        return (backends.listTerminalSessions().find((session) => session.kind === 'agent'
            && session.workspaceId === workspaceId
            && (session.agentId === agentId || (cliSessionId !== undefined && session.sessionId === cliSessionId))) ?? null);
    }
    ...
    function agentProjection(workspace, agentId) {
    ...
    function validatePermissionPreset(args) {
        ...
        if (args.permissionPreset === 'bypass_all') {
            return failure('permission_preset_not_allowed', 'Agents launched over the automation surface may not use permissionPreset "bypass_all". '
                + 'A person can set that preset in the app if it is genuinely needed.');
        }
        if (!LAUNCH_PERMISSION_PRESETS.includes(args.permissionPreset)) {
            return failure('invalid_arguments', `"permissionPreset" must be one of: ${LAUNCH_PERMISSION_PRESETS.join(', ')}.`);
        }
        return args.permissionPreset;
    }
    ...
    function resolveLaunchOptions(args) {
        ...
        if (args.worktree !== undefined) {
            ...
            const rawName = args.worktree.name;
        ...
        return {
            ...
            // allowed value.
            permissionPreset: optionalString(args.permissionPreset)
    ...
    async function launchConfiguredAgent(plan) {
        ...
        if (plan.worktreeRequested || plan.connectorId) {
            ...
            if ('error' in created) {
                return failure('worktree_unavailable', `Could not create an isolated worktree for the launch (${created.error}). The folder must be a git repository.`);
            }
        ...
        if (!live) {
            return failure('launch_confirmation_timeout', `Agent "${agentId}" was launched in workspace "${plan.workspaceId}" but no live terminal session registered within ${LAUNCH_CONFIRM_TIMEOUT_MS}ms; treat the launch as unverified. Read agent.status for the current state.`);
        }
    ...
    const workspaceList = {
        name: 'workspace.list',
        description: 'List the workspaces visible to the user in the running Multicode instance, with window assignment and agent ids. '
    ...
    const MOBILE_GATEWAY_COMMAND_TYPES = ['backlog.update', 'sprintengine.create'];
    const workspaceSnapshot = {
        name: 'workspace.snapshot',
        description: 'The mobile companion snapshot: sprint engines, backlog, automations, and workspaces as one versioned '
        ...
        inputSchema: {
            ...
            properties: {
                include: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Collections to include (sprintEngines, backlog, roleCatalogs, automations, desktopWorkspaces). '
    ...
    const workspaceMobileCommand = {
        name: 'workspace.mobile_command',
        description: 'Dispatch one mobile-control command envelope from a paired companion device — the same commands the phone '
        ...
        handler: async (args, context) => {
            ...
            if (invalidString)
                return invalidString;
            if (!MOBILE_GATEWAY_COMMAND_TYPES.includes(type)) {
                return failure('command_not_supported', `Mobile command "${type}" is not served over the gateway (served: ${MOBILE_GATEWAY_COMMAND_TYPES.join(', ')}).`);
            }
            ...
            const deviceId = metadata?.kind === 'remote-tailnet' && metadata.deviceId ? metadata.deviceId : `local:${metadata?.kind ?? 'unknown'}`;
    ...
    const workspaceCreate = {
        ...
        description: 'Create a workspace in the running Multicode instance. The main process owns the registry, so this '
    ...
    const agentLaunch = {
        name: 'agent.launch',
        description: 'Add a fully-configured agent to a workspace and start its CLI through the same renderer flow the UI uses. '
        ...
        inputSchema: {
            ...
            properties: {
                ...
                cli: {
                    type: 'string',
                    description: 'Agent CLI plugin id; defaults to the last selected CLI. cli.runtime.list enumerates the ids this '
                ...
                cliModel: {
                    type: 'string',
                    description: 'Model id for CLIs that support model selection; forwarded verbatim (the app does not validate it '
                ...
                connectorId: {
                    type: 'string',
                    description: 'Catalog connector id (e.g. "railway"). Attaches the connector\'s single-server MCP and driving skill and '
    ...
    const cliRuntimeList = {
        name: 'cli.runtime.list',
        description: 'List the agent CLIs this app can launch, with the model ids and reasoning-effort levels each one '
        ...
        handler: async (args) => {
            ...
            if (wanted && !plugins.some((plugin) => plugin.manifest.id === wanted)) {
                return failure('unknown_cli', `No agent CLI "${wanted}" is registered in this app. Call cli.runtime.list with no arguments for the ids it holds.`);
            }
            const clis = plugins
    ...
    const agentStatus = {
        name: 'agent.status',
        description: "Read one agent's launch state from the main process store plus its terminal session liveness from the terminal runtime.",
        inputSchema: {
            type: 'object',
            properties: {
    ...
    const terminalList = {
        name: 'terminal.list',
        description: 'List the terminal sessions open in this app: session id, agent name, CLI, working directory, '
     ...
     */
    function resolveTerminalWorkspace(args) {
        ...
        if (!workspaceName) {
            return failure('invalid_arguments', 'Name the workspace to open the terminal in: pass "workspaceId" or "workspaceName".');
        }
        ...
        if (matches.length > 1) {
            return failure('ambiguous_workspace_name', `${matches.length} workspaces are named "${workspaceName}". Name one by id instead: ${matches
    ...
    const terminalCreate = {
        ...
        description: 'Open a new agent terminal on the machine running this app and return its session id, ready to attach. '
        ...
        inputSchema: {
            ...
            properties: {
                ...
                workspaceName: {
                    type: 'string',
                    description: 'Target workspace by display name instead of id (case-insensitive). Refused when more than one '
                        + 'workspace carries the name.',
                },
                cli: {
                    type: 'string',
                    description: "Agent CLI plugin id; defaults to this machine's last-selected CLI. cli.runtime.list enumerates the "
        ...
        handler: async (args) => {
            ...
            if (requestedPreset !== undefined && typeof requestedPreset !== 'string')
    ...
    const PROJECT_ROOT_PROPERTY = {
        type: 'string',
        description: 'Absolute path to the project folder. Optional — when omitted, the tool targets the project this agent '
            + 'connection was launched from. Only needed when calling from outside a Studio-launched agent.',
    };
    const backlogList = {
        name: 'backlog.list',
        description: "List the project's Backlog items and epics (title, display id, status, type, triage axes, epic "
        ...
        handler: async (args, context) => {
            ...
            if (!listed.ok)
                return failure('backlog_unavailable', listed.message);
            return success({ workspaceKey: listed.key, items: listed.items });
        },
    };
    const backlogRead = {
        name: 'backlog.read',
        description: 'Read one Backlog item: parsed frontmatter fields plus the markdown body. '
        ...
        handler: async (args, context) => {
            ...
            if (!read.ok)
                return failure('backlog_read_failed', read.message);
            return success({ item: read.item, body: read.body });
        },
    };
    const automationList = {
        name: 'automation.list',
        description: "List a workspace's Automations (the outbound trigger/action definitions in .multi-code/automations). Read-only.",
        inputSchema: {
            type: 'object',
    ...
    // matters most.
    async function collectModuleRecords() {
        const snapshot = backends.getModuleRegistrySnapshot();
        if (!snapshot) {
            return failure('module_registry_unavailable', 'The running app has not reported its module registry yet (no window has finished starting). '
                + 'Retry once the app window is up.');
        }
        let installed = null;
        let thirdPartyUnavailable;
        try {
        ...
        const installedById = new Map((installed?.modules ?? []).map((module) => [module.manifest.id, module]));
        const records = snapshot.modules.map((entry) => {
    ...
    const moduleList = {
        name: 'module.list',
        description: 'List the capability modules this app has, as the user sees them: bundled and third-party, each with its '
    ...
    const moduleStatus = {
        name: 'module.status',
        description: 'Read one capability module: its manifest, the permissions it declares, the surfaces it contributes '
        ...
        handler: async (args) => {
            ...
            if (!record) {
                if (BUNDLED_MODULE_IDS.includes(id)) {
                    return failure('module_not_in_build', isDevOnlyModule(id) && collected.snapshot.channel === 'production'
    ...
    const marketplaceList = {
        ...
        description: 'Browse the extension marketplace index the app itself reads (same registry client, cache, and '
        ...
        handler: async (args) => {
            ...
            if (invalid)
                return invalid;
            if (args.provides !== undefined && !MARKETPLACE_COMPONENT_KINDS.includes(args.provides)) {
            ...
            const read = await backends.readMarketplaceRegistry(args.forceRefresh === true ? { forceRefresh: true } : undefined);
            if (!read.ok) {
                return failure('marketplace_unavailable', `${read.message} (registry ${read.registryUrl}, state ${read.state})`);
            }
            const provides = optionalString(args.provides);
    ...
    const BACKLOG_TYPES = ['epic', 'feature', 'bug', 'mockup', 'spike'];
    const BACKLOG_STATUSES = ['idea', 'ready', 'in_progress', 'needs_input', 'completed', 'archived'];
    const BACKLOG_DIFFICULTIES = ['xs', 's', 'm', 'l', 'xl'];
    const BACKLOG_CRITICALITIES = ['low', 'normal', 'high', 'critical'];
    const BACKLOG_RISKS = ['low', 'normal', 'high'];
    const backlogUpdate = {
        name: 'backlog.update',
        description: "Update one Backlog item's lifecycle or triage frontmatter: status, type, difficulty, criticality, risk, "
        ...
        inputSchema: {
            ...
            properties: {
                ...
                dependenciesPlanned: {
                    type: 'boolean',
                    description: "On an EPIC: the ordering pass over its children is finished — their dependsOn edges are authored, "
        ...
        handler: async (args, context) => {
            ...
            if (typeof path !== 'string')
                return path;
            const fields = ['status', 'type', 'difficulty', 'criticality', 'risk', 'epic'];
            ...
            if (args.status === null)
                return failure('invalid_arguments', 'Status cannot be cleared, only changed.');
            const resolved = resolveBacklogRoot(args, context);
            if (!('root' in resolved))
                return resolved;
            ...
            if (typeof args.status === 'string') {
                writes.push(() => backends.backlogWrite.updateStatus({ ...base, status: args.status }));
            }
            if ('type' in args) {
                writes.push(() => backends.backlogWrite.updateType({ ...base, type: (args.type ?? null) }));
            }
            if ('difficulty' in args || 'criticality' in args || 'risk' in args) {
                writes.push(() => backends.backlogWrite.updateTriage({
                    ...base,
                    ...('difficulty' in args
                    ...
                    ...('criticality' in args
                        ...
                        : {}),
                    ...('risk' in args ? { risk: (args.risk ?? null) } : {}),
                }));
            }
            if ('epic' in args) {
                writes.push(() => backends.backlogWrite.updateEpic({ ...base, epic: (args.epic ?? null) }));
            }
            if ('dependenciesPlanned' in args) {
                writes.push(() => backends.backlogWrite.updateDependenciesPlanned({
                    ...base,
                    dependenciesPlanned: args.dependenciesPlanned,
                }));
            }
            for (const write of writes) {
                const written = await write();
                if (!written.ok)
                    return failure('backlog_update_failed', written.message);
            }
    ...
    const backlogRepair = {
        name: 'backlog.repair',
        description: 'Repair one diagnosed Backlog integrity defect. This is deliberately narrow: replace embedded NUL bytes '
        ...
        handler: async (args, context) => {
            ...
            if (!repaired.ok)
                return failure('backlog_repair_failed', repaired.message);
            return success({ repaired });
        },
    };
    const backlogAssign = {
        name: 'backlog.assign',
        description: 'Record which agent is working a Backlog item (the working-agent link shown in the Backlog panel). '
        ...
        handler: async (args, context) => {
            ...
            const agent = workspace.agents[agentId];
            if (!agent)
                return failure('unknown_agent', `Agent "${agentId}" is not known in workspace "${workspace.id}".`);
            ...
            if (!written.ok)
                return failure('backlog_assign_failed', written.message);
    ...
    function renderBacklogSkillInvocation(cli, relativePath) {
        ...
        return (`Work the Backlog item at ${relativePath}. `
    ...
    const backlogWork = {
        name: 'backlog.work',
        description: 'Hand a Backlog item to a freshly launched agent in one call: launches a configured agent whose first input '
        ...
        handler: async (args, context) => {
            ...
            const read = await backends.readBacklogItem(resolved.root, path);
            if (!read.ok)
                return failure('backlog_item_not_found', read.message);
            if (read.item.status === 'completed' || isArchivedBacklogPath(read.item.relativePath)) {
                ...
                return failure('backlog_item_not_workable', `Backlog item ${read.item.relativePath} is ${reason} and cannot be handed to an agent.`);
            }
    ...
    function automationsFrontDoorOrFailure() {
        const frontDoor = backends.getAutomationsFrontDoor();
        if (!frontDoor) {
            return failure('automations_module_unavailable', 'The Automations module is disabled or not loaded in this app session; enable it in Settings → Modules.');
        }
        return frontDoor;
    }
    const automationCreate = {
        name: 'automation.create',
        description: 'Create an Automation definition through the same validated pipeline the UI uses (provider/permission '
        ...
        inputSchema: {
            ...
            properties: {
                ...
                definition: {
                    type: 'object',
                    description: 'Automation definition draft: { name, trigger: { kind, config }, action: { kind, config }, '
        ...
        handler: async (args) => {
            ...
            if (preset === 'bypass_all') {
                return failure('permission_preset_not_allowed', 'Automations created over the automation surface may not run on permissionPreset "bypass_all", which is '
            ...
            if (!('createDefinition' in frontDoor))
                return frontDoor;
            const created = await frontDoor.createDefinition({ workspaceRoot: resolved.root, definition: args.definition });
            if (!created.ok)
                return failure(created.code || 'automation_create_failed', created.message);
            return success({ automation: created.value });
        },
    };
    const automationRun = {
        name: 'automation.run',
        description: 'Run an existing schedule-triggered Automation now (the same "Run now" the panel offers). The run record '
        ...
        handler: async (args) => {
            ...
            if (!('root' in resolved))
                return resolved;
            const frontDoor = automationsFrontDoorOrFailure();
            if (!('runNow' in frontDoor))
                return frontDoor;
            const ran = await frontDoor.runNow({ workspaceRoot: resolved.root, automationId });
            if (!ran.ok)
                return failure(ran.code || 'automation_run_failed', ran.message);
    ...
    const SPRINT_TASK_MUTATION_ROLES = [
        'architect',
        'product',
        'developer',
        'frontend',
    ...
    function sprintStatePath(root, slug) {
    ...
    function resolveSprintStatePath(args) {
        const workspaceId = requireString(args, 'workspaceId');
        if (typeof workspaceId !== 'string')
            return workspaceId;
    ...
    async function ensureSprintRunExists(statePath) {
        const projection = await backends.readSprintEngineProjection(statePath);
        if (!projection.ok)
            return failure('sprint_not_found', projection.message);
        return null;
    }
    const sprintList = {
        name: 'sprint.list',
        description: "List a workspace's Sprint Engine runs from their on-disk state (newest first). Read-only; each entry's "
    ...
    const sprintStatus = {
        name: 'sprint.status',
        description: "One Sprint Engine run's current projection (goal, tasks, artifacts, roster, completion) read from disk, "
        ...
        handler: async (args) => {
            ...
            const automationMode = mode.ok ? mode.record?.desiredMode ?? 'manual' : null;
            return success({
    ...
    const sprintSetMode = {
        ...
        description: "Set a Sprint Engine run's automation mode through the main-owned intent service. \"manual\" pauses the "
        ...
        handler: async (args) => {
            ...
            if (typeof mode !== 'string')
                return mode;
            if (!SPRINT_AUTOMATION_MODES.includes(mode)) {
            ...
            if (!written.ok)
                return failure('sprint_mode_failed', written.message);
            return success({ mode: written.record.desiredMode, changed: written.changed });
        },
    };
    const sprintResume = {
        name: 'sprint.resume',
        description: "Re-arm a stopped Sprint Engine run's scheduler for its current mode (the board's Resume). Fire-and-forget: "
    ...
    const sprintCancel = {
        name: 'sprint.cancel',
        description: 'Cancel a Sprint Engine run: writes run/task status through the engine cancel op, then tears down the '
    ...
    const sprintCreate = {
        name: 'sprint.create',
        description: 'Create a Sprint Engine run in a new workspace, through the same creation path the app wizard uses. '
        ...
        inputSchema: {
            ...
            properties: {
                ...
                sourceRef: {
                    type: 'string',
                    description: 'Project-relative backlog item or epic to plan the run from, e.g. "backlog/epics/foo.md". '
                ...
                sourceRefs: {
                    ...
                    items: { type: 'string' },
                    minItems: 1,
                    description: 'Several project-relative backlog items and/or epics to start ONE sprint from, e.g. '
                ...
                roster: {
                    type: 'object',
                    description: 'Explicit roster as role id -> staffed flag, e.g. {"architect":1,"developer":1,"spec_reviewer":1}. '
                ...
                useWorktrees: {
                    type: 'boolean',
                    description: 'Run the sprint in ONE shared git worktree (a per-run isolated checkout on its own branch), '
                ...
                isolation: {
                    ...
                    description: 'Where this sprint works. "none" = the project folder itself (agents edit the working tree you '
                ...
                runtime: {
                    type: 'object',
                    description: 'The run-level execution runtime — the same picker the New Sprint dialog shows above the roster '
                ...
                roleClis: {
                    type: 'object',
                    description: 'Per-role agent CLI, role id -> CLI plugin id, e.g. {"architect":"claude-code","developer":"codex"} '
                ...
                roleModels: {
                    type: 'object',
                    description: 'Per-role launch model, role id -> model id, e.g. {"architect":"claude-opus-5","developer":"claude-sonnet-5"}. '
                ...
                roleEfforts: {
                    type: 'object',
                    description: 'Per-role reasoning-effort level, role id -> level id, e.g. {"architect":"high"}. Same rules as '
                ...
                maxConcurrentAgents: {
                    ...
                    minimum: 1,
                    maximum: 10,
                    description: 'The run\'s ceiling on agents working at once (the dialog\'s "Max concurrent agents"). Default 3. '
                ...
                intake: {
                    type: 'string',
                    enum: ['direct', 'planned'],
                    description: 'How the run gets its task graph. "direct" imports it from the source epic\'s child items at '
        ...
        handler: async (args) => {
            ...
            if (args.sourceRefs !== undefined) {
                ...
                for (const entry of args.sourceRefs) {
                    ...
                    if (!cleaned.includes(entry.trim()))
            ...
            if (args.isolation !== undefined && !['none', 'sprint', 'task'].includes(args.isolation)) {
            ...
            if (args.isolation !== undefined && args.useWorktrees !== undefined
                ...
                return failure('invalid_arguments', `"isolation": "${args.isolation}" contradicts "useWorktrees": ${String(args.useWorktrees)}. Pass one, or agreeing values.`);
            }
            let roster;
            if (args.roster !== undefined) {
                ...
                roster = {};
                for (const [role, count] of Object.entries(args.roster)) {
            ...
            if (args.runtime !== undefined) {
                ...
                const entries = {
                    ...(optionalString(raw.cli) ? { cli: raw.cli.trim() } : {}),
                    ...(optionalString(raw.model) ? { model: raw.model.trim() } : {}),
                    ...(optionalString(raw.effort) ? { effort: raw.effort.trim() } : {}),
                };
            ...
            const roleOverrides = {};
            for (const key of ['roleClis', 'roleModels', 'roleEfforts']) {
            ...
            if (!created.ok)
                return failure(created.code, created.message);
            ...
            const workspace = findWorkspace(workspaceId);
            if (!workspace) {
                return failure('creation_confirmation_failed', `Sprint workspace "${workspaceId}" was created but is not readable from the workspace registry; treat the creation as unverified and read workspace.status.`);
            }
            if (startRunner) {
                const architectLive = await waitFor(LAUNCH_CONFIRM_TIMEOUT_MS, () => {
                    const session = backends
                        .listTerminalSessions()
                        .find((candidate) => candidate.kind === 'agent' && candidate.workspaceId === workspaceId && candidate.processAlive);
                    return session ?? null;
                });
                if (!architectLive) {
                    return failure('launch_confirmation_timeout', `Sprint run and workspace "${workspaceId}" were created, but no live agent terminal registered within ${LAUNCH_CONFIRM_TIMEOUT_MS}ms — the architect start is unverified. Read sprint.status and workspace.status for the current state.`);
                }
            }
    ...
    async function epicOrderingWarnings(folderPath, sourceRef, intake) {
        if (intake === 'planned')
            return [];
        ...
        if (!sourceRef.replace(/\\/g, '/').toLowerCase().startsWith('backlog/epics/'))
            return [];
        const read = await backends.readBacklogItem(folderPath, sourceRef).catch(() => null);
        if (!read?.ok || !read.item.isEpic || read.item.dependenciesPlanned === true)
            return [];
        const mark = `Set "dependenciesPlanned: true" on ${sourceRef} once its children's dependsOn order is authored `
    ...
    const sprintArtifactApprove = {
        name: 'sprint.artifact.approve',
        description: "Approve a Sprint Engine run's review artifact (plan, spec review, etc.), the human-proxy approval the board "
        ...
        handler: async (args) => {
            ...
            if (missing)
                return missing;
            const reviewed = await backends.reviewSprintArtifact({ statePath: resolvedRun.statePath, artifactId, ...(optionalString(args.feedback) ? { feedback: optionalString(args.feedback) } : {}) }, 'approve');
            if (!reviewed.ok)
                return sprintCommandResultFailure('sprint_artifact_approve_failed', reviewed);
            return success({ approved: { slug: resolvedRun.slug, artifactId } });
        },
    };
    const sprintArtifactRequestChanges = {
        name: 'sprint.artifact.request_changes',
        description: "Request changes on a Sprint Engine run's review artifact, returning it to its author with required feedback. "
        ...
        handler: async (args) => {
            ...
            if (missing)
                return missing;
            const reviewed = await backends.reviewSprintArtifact({ statePath: resolvedRun.statePath, artifactId, feedback }, 'request-changes');
            if (!reviewed.ok)
                return sprintCommandResultFailure('sprint_artifact_request_changes_failed', reviewed);
            return success({ requestedChanges: { slug: resolvedRun.slug, artifactId } });
        },
    };
    const sprintTaskComment = {
        name: 'sprint.task.comment',
        description: "Add a comment to a Sprint Engine task — the same board comment an operator leaves. Direct-main, keyed "
        ...
        handler: async (args) => {
            ...
            if (!commented.ok)
                return sprintCommandResultFailure('sprint_task_comment_failed', commented);
            return success({ commented: { slug: resolvedRun.slug, taskId } });
        },
    };
    const sprintTaskResolveInput = {
        name: 'sprint.task.resolve_input',
        description: "Answer a Sprint Engine task's needs_input question so its owner can resume. Direct-main, keyed workspaceId + "
        ...
        handler: async (args) => {
            ...
            if (!resolved.ok)
                return sprintCommandResultFailure('sprint_task_resolve_input_failed', resolved);
            return success({ resolved: { slug: resolvedRun.slug, taskId, complete: args.complete === true } });
        },
    };
    const sprintTaskSetStatus = {
        name: 'sprint.task.set_status',
        description: "Set a Sprint Engine task's status (todo, in_progress, review, needs_input, done, canceled). Direct-main, "
        ...
        handler: async (args) => {
            ...
            if (typeof status !== 'string')
                return status;
            if (!SPRINT_TASK_STATUSES.includes(status)) {
            ...
            if (!set.ok)
                return sprintCommandResultFailure('sprint_task_set_status_failed', set);
            return success({ statusSet: { slug: resolvedRun.slug, taskId, status } });
        },
    };
    const sprintTaskCreate = {
        name: 'sprint.task.create',
        description: 'Add a task to a Sprint Engine run mid-flight. Direct-main, keyed workspaceId + slug. `role` must be an '
        ...
        handler: async (args) => {
            ...
            if (typeof role !== 'string')
                return role;
            if (!SPRINT_TASK_MUTATION_ROLES.includes(role)) {
            ...
            if (!created.ok)
                return sprintCommandResultFailure('sprint_task_create_failed', created);
            return success({ created: { slug: resolvedRun.slug, title, role } });
        },
    };
    const sprintTaskUpdate = {
        name: 'sprint.task.update',
        description: "Update an existing Sprint Engine task's fields (title, description, role, acceptanceCriteria, "
        ...
        handler: async (args) => {
            ...
            if (invalid)
                return invalid;
            if (args.role !== undefined && !SPRINT_TASK_MUTATION_ROLES.includes(args.role)) {
                return failure('invalid_arguments', `"role" must be one of: ${SPRINT_TASK_MUTATION_ROLES.join(', ')}.`);
            }
            const badArray = firstInvalidStringArray(args, ['acceptanceCriteria', 'implementationNotes', 'notes']);
            if (badArray)
            ...
            if (!updated.ok)
                return sprintCommandResultFailure('sprint_task_update_failed', updated);
            return success({ updated: { slug: resolvedRun.slug, taskId } });
        },
    };
    const sprintPrCreate = {
        name: 'sprint.pr.create',
        description: "Open a Sprint Engine run's pull request through the engine's own VCS command. Direct-main, keyed workspaceId "
        ...
        handler: async (args) => {
            ...
            if (!created.ok)
                return sprintCommandResultFailure('sprint_pr_failed', created);
            return success({ slug: resolvedRun.slug, vcs: sprintVcsFromCommandResult(created.data) });
        },
    };
    const sprintPrStatus = {
        name: 'sprint.pr.status',
        description: "Refresh and read a Sprint Engine run's pull-request merge state. Direct-main, keyed workspaceId + slug; runs "
        ...
        handler: async (args) => {
            ...
            if (!refreshed.ok)
                return sprintCommandResultFailure('sprint_pr_status_failed', refreshed);
            return success({ slug: resolvedRun.slug, vcs: sprintVcsFromCommandResult(refreshed.data) });
        },
    };
    const sprintTokenUsage = {
        name: 'sprint.token_usage',
        description: "A Sprint Engine run's token accounting report (per-run totals, per-agent and per-task breakdowns, measurement "
    ...
    function roadmapFrontDoorOrFailure() {
        const frontDoor = backends.getRoadmapFrontDoor();
        if (!frontDoor) {
            return failure('horizon_module_unavailable', 'The Roadmap orchestrator is disabled or not loaded in this app session; enable the Roadmap module in Settings → Modules.');
        }
        return frontDoor;
    }
    ...
    function roadmapResult(code, outcome, data) {
        if (!outcome.ok)
            return failure(code, outcome.message ?? 'The roadmap operation failed.');
        return success(data);
    }
    const roadmapStatus = {
        name: 'horizon.status',
        description: "Read this Multicode's single instance roadmap — the board model the global surface renders. Each lane lists its "
        ...
        handler: async () => {
            ...
            if ('content' in frontDoor)
                return frontDoor;
            const board = await frontDoor.readBoard();
            return success({ roadmap: board });
        },
    };
    const roadmapAddStep = {
        name: 'horizon.add_step',
        description: 'Add a step to the instance roadmap: a backlog item, or an epic as one step (snapshotting its children, matching '
    ...
    const roadmapRemoveStep = {
        name: 'horizon.remove_step',
        description: 'Remove a top-level step from the instance roadmap by its stored (authored) ref — the `ref` horizon.status shows '
    ...
    const roadmapReorder = {
        name: 'horizon.reorder',
        description: "Move a step to a new position in the plan. `ref` is the step's authored ref (from horizon.status); `toIndex` is "
    ...
    const roadmapSkip = {
        name: 'horizon.skip',
        description: 'Skip a step: remove it from the plan so the lane\'s frontier advances past it, recording a reason as an inert audit '
    ...
    const roadmapApprove = roadmapSteerTool({
        ...
        description: "Approve the next start for a lane waiting on you (attention \"approval\"). Consumes the lane's pending approval so "
    ...
    const roadmapMerge = roadmapSteerTool({
        ...
        description: "Merge a lane's delivered run (attention \"merge\") through the engine's own single-repo merge primitive — the same "
    ...
    const roadmapPause = roadmapSteerTool({
        ...
        description: 'Hold a lane at your request: the orchestrator stops advancing/merging/starting-next for it without touching the '
    ...
    const roadmapResume = roadmapSteerTool({
        ...
        description: 'Resume a paused or parked lane. A manual pause continues exactly where it was. A merge park RETRIES the merge rather '
        ...
        extraProperties: {
            replanDeliveredRun: {
                type: 'boolean',
                description: 'Acknowledge that resuming discards a run that already delivered, and re-plan a fresh sprint from the same item. '
    ...
    const HORIZON_POLICY_PROPERTIES = {
        roster: {
            type: 'string',
            description: 'Saved ROSTER name every sprint this horizon starts is staffed with (agent configuration — NOT a run name). '
        ...
        permissions: {
            ...
            description: 'The CLI permission preset this horizon\'s sprints SPAWN their agents with. Omit for bypass — a horizon runs '
    ...
    function horizonPolicyFromArgs(args) {
        const policy = {};
        for (const key of ['advance', 'merge', 'concurrency', 'permissions']) {
            if (args[key] !== undefined)
                policy[key] = args[key];
        }
        if ('roster' in args)
            policy.roster = args.roster === null ? undefined : args.roster;
        return policy;
    }
    const horizonCreate = {
        name: 'horizon.create',
        description: 'Create a horizon: a named plan whose ordered steps each run as one sprint. Steps are backlog item or epic paths, '
        ...
        handler: async (args) => {
            ...
            const outcome = await frontDoor.createHorizon({
                ...
                policy: horizonPolicyFromArgs(args),
                ...(args.start === true ? { start: true } : {}),
                actor: 'automation',
            });
            if (!outcome.ok)
                return failure('horizon_create_failed', outcome.message);
            return success({
                created: {
    ...
    const horizonConfigure = {
        name: 'horizon.configure',
        description: "Set the active horizon's policy: which roster staffs its sprints, whether it advances and merges on its own, its "
        ...
        handler: async (args) => {
            ...
            if (!outcome.ok)
                return failure('horizon_configure_failed', outcome.message ?? 'The horizon policy could not be written.');
...
function terminalSessionProjection(session) {
    return {
        sessionId: session.sessionId,
        kind: session.kind,
...
function cliRuntimeProjection(plugin) {
...
function registeredModuleReport(entry, view) {
    return {
        id: entry.id,
...
function thirdPartyLaunchReport(view) {
    return {
...
function marketplaceEntryMatches(plugin, needle) {
    return [
        plugin.name,
        plugin.id,
        plugin.category,
...
function marketplaceEntryProjection(plugin) {
    return {
        id: plugin.id,
        name: plugin.name,
...
function resolvedActionPermissionPreset(definition) {
...
function isArchivedBacklogPath(relativePath) {
...
function sprintCommandResultFailure(code, result) {
...
function sprintVcsFromCommandResult(data) {
    if (typeof data.projectionContent !== 'string')
        return null;
    try {
        const parsed = JSON.parse(data.projectionContent);
...
function firstInvalidStringArray(args, keys) {
...
function firstInvalidOptionalString(args, keys) {
    for (const key of keys) {
