...
const VARIABLE_TYPES = ['string', 'enum', 'boolean', 'number'];
const PROVIDER_TYPES = ['model-provider', 'agent-harness'];
const PROVIDER_ADAPTER_KINDS = ['declarative', 'trusted-executable'];
const PROVIDER_AUTH_TYPES = ['api-key'];
...
export function validateManifestStructure(value) {
    const issues = [];
    if (!isObject(value)) {
    ...
    if (!cli.ok)
        return { ok: false, issues: cli.issues };
    return { ok: true, manifest: value };
}
function validateProviderManifestStructure(value, issues) {
    ...
    if (typeof value.providerType !== 'string'
        || !PROVIDER_TYPES.includes(value.providerType)) {
        issues.push({
            path: 'providerType',
...
function validateProviderAdapter(value, issues) {
    if (!isObject(value)) {
        issues.push({ path: 'adapter', message: 'adapter must be an object when present.' });
        return;
    }
    if (typeof value.kind !== 'string'
        || !PROVIDER_ADAPTER_KINDS.includes(value.kind)) {
        issues.push({
            path: 'adapter.kind',
...
function validateOpenAiCompatible(value, issues) {
    if (!isObject(value)) {
    ...
    if ('chatCompletionsPath' in value && value.chatCompletionsPath !== undefined) {
        if (typeof value.chatCompletionsPath !== 'string'
    ...
    if ('modelsPath' in value && value.modelsPath !== undefined) {
        if (typeof value.modelsPath !== 'string'
...
function validateSignature(value, issues) {
    if (!isObject(value)) {
...
function validateProviderModels(value, issues) {
...
function validateProviderAuth(value, issues) {
    if (!isObject(value)) {
        issues.push({ path: 'auth', message: 'auth must be an object when present.' });
        return;
    }
    if (typeof value.type !== 'string'
        || !PROVIDER_AUTH_TYPES.includes(value.type)) {
        issues.push({
            path: 'auth.type',
...
function validateVariables(value, issues) {
    if (!isObject(value)) {
    ...
    for (const [name, decl] of Object.entries(value)) {
        ...
        if (typeof decl.type !== 'string' || !VARIABLE_TYPES.includes(decl.type)) {
            issues.push({
                path: `${path}.type`,
...
function requireString(value, key, issues, pattern, rootPath) {
