...
import { buildAuthoredRoleManifest, isValidRoleId, parseRoleManifest, serializeRoleManifest, validateRoleManifest, } from '../shared/sprintengine/role-manifest';
...
export function defaultUserRoleRegistryRoot() {
...
function safeRoleTargets(root, id) {
    if (!isValidRoleId(id) || basename(id) !== id)
        return null;
    ...
    if (!roleFile.startsWith(prefix) || !skillDir.startsWith(prefix))
        return null;
...
export async function resolveRoleInstallTargets(srcDir) {
        ...
        if (hasRolesDir) {
            ...
            for (const entry of await readDirSafe(rolesPath)) {
                if (!entry.isDirectory() && isJsonFile(entry.name))
                    roleFiles.push(join(rolesPath, entry.name));
            }
        }
        if (hasSkillsDir) {
            ...
            for (const entry of await readDirSafe(skillsPath)) {
                if (entry.isDirectory())
                    skillDirs.push(join(skillsPath, entry.name));
            }
        }
...
export async function installRoleFolder(srcDir, root) {
...
export async function loadUserRoleManifests(root) {
    const rolesDir = join(root, 'roles');
    const roles = [];
    const rejected = [];
...
export async function saveUserRole(input, root) {
...
// filesystem work.
export async function deleteUserRole(id, root) {
...
export async function getUserRole(id, root) {
    const targets = safeRoleTargets(root, id);
    if (!targets)
        return { ok: false };
    let source;
