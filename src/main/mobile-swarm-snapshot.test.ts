...
async function assertSnapshotOmitsNonMobileStatePayloads(): Promise<void> {
  ...
  assert.equal(snapshot.artifacts[0].path, 'swarm/team/product-requirements.md')
}

async function assertPublishingIsThrottled(): Promise<void> {
  const statePath = await writeStateFixture({
    swarm: { name: 'Throttle Fixture', updatedAt: generatedAt },
    tasks: [task('T1', 'done', [])],
    artifacts: [],
  })
...
async function writeStateFixture(state: Record<string, unknown>): Promise<string> {
  const workspacePath = await mkdtemp(join(tmpdir(), 'multicode-swarm-snapshot-'))
  const teamDirectory = join(workspacePath, 'swarm', 'team')
...
function artifact(id: string, kind: string, status: string, taskId: string): Record<string, unknown> {
  return {
    ...
    kind,
    title: `Artifact ${id}`,
    path: 'swarm/team/product-requirements.md',
    status,
    createdBy: 'product',
    taskId,
  }
