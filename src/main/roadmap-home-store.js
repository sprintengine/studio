...
function roadmapHomePath(userDataDir) {
...
// to a wrong root.
export function readRoadmapHomeProjectPath(userDataDir) {
...
export async function writeRoadmapHomeProjectPath(userDataDir, projectRoot) {
    const path = roadmapHomePath(userDataDir);
    await mkdir(userDataDir, { recursive: true });
    const record = { path: projectRoot?.trim() ?? '' };
