...
export async function resolveRepoRoot(repoRoot) {
...
export function resolveWorktreeDestination(containerPath, destinationPath) {
    ...
    if (comparableDestination !== comparableContainer
...
export async function validateBaseRef(repoRoot, baseRef) {
...
export function toWorktreeResult(result, data, successMessage = null) {
    if (result.ok) {
...
async function getGitRepoRoot(folderPath) {
    try {
