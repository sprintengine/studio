...
import { AgentControlPlaneToken, ReviewChangeSetServiceToken, ReviewGuideTerminalServiceToken, SprintEngineLaunchSettingsToken, TerminalRuntimeToken, WorkspaceSyncServiceToken, } from '../module-host/service-tokens';
import { createReviewChangeSetService } from '../review/changeset-service';
import { BRIEF_RUN_EVENT_TOPIC } from '../review/brief-run-service';
import { createReviewGuideTerminalService, recordGuideRunEvent, REVIEW_GUIDE_SKILL_ID, } from '../review/guide-terminal-service';
...
export const reviewModule = {
    ...
    registerMain(host) {
        const changeSetService = host.provideService(ReviewChangeSetServiceToken, () => createReviewChangeSetService());
        ...
        const guideTerminals = host.provideService(ReviewGuideTerminalServiceToken, () => createReviewGuideTerminalService({
            ...
            terminal: {
                ...
                setReapExempt: (sessionId, exempt) => terminalRuntime.ipcHandlers.setTerminalReapExempt(sessionId, exempt),
                onAgentSessionExit: (listener) => terminalRuntime.registerAgentSessionExitListener(listener),
            },
            resolveSkillInvocation: (cli) => resolveSkillInvocation(getPluginById(cli)?.manifest.skillIntegration, REVIEW_GUIDE_SKILL_ID),
        ...
        host.registerMcpTools(createReviewGatewayTools({
            listOpenProjectRoots: () => workspaceSyncService
                .getSnapshot()
            ...
            emitBriefRunEvent: (event) => {
                ...
                if (event.phase === 'done')
                    guideTerminals.clearReapExempt(event.workspaceId);
                host.emit(BRIEF_RUN_EVENT_TOPIC, event);
            },
        }));
