...
import { registerSprintEngineAutomationIpc, bindSprintEngineAutomationChangedEmit } from '../ipc/sprintengine-automation-ipc'
import { registerSprintRuntimeIpc, bindSprintRuntimeOpEmit, bindSprintRunsChangedEmit } from '../ipc/sprint-runtime-ipc'
...
import { SPRINT_ENGINE_EVENTS } from '../../shared/sprintengine/ipc-channels'
...
export type SprintEngineModuleServices = Pick<
  AppServices,
...
export function createSprintEngineModule(services: SprintEngineModuleServices): CapabilityModule {
  return {
    manifest: {
      id: 'sprint-engine',
      ...
      permissions: ['ipc:invoke', 'ipc:workspace-read', 'ipc:workspace-write', 'ipc:agents'],
    },
    registerMain(host) {
      ...
      registerSprintEngineRoleRegistryIpc(host)

      bindSprintEngineAutomationChangedEmit((event) => {
        host.emit(SPRINT_ENGINE_EVENTS.automationChanged, event)
      })
      bindSprintRuntimeOpEmit((op) => {
        host.emit(SPRINT_ENGINE_EVENTS.runtimeOp, op)
      })
      bindSprintRunsChangedEmit((event) => {
        host.emit(SPRINT_ENGINE_EVENTS.runsChanged, event)
      })
      host.onShutdown(() => {
        bindSprintEngineAutomationChangedEmit(null)
        bindSprintRuntimeOpEmit(null)
        bindSprintRunsChangedEmit(null)
      })
