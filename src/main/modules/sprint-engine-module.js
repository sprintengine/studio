...
export const sprintEngineModule = {
    manifest: {
        id: 'sprint-engine',
    ...
    registerMain(host) {
        ...
        const withRoadmapHomeProject = (roots) => {
        ...
        mcpHub.claimOwnership({
            onSpawnFailure: (message) => host.notify({ severity: 'error', title: 'Sprint Engine MCP hub failed to start', body: message }),
        });
        ...
        host.onStartup(async () => {
            try {
                ...
                if (report.watching.length === 0 && report.skippedStale.length === 0)
                    return;
                void writeDiagnosticLog({
                    level: 'info',
                    source: 'sprintengine',
                    title: 'Sprint pull-request merge polling armed',
                    message: `${report.watching.length} of ${report.discovered} sprint runs have an open pull request and are `
...
function readTokenUsageCached(statePath) {
    const now = Date.now();
    ...
    for (const [key, entry] of tokenUsageCache) {
        if (now - entry.at >= TOKEN_USAGE_CACHE_TTL_MS && key !== statePath)
            tokenUsageCache.delete(key);
    }
    return report;
}
const HUB_STATE_TO_SIDECAR_STATE = {
    stopped: 'stopped',
    starting: 'starting',
    ready: 'running',
    failed: 'failed',
};
function hubSidecarStatus(status) {
