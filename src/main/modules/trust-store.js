...
export function trustedModulesPath(userDataDir) {
    return join(userDataDir, FILE_NAME);
}
export function parseTrustedModules(raw) {
    try {
        ...
        for (const [id, fingerprint] of Object.entries(parsed)) {
            if (id.length > 0 && typeof fingerprint === 'string' && fingerprint.length > 0)
...
export function readTrustedModulesSync(userDataDir) {
...
async function persist(userDataDir, trusted) {
    const path = trustedModulesPath(userDataDir);
    try {
        await mkdir(userDataDir, { recursive: true });
        const tmp = `${path}.tmp`;
        const record = {};
        for (const id of [...trusted.keys()].sort())
            record[id] = trusted.get(id);
...
export async function setModuleTrust(userDataDir, id, fingerprint) {
