...
import { AgentControlPlaneToken, AgentLaunchServiceToken, SprintCreateServiceToken, CompanionAgentServiceToken, CompanionAgentsModuleServiceToken, GitHubTokenStoreToken, ModuleStorageToken, EntitlementServiceToken, MulticodeAuthToken, SprintEngineArtifactsToken, SprintEngineAutomationServiceToken, SprintEngineLaunchSettingsToken, SprintEngineMcpHubToken, SprintPullRequestMergePollerToken, SprintRuntimeToken, TerminalRuntimeToken, WorkspaceContextToken, WorkspaceRegistryToken, WorkspaceServiceToken, WorkspaceSyncServiceToken, } from '../module-host/service-tokens';
...
export function createAgentRuntimeModule(services, options) {
    return {
        ...
        registerMain(host) {
            ...
            host.provideService(WorkspaceServiceToken, () => createModuleWorkspaceService({ workspaceSync: services.workspaceSyncService }));
            // Read-only workspace context (id → root/name/mode), read from the same
            // registry the create flow writes.
            host.provideService(WorkspaceContextToken, () => createModuleWorkspaceContextService({
            ...
            host.provideService(ModuleStorageToken, () => createModuleStorageRegistry({ userDataDir: () => app.getPath('userData') }));
            ...
            host.provideService(CompanionAgentsModuleServiceToken, () => createCompanionAgentsModuleRegistry({
