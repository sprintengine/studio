...
import { createAutomationsEngine, projectFoldersFromWorkspaceSyncSnapshot, } from '../automations/engine';
import { allowAutomationProvider, createBuiltInAutomationProviderRegistry, executableTriggerProviders, } from '../automations/provider-registry';
...
import { SprintCreateServiceToken, AutomationsAppFrontDoorToken, AutomationsEngineToken, AutomationsModuleServiceToken, AutomationsProviderRegistryToken, RoadmapAppFrontDoorToken, SprintEngineAutomationFrontDoorsToken, SwitchboardAutomationFrontDoorsToken, AgentLaunchServiceToken, TerminalRuntimeToken, WorkspaceSyncServiceToken, } from '../module-host/service-tokens';
import { AUTOMATIONS_DEFINITIONS_CHANGED_CHANNEL, AUTOMATIONS_RUN_EVENT_CHANNEL, } from '../../shared/automations/contracts';
import { SPRINT_ENGINE_AUTOMATION_INTEGRATION_ID, } from '../automations/actions/sprint-engine';
import { SWITCHBOARD_AUTOMATION_INTEGRATION_ID, WATCHTOWER_AUTOMATION_INTEGRATION_ID, } from '../automations/actions/switchboard';
...
export function broadcastAutomationsRunEvent(event, windows = BrowserWindow.getAllWindows()) {
...
export function broadcastAutomationsDefinitionsChanged(event, windows = BrowserWindow.getAllWindows()) {
    for (const window of windows) {
        if (window.isDestroyed() || window.webContents.isDestroyed())
            continue;
        try {
...
export function createAutomationsModule(options = {}) {
    return {
        manifest: {
            id: 'automations',
        ...
        registerMain(host) {
            ...
            const switchboardFrontDoors = serviceBackedSwitchboardFrontDoors(() => host.getService(SwitchboardAutomationFrontDoorsToken));
            const sprintEngineFrontDoors = serviceBackedSprintEngineFrontDoors(() => host.getService(SprintEngineAutomationFrontDoorsToken));
            ...
            const getTriggerProviders = () => executableTriggerProviders(getTriggerProviderRegistrations(), checkProviderPermission);
            ...
            const resolveUserDataDir = () => app.getPath('userData');
            const roadmapOrchestrator = createRoadmapOrchestrator(createRoadmapOrchestratorPorts({
                ...
                getWorkspaceRoots: () => projectFoldersFromWorkspaceSyncSnapshot(workspaceSyncService.getSnapshot()).map((folder) => folder.folderPath),
            ...
            const engine = host.provideService(AutomationsEngineToken, () => (options.createEngine ?? createAutomationsEngine)({
                ...
                onEvaluation: () => {
                    ...
                    if (options.isRoadmapReconcileEnabled && !options.isRoadmapReconcileEnabled())
                        return;
                ...
                openRunPullRequest: (input) => openAutomationRunPullRequest({
                ...
                getLiveAgentExecutionIds: () => terminalRuntime.getLiveAgentExecutionIds().map((execution) => execution.executionId),
            }));
            ...
            // engine.
            const unregisterAgentPhaseListener = terminalRuntime.registerAgentPhaseListener((event) => engine.noteAgentPhase(event));
            const unregisterAgentExitListener = terminalRuntime.registerAgentSessionExitListener((event) => engine.finalizeRunOnAgentExit({
            ...
            const onDefinitionsChanged = async (workspaceRoot) => {
                try {
            ...
            moduleAutomations = host.provideService(AutomationsModuleServiceToken, () => createModuleAutomationsRegistry({
...
function createFirstPartyAutomationIntegrationResolver(input) {
    return (id) => {
        if (id === SWITCHBOARD_AUTOMATION_INTEGRATION_ID)
            return input.hasSwitchboard();
        if (id === WATCHTOWER_AUTOMATION_INTEGRATION_ID)
            return input.hasSwitchboard();
        if (id === SPRINT_ENGINE_AUTOMATION_INTEGRATION_ID)
            return input.hasSprintEngine();
        return undefined;
    };
}
function serviceBackedSwitchboardFrontDoors(resolve) {
    return {
        readAllTasks: async (input) => resolve()?.readAllTasks(input) ?? { ok: false, message: 'Switchboard is unavailable.' },
        tickRunner: async (input) => resolve()?.tickRunner(input) ?? { ok: false, message: 'Switchboard is unavailable.' },
        startWatchtowerReview: async (input) => resolve()?.startWatchtowerReview(input) ?? { ok: false, message: 'Watchtower is unavailable.' },
    };
}
function serviceBackedSprintEngineFrontDoors(resolve) {
    return {
        setRunnerMode: async (input) => resolve()?.setRunnerMode(input) ?? { ok: false, message: 'Sprint Engine is unavailable.' },
        readProjection: async (input) => resolve()?.readProjection(input) ?? { ok: false, message: 'Sprint Engine is unavailable.' },
        refreshPullRequestStatus: async (input) => resolve()?.refreshPullRequestStatus(input) ?? { ok: false, message: 'Sprint Engine is unavailable.' },
        mergePullRequest: async (input) => resolve()?.mergePullRequest(input) ?? { ok: false, message: 'Sprint Engine is unavailable.' },
    };
}
