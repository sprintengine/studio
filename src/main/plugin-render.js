...
export function renderPluginLaunch(manifest, context) {
    return renderArgvSpec(manifest, context, manifest.launch.argv, manifest.launch);
}
export function renderPluginResume(manifest, context) {
    if (!manifest.resume || !manifest.resume.supported)
        return null;
...
export function renderReasoningArgs(manifest, reasoning, scope) {
    ...
    const variables = scope ?? new Map([['binary', manifest.binary]]);
    variables.set('reasoning', level);
    return selection.args.map((template) => substituteString(template, variables));
}
function renderArgvSpec(manifest, context, argvSpec, launchSpec) {
...
const PERMISSION_PRESET_LADDER = ['default', 'auto_workspace', 'bypass_all'];
function resolvePermissionPreset(manifest, requested) {
    const name = requested ?? 'default';
    const declared = manifest.permissionPresets[name];
    if (declared)
        return declared;
    const rung = PERMISSION_PRESET_LADDER.indexOf(name);
...
function buildVariableScope(manifest, context) {
    const scope = new Map();
    ...
    if (context.sessionId !== undefined)
        scope.set('sessionId', context.sessionId);
    if (context.prompt !== undefined)
        scope.set('prompt', context.prompt);
    if (context.cwd !== undefined)
        scope.set('cwd', context.cwd);
    if (context.workspaceRoot !== undefined)
        scope.set('workspaceRoot', context.workspaceRoot);
    if (context.files !== undefined)
        scope.set('files', context.files);
    ...
    if (model && modelArgTemplates && modelArgTemplates.length > 0) {
        ...
        scope.set('modelArgs', modelArgTemplates.map((template) => substituteString(template, scope)));
    }
    else {
        scope.set('modelArgs', []);
    }
    ...
    if (colorScheme && themeName && themeArgTemplates && themeArgTemplates.length > 0) {
        ...
        scope.set('themeArgs', themeArgTemplates.map((template) => substituteString(template, scope)));
    }
    else {
        scope.set('themeArgs', []);
    }
...
function expandArgv(argvSpec, variables) {
    const out = [];
    for (const token of argvSpec) {
        ...
        if ('valueIf' in token) {
            ...
            const truthy = (Array.isArray(condition) && condition.length > 0) ||
                (typeof condition === 'string' && condition.length > 0);
            if (truthy)
                out.push(substituteString(token.value, variables));
            continue;
        }
    }
    return out;
...
function substituteString(template, variables) {
    return template.replace(SUBSTITUTION_PATTERN, (_match, name) => {
...
function renderEnv(envSpec, variables) {
    const env = {};
    if (!envSpec)
        return env;
