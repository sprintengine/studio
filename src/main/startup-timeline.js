...
import { createStartupTimeline, formatStartupTimeline, startupTimelineEnabledFor, STARTUP_MARK_CHANNEL, } from '../shared/startup-timeline';
...
export function startupTimelineEnabled() {
    return enabled;
}
...
export function attachStartupTimeline(ipcMain) {
    if (!timeline)
        return;
    ipcMain.on(STARTUP_MARK_CHANNEL, (_event, id, atEpochMs) => {
        timeline?.record(String(id), typeof atEpochMs === 'number' ? atEpochMs : Number.NaN);
    });
}
function reportStartupTimeline(report) {
    timeline = null;
