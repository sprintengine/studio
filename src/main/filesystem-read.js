    ...
    return {
        ...
        async readTextFile(filePath) {
            ...
            if (targetStats.size > MAX_TEXT_FILE_READ_BYTES) {
                throw new Error(`File is too large to open in Multicode (${formatBytes(targetStats.size)}; limit ${formatBytes(MAX_TEXT_FILE_READ_BYTES)}).`);
            }
        ...
        async readImageDataUrl(filePath) {
            ...
            if (targetStats.size > MAX_IMAGE_DATA_URL_BYTES) {
                throw new Error(`Image is too large to preview in Multicode (${formatBytes(targetStats.size)}; limit ${formatBytes(MAX_IMAGE_DATA_URL_BYTES)}).`);
            }
            const content = await readFile(filePath);
        ...
        async openHtmlFileInBrowser(targetPath) {
...
export async function assertReportSymlinkContained(filePath) {
...
function reportsBoundary(filePath) {
...
export function looksLikeBinary(content) {
    if (content.length === 0)
        return false;
    ...
    for (const byte of sample) {
        if (byte === 0)
            return true;
        const isAllowedControl = byte === 0x09
            || byte === 0x0a
            || byte === 0x0d
            || byte === 0x1b;
