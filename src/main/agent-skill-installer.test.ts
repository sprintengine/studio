...
test('agent-skill-installer', async () => {
  ...
  function allInstalled(plugins: readonly PluginRegistryListEntry[]): AgentCliAvailabilityMap {
    ...
    for (const plugin of plugins) {
      map[plugin.id] = {
        cli: plugin.id,
        installed: true,
        resolvedPath: `/usr/local/bin/${plugin.id}`,
        version: '1.0.0',
      }
    }
    return map
  ...
  async function main(): Promise<void> {
    ...
    const installedEntry = await readFile(
      join(fullRoot, '.codex', 'skills', 'backlog', 'reference', 'notes.md'),
      'utf-8',
    )
    ...
    const claudeOnly = await installer({
      availability: {
        'claude-code': { cli: 'claude-code', installed: true, resolvedPath: '/bin/claude', version: '1' },
      },
    ...
    await writeSkillSource(
      join(strandedRoot, '.grok', 'skills', 'stranded'),
      '---\nname: stranded\n---\n\n# Stranded\n',
    )
