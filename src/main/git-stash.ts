import type { GitCommandResult, GitStashEntry, GitStashListSnapshot } from './git'
...
export async function listGitStashes(repoRoot: string): Promise<GitStashListSnapshot> {
  ...
  if (result.ok) {
    for (const line of result.stdout.split('\n')) {
      ...
      if (!selector || !hash || subject === undefined) continue
...
export async function dropGitStash(repoRoot: string, index: number, expectedHash: string): Promise<GitCommandResult> {
  const resolved = await resolveStashSelector(repoRoot, index, expectedHash)
  if ('error' in resolved) return resolved.error
