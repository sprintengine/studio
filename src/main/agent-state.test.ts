...
test('agent-state', async () => {
  ...
  async function run(): Promise<void> {
    ...
    assert.deepEqual(resolveAgentStateEvent(grokSpec, { event: 'pre_tool_used', phase: 'thinking' }), {
      action: 'drop',
    })
    ...
    assert.equal(
      changeOf({ path: '/' + 'x'.repeat(MAX_FILE_CHANGE_PATH_LENGTH), additions: 1, deletions: 0 }),
      undefined,
    )
    assert.deepEqual(
    ...
    assert.equal(
      changeOf({ path: '/repo/\u0000evil.ts', additions: 1, deletions: 0 }),
      undefined,
      'a NUL is not a path',
    )
    assert.equal(
    ...
    assert.equal(
      statusOf({ model: 'x'.repeat(MAX_STATUS_LINE_NAME_LENGTH) })?.model?.length,
      MAX_STATUS_LINE_NAME_LENGTH,
    )
    ...
    const prOf = (pullRequest: unknown) =>
      parseAgentStateFrame({ type: 'agent_state', agentId: 'a1', event: 'PostToolUse', ts: 5, pullRequest }, 999)
        ?.pullRequest
    assert.deepEqual(prOf({ url: 'https://github.com/acme/app/pull/12' }), {
    ...
    assert.equal(
      prOf({ url: 'https://github.com/acme/app/commit/9f2c1ab' }),
      undefined,
      'a commit is not a pull request',
    )
    assert.equal(
    ...
    assert.equal(
      prOf({ url: 'https://github.com/acme/app/pull/12\u001b[31m' }),
      undefined,
      'an ANSI escape is not a URL',
    )
    ...
    assert.equal(
      idOf('x'.repeat(MAX_TOOL_USE_ID_LENGTH)),
      'x'.repeat(MAX_TOOL_USE_ID_LENGTH),
    ...
    assert.deepEqual(evaluateAgentStall({ ...stallBase, phase: 'starting', source: 'lifecycle' }), {
      action: 'stalled',
    })
    assert.deepEqual(
      evaluateAgentStall({
    ...
    const kimiMergedOnce = mergeTomlArrayAgentStateHooks(
      kimiUserToml,
    ...
    }
    {
      ...
      await writeFile(
        world.settingsPath,
        JSON.stringify(
          { ...installed, statusLine: { type: 'command', command: installed.statusLine.command } },
          null,
          2,
        ),
        'utf8',
