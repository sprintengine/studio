...
const forceCloseWindowIds = new WeakSet();
...
export function applyWindowMaterialToWorkspaceWindows(material) {
...
export function markAppQuitInProgressForWindowClose() {
    appQuitInProgress = true;
}
export function confirmWorkspaceWindowClose(win) {
    if (win.isDestroyed())
        return;
    forceCloseWindowIds.add(win);
    win.close();
}
export function createMainWindow({ diagnosticsEnabled, windowId = 'primary', bounds = null, isMaximized = false, deferShow = false, }) {
    ...
    win.on('close', (event) => {
        if (windowId === 'primary' || appQuitInProgress || forceCloseWindowIds.has(win))
            return;
    ...
    const grantMedia = (permission) => permission === 'media' || permission === 'audioCapture';
...
export function createDiagnosticsWindow() {
    if (diagnosticsWindow && !diagnosticsWindow.isDestroyed()) {
        if (diagnosticsWindow.isMinimized())
            diagnosticsWindow.restore();
...
export function openAuxWindow({ kind, singletonKey, params, bounds = null, }) {
    ...
    win.on('closed', () => {
        if (auxWindows.get(registryKey) === win)
            auxWindows.delete(registryKey);
    });
    const schedulePlacementUpdate = createPlacementUpdateScheduler(win);
    win.on('move', schedulePlacementUpdate);
    win.on('resize', schedulePlacementUpdate);
...
function createPlacementUpdateScheduler(win) {
    let timer = null;
    return () => {
        if (timer)
            clearTimeout(timer);
