...
const READY_PHASES = new Set(['idle', 'awaiting_input']);
/** Phases that mean the session took the turn: it is no longer sitting idle. */
const WORKING_PHASES = new Set(['thinking', 'tool_use', 'starting']);
const CTRL_C = '\x03';
export class AgentControlPlane {
    ...
    async interrupt(target) {
        ...
        if (session.transport === 'conversation') {
            ...
            if (!conversation)
                return this.conversationUnavailable(session.sessionId);
            return this.enqueue(session.sessionId, async () => {
    ...
    // ── Internals ─────────────────────────────────────────────────────────────
    async sendTerminalPrompt(sessionId, text, submit, options) {
        ...
        if (!pasted.ok)
            return pasted;
        if (!submit)
            return { ok: true, sessionId, transport: 'terminal', submitted: false };
        ...
        if (!submitted.ok)
            return submitted;
        if (!options.confirm)
            return { ok: true, sessionId, transport: 'terminal', submitted: true };
        const confirmed = await this.awaitTurnAccepted(sessionId, before, options.confirmTimeoutMs ?? DEFAULT_CONFIRM_TIMEOUT_MS);
        return { ok: true, sessionId, transport: 'terminal', submitted: true, confirmed };
    }
    async sendConversationTurn(session, text) {
        const conversation = this.deps.conversation;
        if (!conversation)
            return this.conversationUnavailable(session.sessionId);
     ...
     */
    async awaitReady(sessionId, timeoutMs) {
        const deadline = this.now() + timeoutMs;
        for (;;) {
     ...
     */
    async awaitTurnAccepted(sessionId, before, timeoutMs) {
        const deadline = this.now() + timeoutMs;
        ...
        for (;;) {
            ...
            if (!session)
                return false;
            if (!wasWorking && session.phase && WORKING_PHASES.has(session.phase))
                return true;
            if (session.lastOutputAt != null
                && (before.lastOutputAt == null || session.lastOutputAt > before.lastOutputAt)) {
                return true;
            }
            if (!session.alive || this.now() >= deadline)
                return false;
            await this.delay(DEFAULT_POLL_INTERVAL_MS);
        }
    }
    isUserTyping(session) {
        if (session.lastInputAt == null)
            return false;
        if (this.ownInputAt.get(session.sessionId) === session.lastInputAt)
            return false;
        return this.now() - session.lastInputAt < USER_TYPING_QUIET_MS;
    }
    isQuietFor(session, idleMs) {
        if (session.lastOutputAt == null)
            return true;
    ...
    writeTerminal(sessionId, data) {
        ...
        if (after?.lastInputAt != null)
            this.ownInputAt.set(sessionId, after.lastInputAt);
        return { ok: true };
    }
    findSession(sessionId) {
        return this.listSessions().find((session) => session.sessionId === sessionId);
    }
    conversationUnavailable(sessionId) {
        return {
            ok: false,
            sessionId,
            reason: 'unsupported',
    ...
    enqueue(sessionId, job) {
        ...
        void settled.then(() => {
            ...
            if (!this.findSession(sessionId))
                this.ownInputAt.delete(sessionId);
        });
        return run;
    }
}
export function createAgentControlPlane(deps) {
...
export function parseTarget(target) {
    if (typeof target !== 'string') {
        ...
        if ('cwd' in target)
            return target.cwd.trim() ? { kind: 'cwd', value: target.cwd.trim() } : null;
        if ('cli' in target)
            return target.cli.trim() ? { kind: 'cli', value: target.cli.trim() } : null;
        return null;
    }
...
function matchesTarget(session, target) {
    switch (target.kind) {
        ...
        case 'agentId':
            if (session.agentId !== target.value)
                return false;
        ...
        case 'agentName':
            ...
            return (session.agentName?.toLowerCase() === target.value.toLowerCase()
...
export function toTerminalSession(snapshot) {
    return {
...
export function toConversationSession(summary) {
    return {
...
function conversationPhase(status) {
    switch (status) {
        case 'starting':
            return 'starting';
        case 'ready':
