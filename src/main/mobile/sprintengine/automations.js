...
import { automationRecentRunsMax, automationRunTextMaxChars, automationsPerProjectMax, } from '../../../shared/mobile-control/protocol';
...
export async function readMobileAutomationSnapshots(workspaceRoot, generatedAt) {
    const store = new AutomationsStore(workspaceRoot);
    const definitions = await store.listDefinitions();
    if (!definitions.ok || definitions.values.length === 0)
        return [];
...
function compareDefinitionsNewestFirst(left, right) {
...
function renderCadence(definition, at) {
    if (definition.trigger.kind !== SCHEDULE_TRIGGER_KIND)
        return undefined;
...
function isoField(field, value) {
    if (!value || Number.isNaN(Date.parse(value)))
        return {};
    return { [field]: value };
}
function truncateRunText(value) {
