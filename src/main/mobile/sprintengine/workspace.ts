...
export async function resolveStateForSprintEngine(input: {
  ...
  const statePath = statePaths.length > 0
    ...
    : join(workspaceRoot, '.multi-code', 'sprintengine', sprintEngineId, 'state.yaml')
  ...
  if (!isAllowedWorkspace(allowedWorkspaceRoots, state.workspaceRoot)) {
    throw new MobileSprintEngineCommandError('path_not_allowed', 'Sprint Engine state path is outside the allowed workspace roots.', false)
  }

  try {
    const stateStats = await stat(state.statePath)
    if (!stateStats.isFile()) {
      throw new MobileSprintEngineCommandError('sprintengine_not_found', 'Sprint Engine state path is not a file.', false)
    }
  } catch (error) {
    if (error instanceof MobileSprintEngineCommandError) throw error
    throw new MobileSprintEngineCommandError('sprintengine_not_found', 'Requested Sprint Engine state was not found.', false)
  }
