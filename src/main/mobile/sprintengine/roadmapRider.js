...
import { buildRoadmapBoardModel, } from '../../../shared/sprintengine/roadmap-surface';
...
export async function readMobileRoadmapRiders(workspaceRoot) {
    ...
    for (const item of listed.items) {
        if (!item.isRoadmap)
            continue;
        if (!ACTIVE_ROADMAP_STATUSES.has((item.status ?? 'idea')))
            continue;
        let content;
        try {
        ...
        const laneRuntime = new Map();
        for (const [lane, value] of runtime)
            laneRuntime.set(lane, toLaneStateView(value));
...
function toLaneStateView(runtime) {
    return {
        lane: runtime.lane,
