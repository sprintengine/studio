...
import { idempotencyKeyForCommand, rememberedCommandResult, rememberCommandResult, requestHashFor, } from './command-cache';
import { validateMobileControlCommand } from './command-validation';
import { mobileActorId, safeSlug, uniqueResolved, } from './path-utils';
import { createSprintEngineToolExecutor, defaultSprintEngineToolPath, parseToolJson, redactToolArgs, } from './tool-runner';
import { getMobileSprintEngineCommandErrorMessage, MobileSprintEngineCommandError, } from './command-error';
import { validateSprintEngineStatePath, } from './state-path';
import { normalizeFollowUpText } from './follow-up-text';
import { assertKnownActiveSprintEngineAgent, findReadySprintEngineTask, findSprintEngineArtifact, } from './state-reader';
import { MobileSprintEngineCommandResultRecorder } from './command-results';
import { assertExpectedSnapshotVersion, resolveStateForSprintEngine, validateMobileWorkspacePath, } from './workspace';
import { assertBacklogRelativePath, resolveBacklogStartContext } from './backlog';
import { attachSprintEnginePullRequestLink, recordSprintEngineExecutionLink, } from '../../sprintengine-backlog-links';
import { teamSlugFromStatePath } from '../../../shared/backlog/sprintengine-links';
import { workspaceRootFromStatePath } from './workspace-id';
import { createBacklogItem, updateBacklogModuleMetadata, updateBacklogStatus, updateBacklogTriage, updateBacklogType, } from '../../backlog-service';
...
function mobileAutomationError(code) {
    switch (code) {
...
async function isGitRepositoryRoot(workspaceRoot) {
    try {
...
async function discardBootstrappedRunStore(workspaceRoot, statePath) {
...
function statePathFromHandover(data) {
 ...
 */
export function pullRequestsFromVcsPr(data) {
    if (typeof data !== 'object' || data === null)
        return [];
    const repos = data.repos;
    if (!Array.isArray(repos)) {
        const url = trimmedString(data.pullRequestUrl);
    ...
    for (const entry of repos) {
        if (typeof entry !== 'object' || entry === null)
            continue;
        const repo = trimmedString(entry.repo);
        const url = trimmedString(entry.pullRequestUrl);
        if (!repo || !url)
            continue;
        pullRequests.push({
            repo,
            ...
            ...(repos.length > 1 ? { repoLabel: trimmedString(entry.project) ?? repo } : {}),
        });
    }
    return pullRequests;
}
export class MobileSprintEngineCommandService {
    workspaceRoot;
    configuredAllowedWorkspaceRoots;
    ...
    async executeCommand(command, scope) {
        switch (command.type) {
    ...
    async executeTaskStartCommand(command, scope) {
        ...
        return this.resultRecorder.acceptSessionCommand(command, data, state, 'Mobile task start was accepted by desktop session orchestration.');
    }
    async executeAgentFollowUpCommand(command, scope) {
        if (!this.sessionOrchestrator) {
        ...
        return this.resultRecorder.acceptSessionCommand(command, data, state, 'Mobile follow-up was accepted by desktop session orchestration.');
    }
    async executeArtifactReviewCommand(command, action, scope) {
        const state = await this.resolveStateForSprintEngine(command.payload.sprintEngineId, scope);
        await assertExpectedSnapshotVersion({
            expectedSnapshotVersion: command.expectedSnapshotVersion,
            statePath: state.statePath,
    ...
    async executeOpenPullRequestCommand(command, scope) {
        const state = await this.resolveStateForSprintEngine(command.payload.sprintEngineId, scope);
        await assertExpectedSnapshotVersion({
            expectedSnapshotVersion: command.expectedSnapshotVersion,
            statePath: state.statePath,
    ...
    async executeSetAutomationModeCommand(command, scope) {
        if (!this.sessionOrchestrator?.setAutomationMode) {
            return this.resultRecorder.reject(command, 'command_not_supported', 'Setting the automation mode is not supported by this desktop build.', false);
        }
        const state = await this.resolveStateForSprintEngine(command.payload.sprintEngineId, scope);
        await assertExpectedSnapshotVersion({
            expectedSnapshotVersion: command.expectedSnapshotVersion,
        ...
        return this.resultRecorder.acceptSessionCommand(command, data, state, 'Mobile automation-mode change was accepted by desktop session orchestration.');
    }
    ...
    async executeAutomationsControlCommand(command, scope) {
        if (!this.automationsController) {
            return this.resultRecorder.reject(command, 'command_not_supported', 'Controlling automations is not supported by this desktop build.', false);
        }
        ...
        return this.resultRecorder.acceptWorkspaceCommand(command, result.value, workspacePath, 'Mobile automation control was applied by the desktop automations engine.');
    }
    ...
    sprintEngineConfigArgs(config, fallbackTeamName) {
        ...
        for (const role of Object.keys(config?.roleCounts ?? {})) {
            ...
            if (roleId && !configuredRoles.includes(roleId))
                configuredRoles.push(roleId);
        }
    ...
    configuredRolesInitArgs(configuredRoles) {
    ...
    async executeSprintEngineCreateCommand(command, scope) {
        const workspacePath = await validateMobileWorkspacePath({
            workspacePath: command.payload.workspacePath,
            allowedWorkspaceRoots: scope.allowedWorkspaceRoots,
            workspaceRootCandidates: this.workspaceRootCandidates(scope),
        ...
        const { teamName, configuredRoles } = this.sprintEngineConfigArgs(command.payload.config, `mobile-${safeSlug(command.commandId)}`);
        const args = [
            'handover',
            '--name',
            teamName,
        ...
        if (statePath && roleArgs.length > 0) {
            const init = await this.invokeTool(command, ['--state', statePath, 'init', ...roleArgs], workspacePath, undefined, undefined, workspacePath);
            if (!init.ok) {
    ...
    async executeBacklogUpdateCommand(command, scope) {
        const workspacePath = await validateMobileWorkspacePath({
            workspacePath: command.payload.workspacePath,
            allowedWorkspaceRoots: scope.allowedWorkspaceRoots,
            workspaceRootCandidates: this.workspaceRootCandidates(scope),
        ...
        const mutations = [];
        if (status !== undefined) {
            mutations.push(() => updateBacklogStatus({ workspaceRoot: workspacePath, relativePath, status: status }));
        }
        if (type !== undefined) {
            mutations.push(() => updateBacklogType({ workspaceRoot: workspacePath, relativePath, type: type }));
        }
        if (difficulty !== undefined || criticality !== undefined) {
            mutations.push(() => updateBacklogTriage({
                ...
                ...(criticality !== undefined ? { criticality: criticality } : {}),
            }));
        }
        let store = null;
        ...
        return this.resultRecorder.acceptWorkspaceCommand(command, { item }, workspacePath, 'Mobile backlog update was applied to the workspace backlog store.');
    }
    async executeBacklogStartSprintEngineCommand(command, scope) {
        const workspacePath = await validateMobileWorkspacePath({
            workspacePath: command.payload.workspacePath,
            allowedWorkspaceRoots: scope.allowedWorkspaceRoots,
            workspaceRootCandidates: this.workspaceRootCandidates(scope),
        ...
        const { teamName, configuredRoles } = this.sprintEngineConfigArgs(command.payload.config, `backlog-${safeSlug(command.commandId)}`);
        ...
        if (statePath) {
            const init = await this.invokeTool(command, ['--state', statePath, 'init', '--use-worktrees', useWorktrees ? 'true' : 'false', ...this.configuredRolesInitArgs(configuredRoles)], workspacePath, undefined, undefined, workspacePath);
            if (!init.ok) {
    ...
    async executeBacklogCreateCommand(command, scope) {
        const workspacePath = await validateMobileWorkspacePath({
            workspacePath: command.payload.workspacePath,
            allowedWorkspaceRoots: scope.allowedWorkspaceRoots,
            workspaceRootCandidates: this.workspaceRootCandidates(scope),
        ...
        return this.resultRecorder.acceptWorkspaceCommand(command, { id: result.id, relativePath: result.relativePath, item }, workspacePath, 'Mobile backlog item was created in the workspace backlog store.');
    }
    commandScope(options) {
    ...
    async invokeTool(command, args, cwd, state, artifactId, workspacePath) {
    ...
    replayCachedResult(command, key, requestHash) {
        const remembered = rememberedCommandResult(key, requestHash);
        if (remembered.status === 'miss')
            return null;
