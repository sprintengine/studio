...
export class MobileSprintEngineActivityPublisher {
    ...
    publishSnapshotActivity(snapshot) {
        const previousBySprintEngine = new Map((this.previousSnapshot?.sprintEngines ?? []).map((sprintengine) => [sprintengine.sprintEngineId, sprintengine]));
        const events = snapshot.sprintEngines.flatMap((sprintengine) => this.eventsForSprintEngineTransition(previousBySprintEngine.get(sprintengine.sprintEngineId) ?? null, sprintengine));
        this.previousSnapshot = snapshot;
        return this.deliver(events);
    }
    publishCommandAudit(entry) {
    ...
    eventsForSprintEngineTransition(previous, next) {
    ...
    artifactReadyEvent(sprintengine, artifact) {
    ...
    taskNeedsInputEvent(sprintengine, task) {
        return this.notification({
    ...
    sprintEngineCompleteEvent(sprintengine) {
        return this.notification({
    ...
    deliver(events) {
        ...
        const deliveries = events.flatMap((event) => targets.map((target) => ({
...
function isComplete(sprintengine) {
