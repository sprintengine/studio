...
export async function validateMobileWorkspacePath(input) {
    ...
    if (isWorkspaceIdToken(requestedPath)) {
        const resolvedRoot = resolveWorkspaceIdToRoot(requestedPath, input.workspaceRootCandidates ?? input.allowedWorkspaceRoots);
        if (!resolvedRoot) {
...
function isAllowedWorkspace(allowedWorkspaceRoots, targetPath) {
