...
const CAPABILITY_BY_RELAY_SCOPE = {
...
export function relayDeviceCapabilities(device) {
    const capabilities = new Set();
    for (const capability of device.capabilities ?? []) {
        if (isMobileControlCapability(capability))
...
export function normalizeDevicePlatform(platform) {
    if (platform === 'ios' || platform === 'android' || platform === 'web')
        return platform;
    return 'web';
}
export function upsertRelayDevice(pairedDevices, device, protocolVersion) {
