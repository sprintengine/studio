...
import { sanitizeMobileSnapshotForRelay } from '../sprintengine/snapshot';
import { filterToDefaultSnapshotStatePaths } from '../../mobile-sprintengine-discovery';
import { isWorkspaceIdToken, resolveWorkspaceIdToRoot, workspaceRootFromStatePath, } from '../sprintengine/workspace-id';
import { mobileSnapshotCollections } from '../../../shared/mobile-control/protocol';
import { acceptedBridgeCommand, relayResultSummaryMaxBytes, relaySummaryByteLength, summarizeCommandResult, } from './command-results';
...
export async function dispatchSnapshotRequest(input) {
    ...
    const [allStatePaths, allWorkspaceRoots] = await Promise.all([
        statePathsProvider(),
        workspaceRootsProvider ? workspaceRootsProvider() : Promise.resolve([]),
    ]);
...
function resolveScopedRoot(requested, candidateRoots) {
...
function readIncludeCollections(value) {
    if (!Array.isArray(value)) {
        return undefined;
    }
    const collections = value.filter((entry) => mobileSnapshotCollections.includes(entry));
...
function relaySizedSnapshot(command, snapshot, scopedRequest) {
...
function withoutRoleCatalogs(snapshot) {
...
function withoutAutomationRecentRuns(snapshot) {
...
function snapshotFitsRelayResult(command, snapshot) {
...
function limitSprintEngines(snapshot, includedCount) {
    ...
    return {
        ...snapshot,
        sprintEngines,
        workspaces: snapshot.workspaces?.filter((workspace) => workspace.kind !== 'sprintengine' || includedIds.has(workspace.workspaceId)),
        snapshotLimits: {
