...
run('turns accumulate in order and the next turn follows the highest', () => {
  const store = index()
  store.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 0, ref: 'r0', at: 1 })
  assert.equal(store.hasBaseline('w1'), true)
  assert.equal(store.nextTurn('w1'), 1)
  store.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 1, ref: 'r1', at: 2 })
...
run('recording a turn twice corrects it rather than duplicating it', () => {
  const store = index()
  store.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 0, ref: 'old', at: 1 })
  store.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 0, ref: 'new', at: 5 })
  const turns = store.timelineFor('w1')?.turns ?? []
  assert.equal(turns.length, 1, 'one entry per turn, always')
  assert.equal(turns[0].ref, 'new')
  assert.equal(store.nextTurn('w1'), 1)
})
...
run('the cap evicts the oldest turns but NEVER the baseline', () => {
  ...
  for (let turn = 0; turn <= MAX_TURNS_PER_WORKSPACE; turn += 1) {
    ...
    if (turn < MAX_TURNS_PER_WORKSPACE) {
      ...
      assert.deepEqual(result.evicted, ['r1'], 'the oldest non-baseline turn is evicted')
    }
  }
  const turns = store.timelineFor('w1')?.turns ?? []
...
run('forget returns what the caller must delete, and only that workspace', () => {
  const store = index()
  store.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 0, ref: 'a', at: 1 })
  store.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 1, ref: 'b', at: 2 })
  store.recordTurn({ workspaceId: 'w2', cwd: '/repo', turn: 0, ref: 'c', at: 3 })

  const forgotten = store.forget('w1')
  assert.deepEqual(forgotten, { cwd: '/repo', refs: ['a', 'b'] })
  assert.equal(store.timelineFor('w1'), null)
...
run('a moved worktree keeps its timeline under the new cwd', () => {
  const store = index()
  store.recordTurn({ workspaceId: 'w1', cwd: '/old', turn: 0, ref: 'a', at: 1 })
  store.recordTurn({ workspaceId: 'w1', cwd: '/new', turn: 1, ref: 'b', at: 2 })
...
run('the timeline survives a restart', () => {
  const first = index()
  first.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 0, ref: 'a', at: 1 })
  first.recordTurn({ workspaceId: 'w1', cwd: '/repo', turn: 1, ref: 'b', at: 2 })
  ...
  assert.deepEqual(second.timelineFor('w1')?.turns.map((turn) => turn.ref), ['a', 'b'])
})
...
run('a malformed or absent file reads as no timelines, never a throw', () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      good: { cwd: '/repo', turns: [{ turn: 0, ref: 'a', at: 1 }] },
      noCwd: { turns: [{ turn: 0, ref: 'a', at: 1 }] },
      noTurns: { cwd: '/repo' },
      emptyTurns: { cwd: '/repo', turns: [] },
      badTurn: { cwd: '/repo', turns: [{ turn: 'zero', ref: 'a', at: 1 }] },
      emptyRef: { cwd: '/repo', turns: [{ turn: 0, ref: '', at: 1 }] },
      notAnObject: 'nope',
    }),
    'utf8'
  )
