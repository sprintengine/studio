...
import type { McpConfigReader, ReadServersResult } from './reader'
...
export function createMcpServerResolver(
  ...
  return {
    async resolve({ workspaceRoot, targets }) {
      ...
      const reads = new Map<string, Promise<ReadServersResult>>()
      const readOnce = (reader: McpConfigReader, path: string): Promise<ReadServersResult> => {
...
async function resolveTarget(
  ...
  context: {
    ...
    readers: ReadonlyMap<PluginMcpConfigFormat, McpConfigReader>
    homeDir: () => string
    readOnce: (reader: McpConfigReader, path: string) => Promise<ReadServersResult>
  },
  ...
  const byId = new Map<string, AgentMcpServer>()
