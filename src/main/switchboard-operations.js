...
function mutationResult(result, fallbackMessage) {
    if (!result.ok)
...
function claimResult(result) {
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to claim Switchboard task.' };
    ...
    return mutationResult(result, 'Unable to claim Switchboard task.');
}
export async function initializeSwitchboard(input) {
    const result = await runSwitchboardCore(['init', ...workspaceArgs(input.workspaceRoot)]);
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to initialize Switchboard.' };
    return result.payload;
}
export async function readAllSwitchboardTasks(input) {
    const result = await runSwitchboardCore(['read-all', ...workspaceArgs(input.workspaceRoot)]);
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to read Switchboard tasks.' };
    return result.payload;
}
export async function createSwitchboardTask(input) {
...
export async function importSwitchboardItem(workspaceRoot, item) {
    const result = await runSwitchboardCore([
        'import-task',
...
export async function updateSwitchboardTask(input) {
    const result = await runSwitchboardCore([
        'update',
        ...workspaceArgs(input.workspaceRoot),
        input.id,
...
export async function moveSwitchboardTask(input) {
...
export async function promoteSwitchboardInboxTask(input) {
...
export async function cancelSwitchboardTask(input) {
...
export async function addSwitchboardComment(input) {
    const args = ['comment', ...workspaceArgs(input.workspaceRoot), input.id, '--body', input.body];
    if (input.author?.name)
        args.push('--author', input.author.name);
    if (input.author?.type)
        args.push('--author-type', input.author.type);
    if (input.author?.id)
        args.push('--author-id', input.author.id);
    if (input.kind)
        args.push('--kind', input.kind);
    if (input.confidencePct != null)
        args.push('--confidence-pct', String(input.confidencePct));
    const result = await runSwitchboardCore(args);
    return mutationResult(result, 'Unable to add Switchboard comment.');
}
export async function claimSwitchboardTask(input) {
    const result = await runSwitchboardCore([
        'claim',
        ...workspaceArgs(input.workspaceRoot),
        '--from',
...
export async function publishSwitchboardTask(input) {
    ...
    for (const artifact of input.artifacts ?? []) {
        if (artifact.trim())
            args.push('--artifact', artifact.trim());
    }
    ...
    for (const touchedFile of input.touchedFiles ?? []) {
        if (touchedFile.trim())
            args.push('--touched-file', touchedFile.trim());
    }
...
export async function recoverSwitchboardLock(input) {
    const result = await runSwitchboardCore([
        'recover-lock',
        ...workspaceArgs(input.workspaceRoot),
        '--status',
    ...
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to recover Switchboard lock.' };
    return result.payload;
}
export async function requeueSwitchboardTask(input) {
...
export async function getSwitchboardExecutionStatus(input) {
...
export async function getSwitchboardExecutionLogs(input) {
    if (!input.workspaceRoot?.trim())
        return { ok: false, message: 'workspaceRoot is required.' };
    if (!input.executionId?.trim())
        return { ok: false, message: 'executionId is required.' };
    ...
    const tail = Number.isFinite(input.tail) ? Math.max(1, Math.min(5000, Math.floor(input.tail))) : 200;
    const fallback = await runSwitchboardCore([
        'execution',
        'logs',
        ...workspaceArgs(input.workspaceRoot),
...
export async function startWatchtowerReview(input) {
    const args = [
        'watchtower',
        'start-review',
        ...workspaceArgs(input.workspaceRoot),
    ...
    if (input.workspaceId)
        args.push('--workspace-id', input.workspaceId);
    const result = await runSwitchboardCore(args);
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to start Watchtower review.' };
    ...
    if (payload.ok) {
        ...
        for (const descriptor of payload.descriptors ?? []) {
            ...
            if (spawned && !spawned.ok) {
                await stopSpawnedWatchtowerSessions(input.workspaceRoot, descriptors, spawned.message || 'Watchtower review startup failed.');
...
export async function startWatchtowerTriage(input) {
    const args = [
        'watchtower',
        'start-triage',
        ...workspaceArgs(input.workspaceRoot),
    ...
    if (input.taskId?.trim())
        args.push('--task-id', input.taskId.trim());
    const result = await runSwitchboardCore(args);
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to start Watchtower triage.' };
    const payload = result.payload;
    if (payload.ok) {
        const descriptors = payload.descriptors ?? [];
        for (const descriptor of payload.descriptors ?? []) {
            ...
            if (spawned && !spawned.ok) {
                await stopSpawnedWatchtowerSessions(input.workspaceRoot, descriptors, spawned.message || 'Watchtower triage startup failed.');
...
export async function getWatchtowerRun(input) {
    const result = await runSwitchboardCore(['watchtower', 'run-status', ...workspaceArgs(input.workspaceRoot), input.runId]);
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to read Watchtower run.' };
    return result.payload;
}
export async function listWatchtowerRuns(workspaceRoot) {
    const result = await runSwitchboardCore(['watchtower', 'run-list', ...workspaceArgs(workspaceRoot)]);
    if (!result.ok)
        return { ok: false, message: result.message || 'Unable to list Watchtower runs.' };
    return result.payload;
}
