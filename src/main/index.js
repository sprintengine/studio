...
import { isFirstPartyAutomationProviderModule } from './automations/provider-registry';
import { isLoadEligible } from './modules/module-signature';
...
// dismissing it.
attachBuildSkewWatch(ipcMain, createBuildSkewWatch({
...
const thirdPartyMainLoad = planThirdPartyMainModules(discoverUserModulesSync(defaultUserModuleRoot(), readModuleTrustContext()));
...
const enabledMainModuleIds = new Set();
const activeMainModules = activeForChannel(createBundledMainModules({
    automations: {
        checkProviderPermission: checkAutomationProviderPermission,
        isRoadmapReconcileEnabled: () => enabledMainModuleIds.has('roadmap'),
    },
}), (module) => module.manifest.id, includeDevModules);
...
const moduleManifestsById = new Map([AGENT_RUNTIME_MANIFEST, ...activeMainModules.map((module) => module.manifest), ...thirdPartyMainLoad.modules.map((module) => module.manifest)].map((manifest) => [manifest.id, manifest]));
const getModulePermissions = (moduleId) => moduleManifestsById.get(moduleId)?.permissions;
...
applyModuleEnablementLive = async (overrides) => {
    ...
    if (automationsError)
        return { ok: false, message: automationsError.message };
...
services.setAutomationsAppFrontDoorResolver(() => moduleLoad.kernel.hostFor('@host').getService(AutomationsAppFrontDoorToken) ?? null);
// Roadmap.* tools ← the same Automations module, which constructs the orchestrator.
services.setRoadmapAppFrontDoorResolver(() => moduleLoad.kernel.hostFor('@host').getService(RoadmapAppFrontDoorToken) ?? null);
...
recordThirdPartyMainLaunchReport(thirdPartyMainLoad.modules.map((module) => module.manifest.id), moduleLoad.report);
...
if (MULTICODE_DIAGNOSTICS) {
    ...
    console.info('[modules] loaded:', moduleLoad.report.loaded, 'manifest-only:', moduleLoad.report.manifestOnly, 'disabled:', moduleLoad.report.disabled, 'sidecars:', moduleLoad.report.sidecars.map((s) => s.id));
...
function readModuleEnablementOverrides() {
    try {
...
function readModuleTrustContext() {
    return {
...
function checkAutomationProviderPermission(registration) {
    if (isFirstPartyAutomationProviderModule(registration.moduleId))
        return { ok: true };
...
registerAppLifecycle({
    ...
    allowMultipleInstances: !app.isPackaged &&
