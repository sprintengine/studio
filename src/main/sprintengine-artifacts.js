...
function validateSprintEngineStatePath(input) {
...
function resolveArtifactFilePath(state, artifactPathInput) {
...
function resolveSprintEngineArtifactId(input) {
    if (typeof input !== 'string' || !input.trim()) {
...
function resolveSprintEngineTaskId(input) {
    if (typeof input !== 'string' || !input.trim()) {
...
function resolveSprintEngineRosterRole(input) {
    if (typeof input !== 'string' || !input.trim()) {
...
function resolveTaskRole(input) {
    if (typeof input !== 'string' || !validTaskRoles.has(input)) {
        throw new Error('Task role is invalid.');
    }
    return input;
}
function resolveOptionalTaskRole(input) {
    return input === undefined || input === null ? undefined : resolveTaskRole(input);
}
function resolveOptionalString(input, field) {
...
function resolveRequiredString(input, field) {
    const value = resolveOptionalString(input, field);
    if (!value)
        throw new Error(`${field} is required.`);
    return value;
}
function resolveStringList(input, field) {
    if (input === undefined || input === null)
        return undefined;
...
function resolveRecord(input, field) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) {
        throw new Error(`${field} must be an object.`);
    }
    return input;
}
function resolveArray(input, field) {
    if (input === undefined || input === null)
        return [];
    if (!Array.isArray(input))
        throw new Error(`${field} must be a list.`);
...
function eventMetadataFromUnknown(input) {
    ...
    if (!id && !type && !timestamp && !actor && !message)
        return null;
    return { id, type, timestamp, actor, message };
}
function collectSprintEngineEvents(toolResult) {
    ...
    if (Array.isArray(record.notifications)) {
        for (const event of record.notifications)
            add(event);
    }
    return events;
}
async function readLatestSprintEngineEvent(state) {
    try {
...
async function buildSprintEngineMutationData(state, data) {
 ...
 * than one document.
 */
function parseSprintEngineCliJsonOutput(stdout) {
...
function resolveInitialSprintEngineStatePayload(payload) {
...
function resolveInitIntake(input) {
...
function resolveDefaultPhases(input) {
    if (!Array.isArray(input))
        return null;
    return input.filter((phase) => typeof phase === 'string' && phase.trim().length > 0);
}
...
function resolveInitRepos(input) {
    if (!Array.isArray(input))
        return [];
    const repos = [];
...
function resolveInitSourceItem(input) {
    ...
    if (!kind || !origin || !path)
        return null;
    const resolved = { kind, origin, path };
    ...
    if (planKind)
        resolved.planKind = planKind;
    if (originalPath)
        resolved.originalPath = originalPath;
    if (capturedAt)
        resolved.capturedAt = capturedAt;
    return resolved;
}
function resolveInitSource(input) {
    return resolveInitSourceItem(input);
}
function resolveInitSourceBundle(input) {
    if (!Array.isArray(input))
        return [];
    return input
        .map((item) => {
...
// mistyped role.
function resolveEnabledRoles(input) {
    if (!Array.isArray(input))
        return null;
    const seen = new Set();
    const out = [];
...
function resolveRoleRuntimes(input) {
    if (!input || typeof input !== 'object')
        return {};
    const out = {};
    for (const [role, entry] of Object.entries(input)) {
        const roleKey = role.trim();
        if (!roleKey || !entry || typeof entry !== 'object')
            continue;
...
function getSprintEngineMcpRuntimeRoot() {
    return findSprintEngineRuntimeRoot() ?? process.cwd();
}
function getSprintEngineMcpPythonExecutable(runtimeRoot) {
...
function sprintEngineInitArgs(state, payload) {
    ...
    for (const [agentId, agent] of Object.entries(payload.agents)) {
        ...
        if (roleId)
            args.push('--agent', `${roleId}:${agentId}`);
        else if (declaresRoleSet)
...
function runSprintEngineCli(state, args) {
 ...
 */
export function sprintEngineDeclaredSiblingRepoRoots(statePath) {
    let state;
    try {
    ...
    try {
        ...
        repos = projection?.run?.vcs?.repos;
    ...
    for (const entry of repos) {
        if (!entry || typeof entry !== 'object')
            continue;
        const { id, root } = entry;
        if (typeof id !== 'string' || typeof root !== 'string' || !root.trim())
            continue;
        if (id === 'primary' || root.trim() === '.')
            continue;
 ...
 */
export function sprintEngineSessionBindingForLaunchCwd(statePath, launchCwd) {
    ...
    if (Array.isArray(tasks)) {
        for (const entry of tasks) {
            if (!entry || typeof entry !== 'object')
                continue;
            const { id, repo, worktreePath } = entry;
            if (typeof id !== 'string' || !id.trim() || !matchesCwd(worktreePath))
                continue;
            return {
    ...
    if (Array.isArray(repos)) {
        for (const entry of repos) {
            if (!entry || typeof entry !== 'object')
                continue;
            const { id, worktreePath } = entry;
            if (typeof id !== 'string' || !matchesCwd(worktreePath))
                continue;
...
export function sprintEngineRepoIdForLaunchCwd(statePath, launchCwd) {
    return sprintEngineSessionBindingForLaunchCwd(statePath, launchCwd).repo;
}
function runSprintEngineMcpToolProcess(context, tool, payload, actor) {
    return new Promise((resolvePromise) => {
...
function sprintEngineRegistryRootsForRead() {
    try {
        ...
        if (existsSync(userRoot))
            roots.push({ id: 'user-roles', root: userRoot });
        return roots;
    }
    catch {
        return [];
...
async function runReadOnlyMcpTool(runner, context, tool, payload) {
...
async function readSprintEngineRegistryRolesWith(runMcpTool, payload, timeoutMs) {
    try {
        ...
        return await runReadOnlyMcpTool(runMcpTool, {
            workspaceRoot,
            allowedRoots: pluginRegistryRoots.map((root) => root.root),
            ...(timeoutMs !== undefined ? { timeoutMs } : {}),
        }, 'sprintengine.roles.list', { workspaceRoot, includeShadowed: payload?.includeShadowed === true, pluginRegistryRoots });
    }
    catch (error) {
 ...
 */
export function readSprintEngineRegistryRoles(payload, timeoutMs) {
    return readSprintEngineRegistryRolesWith(runSprintEngineMcpToolProcess, payload, timeoutMs);
}
async function requireSprintEngineMcpAuthority(deps) {
...
function parseSprintEngineProjectionForArtifactReview(content) {
    ...
    const artifacts = (Array.isArray(parsed.artifacts) ? parsed.artifacts : []).flatMap((artifact) => {
...
function normalizeSprintEngineArtifactFileKey(path) {
    const segments = path
        .trim()
        .replace(/\\/g, '/')
        .split('/')
...
function isSameSprintEngineArtifactFile(left, right) {
    const leftKey = normalizeSprintEngineArtifactFileKey(left);
    return leftKey !== '' && leftKey === normalizeSprintEngineArtifactFileKey(right);
}
export function getArtifactAutoApprovalBlocker(artifact, tasks, artifacts) {
    if (artifact.status === 'approved')
        return 'Artifact is already approved.';
    if (artifact.status === 'superseded')
        return 'Superseded artifacts are obsolete and cannot be auto-approved.';
    ...
    if (!artifact.path.trim())
        return 'Artifact file path is missing.';
    if (!autoApprovableArtifactKinds.has(artifact.kind))
        return 'Artifact kind is not eligible for auto-approval.';
    const task = tasks.find((candidate) => candidate.id === artifact.taskId);
    if (!task)
        return 'Artifact task was not found.';
    const blockingArtifacts = artifacts.filter((candidate) => candidate.taskId === task.id
    ...
    if (blockingArtifacts.length === 0)
        return 'No blocking review artifact is waiting for approval.';
    const ineligibleBlockingArtifact = blockingArtifacts.find((candidate) => !['ready_for_review', 'changes_requested'].includes(candidate.status) || !candidate.path.trim());
    if (ineligibleBlockingArtifact) {
        return `Related artifact ${ineligibleBlockingArtifact.id} is not eligible for auto-approval.`;
    }
    return null;
}
async function readSprintEngineProjectionForArtifactReview(state) {
...
async function assertAutoApprovalAllowed(state, artifactId) {
    const { tasks, artifacts } = await readSprintEngineProjectionForArtifactReview(state);
    const artifact = artifacts.find((candidate) => candidate.id === artifactId);
    if (!artifact)
        throw new Error('Requested artifact was not found in the sprint projection.');
    const blocker = getArtifactAutoApprovalBlocker(artifact, tasks, artifacts);
    if (blocker)
        throw new Error(blocker);
    if (/^https?:\/\//i.test(artifact.path))
        throw new Error('Remote artifact links cannot be auto-approved.');
    const artifactPath = resolveArtifactFilePath(state, artifact.path);
    const artifactStats = await stat(artifactPath);
    if (!artifactStats.isFile())
        throw new Error('Artifact path must be a file.');
    await readFile(artifactPath, 'utf8');
    return artifact;
}
export function createSprintEngineArtifactHandlers(deps) {
    ...
    return {
        ...
        async reviewArtifact(payload, action, mode) {
            try {
                ...
                if (action === 'request-changes') {
                    ...
                    if (!feedback)
                        return { ok: false, message: 'Artifact change requests require feedback.' };
                }
                if (mode === 'auto-run') {
                    if (action !== 'approve')
                        throw new Error('Auto-run can only approve eligible artifacts.');
        ...
        async commentTask(payload) {
            try {
                ...
                const toolResult = await runMcpTool({ workspaceRoot: state.workspaceRoot }, 'sprintengine.task.comment', { statePath: state.statePath, taskId, id: actor.id, body, source: 'user' }, actor);
                if (toolResult.exitCode !== 0 || !toolResult.response?.ok) {
                    const message = toolResult.response && !toolResult.response.ok
                        ? toolResult.response.error?.message
                        : undefined;
        ...
        async resolveTaskInput(payload) {
            try {
                ...
                const toolResult = await runMcpTool({ workspaceRoot: state.workspaceRoot }, 'sprintengine.task.resolve_input', {
                    statePath: state.statePath,
                    taskId,
                    id: actor.id,
                    resolution,
        ...
        async setTaskStatus(payload) {
            try {
                ...
                const toolResult = await runMcpTool({ workspaceRoot: state.workspaceRoot }, 'sprintengine.task.status', 
                ...
                { statePath: state.statePath, taskId, id: actor.id, status, actorKind: 'human' }, actor);
                if (toolResult.exitCode !== 0 || !toolResult.response?.ok) {
                    const message = toolResult.response && !toolResult.response.ok
                        ? toolResult.response.error?.message
                        : undefined;
        ...
        async ensureTaskWorktree(payload) {
            try {
                ...
                if (!taskId)
                    return { ok: false, isolated: false, worktreePath: null, message: 'Task id is required.' };
                const toolResult = await runSprintEngineCli(state, [
                    '--state',
                    state.statePath,
                    'vcs',
        ...
        async mergePullRequest(payload) {
            try {
                ...
                const refusal = typeof parsed === 'object' && parsed !== null && parsed.ok === false
        ...
        async readProjection(payload) {
            try {
                ...
                try {
                    ...
                    if (code === 'ENOENT' && !existsSync(state.teamDirectory)) {
                        return {
                            ok: false,
                            permanent: true,
                            message: `Sprint run data is missing: "${state.teamDirectory}" no longer exists `
                ...
                if (rejection)
                    return { ok: false, message: rejection, permanent: true };
        ...
        async readRegistryRole(payload) {
            try {
                ...
                const pluginRegistryRoots = sprintEngineRegistryRootsForRead();
                return runReadOnlyMcpTool(runMcpTool, { workspaceRoot, allowedRoots: pluginRegistryRoots.map((root) => root.root) }, 'sprintengine.roles.get', { workspaceRoot, roleId, pluginRegistryRoots });
            }
            catch (error) {
                return { ok: false, message: error instanceof Error ? error.message : String(error) };
            }
        ...
        async summarizeFeedback(payload) {
            try {
                const state = validateSprintEngineStatePath(payload?.statePath);
                return runReadOnlyMcpTool(runMcpTool, { workspaceRoot: state.workspaceRoot }, 'sprintengine.feedback.summarize', { statePath: state.statePath });
            }
            catch (error) {
                return { ok: false, message: error instanceof Error ? error.message : String(error) };
            }
