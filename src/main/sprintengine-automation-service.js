...
import { SPRINT_ENGINE_AUTOMATION_NOTIFICATION_TITLE, sprintEngineAutomationModeLabel, } from '../shared/sprintengine/automation-types';
import { isSprintEngineAutomationMode, sprintEngineCliWatchPollingForAutomationMode, } from '../shared/sprintengine/automation-lifecycle';
import { SPRINT_ENGINE_AUTOMATION_INTENT_FILE, isSprintEngineCliPermissionPreset, nextSprintEngineAutomationIntentRecord, parseSprintEngineAutomationIntentRecord, serializeSprintEngineAutomationIntentRecord, } from '../shared/sprintengine/automation-intent';
function automationIntentPathForState(statePath) {
    const normalized = statePath?.trim();
    if (!normalized || basename(normalized) !== 'run.yaml')
        return null;
...
export function createSprintEngineAutomationService(deps) {
    ...
    const bridgedCliWatchPolling = new Map();
    ...
    async function readRecord(intentPath) {
        let raw;
        try {
    ...
    async function writeRecordAtomically(intentPath, record) {
    ...
    function auditManualTransition(input, previousMode) {
        ...
        if (input.mode !== 'manual' || previousMode === 'manual')
            return;
        if (input.suppressManualAudit)
            return;
    ...
    function reconcileCliWatchPollingBridge(input, queueKey) {
        const cliWatchPolling = sprintEngineCliWatchPollingForAutomationMode(input.mode);
        if (bridgedCliWatchPolling.get(queueKey) === cliWatchPolling)
            return;
    ...
    return {
        async readAutomationMode(input) {
            const paths = automationIntentPathForState(input.statePath);
            if (!paths)
                return { ok: false, message: 'Sprint run state path is not a readable run.yaml location.' };
        ...
        async setAutomationMode(input) {
            ...
            const paths = automationIntentPathForState(input.statePath);
            if (!paths)
                return { ok: false, message: 'Sprint run state path is not a writable run.yaml location.' };
            return enqueue(paths.queueKey, async () => {
         ...
         */
        async setCliPermissionPreset(input) {
