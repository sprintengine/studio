...
export function trackerProviderLabel(provider: TrackerProviderId): string {
...
export function proxyProvenanceMarker(provider: TrackerProviderId): string {
...
export function trackerIssueTargetKind(provider: TrackerProviderId): string {
  return `${provider}.issue`
}

// The flat underscore external-identity keys written through
// serializeBacklogFrontmatterFields. Dotted keys (`external.provider`) do NOT
// round-trip (frontmatter key charset excludes the dot) and would duplicate on
// update, so underscore only (plan §3.4).
...
export function proxyExternalFields(issue: NormalizedIssue, connection: TrackerConnection): ProxyExternalFields {
  return {
...
export function composeProxyBody(issue: NormalizedIssue): string {
...
export function proxyIssueLink(issue: NormalizedIssue): BacklogItemLinkPayload {
  return {
