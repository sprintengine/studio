...
export type JiraBodyConversion = {
  markdown: string
  // The verbatim original: the wiki string as-is, or the ADF tree JSON. Absent
  // only when the body was empty, so an empty description carries no rawBody.
  raw?: string
}
...
export function jiraBodyToMarkdown(body: unknown): JiraBodyConversion {
  if (body === null || body === undefined) return { markdown: '' }
...
export function jiraCommentBodyToMarkdown(body: unknown): string {
...
export function wikiMarkupToMarkdown(wiki: string): string {
  ...
  text = text.replace(/\{code(?::([^}]*))?\}\n?([\s\S]*?)\{code\}/g, (_m, params: string | undefined, inner: string) => {
    const lang = extractCodeLang(params)
    return protect(`\n\`\`\`${lang}\n${inner.replace(/\n$/, '')}\n\`\`\`\n`)
  })
  text = text.replace(/\{noformat\}\n?([\s\S]*?)\{noformat\}/g, (_m, inner: string) => {
    return protect(`\n\`\`\`\n${inner.replace(/\n$/, '')}\n\`\`\`\n`)
  })
  // Inline monospace {{...}} → `...`.
  text = text.replace(/\{\{(.+?)\}\}/g, (_m, inner: string) => protect(`\`${inner}\``))
  ...
  result = result.replace(PLACEHOLDER_RE, (_m, idx: string) => protectedSpans[Number(idx)] ?? '')

  return result.replace(/\n{3,}/g, '\n\n').trim()
}

function extractCodeLang(params: string | undefined): string {
  if (!params) return ''
...
function convertWikiLine(line: string): string {
  ...
  if (bq) return `> ${convertInline(bq[1])}`
...
function splitTableCells(line: string, delimiter: '|' | '||'): string[] {
  const trimmed = line.trim()
  const inner =
    delimiter === '||' ? trimmed.replace(/^\|\|/, '').replace(/\|\|$/, '') : trimmed.replace(/^\|/, '').replace(/\|$/, '')
...
function convertInline(text: string): string {
  ...
  result = result.replace(/\[([^\]|]+)\|([^\]]+)\]/g, (_m, label: string, target: string) => {
  ...
  result = result.replace(/\[([^\]|]+)\](?!\()/g, (_m, target: string) => {
...
function renderAdfNodes(nodes: AdfNode[]): string[] {
...
function renderAdfBlock(node: AdfNode): string {
  switch (node.type) {
    ...
    case 'rule':
      return '---'
    default:
      if (node.content && node.content.length > 0) return renderAdfNodes(node.content).join('\n\n')
      return node.text ?? ''
  }
}

function renderAdfList(items: AdfNode[], bullet: string): string {
  return items
    .map((item) => {
...
function renderAdfInline(nodes: AdfNode[]): string {
  return nodes
    .map((child) => {
      ...
      if (child.type === 'text') return applyAdfMarks(child.text ?? '', child.marks ?? [])
      if (child.content) return renderAdfInline(child.content)
      return child.text ?? ''
    })
    .join('')
}

function applyAdfMarks(text: string, marks: Array<{ type?: string; attrs?: Record<string, unknown> }>): string {
  let result = text
...
function collectAdfText(node: AdfNode): string {
