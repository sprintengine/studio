...
export async function jiraFetch(request, fetchImpl) {
    ...
    if (!response.ok)
        throw await toProviderError(response, connection);
...
function buildAuthHeader(connection, secret) {
    if (connection.authMode === 'jira_pat')
...
async function toProviderError(response, connection) {
    const message = await extractJiraMessage(response);
    const context = { provider: 'jira', connectionId: connection.id };
...
async function extractJiraMessage(response) {
    let raw;
    try {
    ...
    try {
        ...
        const fromMessages = Array.isArray(parsed.errorMessages)
            ? parsed.errorMessages.filter((m) => typeof m === 'string')
            : [];
        const fromErrors = parsed.errors && typeof parsed.errors === 'object'
            ? Object.values(parsed.errors).filter((m) => typeof m === 'string')
            : [];
