...
import { normalizeJiraIssue } from './jira-normalize';
...
export class JiraTrackerProvider {
    provider = 'jira';
    capabilities = { canComment: true, canTransition: true, selfHostable: true };
    connections;
    fetchImpl;
    constructor(deps) {
    ...
    async listAssignedToMe(args) {
        ...
        if (isCloudConnection(connection)) {
            ...
            for (let pageIndex = 0; pageIndex < MAX_ASSIGNED_PAGES; pageIndex += 1) {
                ...
                if (page.issues.length === 0 || !page.nextPageToken)
                    break;
    ...
    async listTransitions(args) {
        ...
        if (!Array.isArray(result.transitions))
            return [];
        return result.transitions.flatMap((entry) => {
            const t = entry;
    ...
    async testConnection(args) {
        try {
            const { connection, secret } = await this.resolve(args.connectionId);
            const me = (await jiraFetch({ connection, secret, path: `${JIRA_API_BASE}/myself` }, this.fetchImpl));
    ...
    async searchPageDataCenter(connection, secret, jql, startAt, maxResults) {
        const result = (await jiraFetch({
            connection,
            secret,
    ...
    async searchPageCloud(connection, secret, jql, pageToken, maxResults) {
        const result = (await jiraFetch({
            connection,
            secret,
        ...
        const issues = Array.isArray(result.issues) ? result.issues : [];
        const nextPageToken = result.isLast !== true && typeof result.nextPageToken === 'string' && result.nextPageToken
...
function isCloudConnection(connection) {
...
export function markdownToJiraWiki(markdown) {
    return markdown
        .split('\n')
