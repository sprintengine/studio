...
function createReconcileScheduler(
  engine: TrackerWriteBackEngine,
  logDiagnostic?: (event: string, payload: Record<string, unknown>) => void,
): { notify(statePath: string): void } {
  type Loop = { timer: ReturnType<typeof setTimeout> | null; running: boolean; rerun: boolean }
  const loops = new Map<string, Loop>()

  const run = async (statePath: string, workspaceRoot: string, loop: Loop): Promise<void> => {
    loop.running = true
    try {
      do {
        loop.rerun = false
...
export function workspaceRootFromStatePath(statePath: string): string | null {
