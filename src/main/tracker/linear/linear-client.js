...
export async function postLinearGraphQL(args) {
    ...
    try {
        ...
        throw new TrackerProviderError('network', aborted ? 'Linear did not respond in time.' : 'Could not reach Linear.', { provider: 'linear', connectionId });
    }
    finally {
        clearTimeout(timer);
    }
...
function statusError(response, body, connectionId) {
...
function graphQLError(errors, connectionId) {
...
function graphQLErrorKind(type) {
...
function defaultStatusMessage(status) {
    ...
    if (status === 429)
        return 'Linear rate limit reached.';
    if (status === 404)
        return 'The Linear issue could not be found.';
    if (status >= 500)
        return 'Linear is temporarily unavailable.';
