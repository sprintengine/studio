...
import { LINEAR_GRAPHQL_ENDPOINT, LINEAR_REQUEST_TIMEOUT_MS, postLinearGraphQL, } from './linear-client';
import { ASSIGNED_ISSUES_QUERY, FETCH_ISSUE_QUERY, LINEAR_ASSIGNED_MAX_PAGES, LINEAR_PAGE_SIZE, LIST_ISSUES_QUERY, SEARCH_ISSUES_QUERY, VIEWER_PROBE_QUERY, normalizeLinearIssue, } from './linear-queries';
...
export class LinearTrackerProvider {
    provider = 'linear';
    ...
    async searchIssues(args) {
        ...
        const connection = term
            ? await this.request(args.connectionId, SEARCH_ISSUES_QUERY, {
                term,
                first: LINEAR_PAGE_SIZE,
                after: args.cursor ?? null,
            }).then((data) => data.issueSearch)
            : await this.request(args.connectionId, LIST_ISSUES_QUERY, {
                first: LINEAR_PAGE_SIZE,
                after: args.cursor ?? null,
    ...
    async fetchIssue(args) {
        const data = await this.request(args.connectionId, FETCH_ISSUE_QUERY, { id: args.externalId });
        if (!data.issue) {
    ...
    async listAssignedToMe(args) {
        ...
        for (let page = 0; page < LINEAR_ASSIGNED_MAX_PAGES; page++) {
            const data = await this.request(args.connectionId, ASSIGNED_ISSUES_QUERY, { first: LINEAR_PAGE_SIZE, after: cursor });
            const connection = data.viewer?.assignedIssues;
            if (!connection)
                break;
            for (const node of connection.nodes)
                issues.push(normalizeLinearIssue(node, args.connectionId));
    ...
    async testConnection(args) {
        try {
            const data = await this.request(args.connectionId, VIEWER_PROBE_QUERY, {});
            if (!data.viewer) {
    ...
    async request(connectionId, query, variables) {
        const apiKey = await this.resolveApiKey(connectionId);
        return postLinearGraphQL({
    ...
    async resolveApiKey(connectionId) {
