import { TrackerConnectionStore } from './connection-store';
...
export class TrackerService {
    ...
    async testConnection(input) {
        try {
            if ('draft' in input) {
                const probe = await this.connectionStore.withDraftConnection(input.draft, (connectionId) => this.probe(input.draft.provider, connectionId));
    ...
    async probe(provider, connectionId) {
        const client = this.registry.get(provider);
        if (!client)
            return { ok: false, reason: 'No client available for this tracker yet.' };
        try {
    ...
    async withProvider(connectionId, run) {
        let provider;
        try {
            const connection = await this.connectionStore.getConnection(connectionId);
            if (!connection)
                return { ok: false, error: notConfigured('This tracker connection no longer exists.', { connectionId }) };
...
function notConfigured(message, context = {}) {
...
function defaultWriteBackCleanup() {
    return {
...
export function getSharedTrackerService(overrides) {
    if (overrides)
        return new TrackerService({ connectionStore: new TrackerConnectionStore(overrides) });
