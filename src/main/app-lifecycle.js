...
import { emptyBackgroundStatus } from '../shared/background-mode';
import { writeDiagnosticLog } from './diagnostics-service';
export function registerAppLifecycle({ diagnosticsEnabled, allowMultipleInstances = false, terminalRuntime, conversationRuntime, automationService, agentStateService, workspaceSyncService, sprintRuntime, moduleKernel, updateService, handleAuthCallback, backgroundMode, }) {
    ...
    app.whenReady().then(async () => {
        ...
        if (process.platform === 'win32') {
            app.setAppUserModelId(process.env['ELECTRON_RENDERER_URL'] ? process.execPath : 'com.multicode');
        }
    ...
    app.on('window-all-closed', () => {
        if (backgroundPresence.onWindowAllClosed() === 'quit')
            app.quit();
    });
...
function registerMulticodeProtocol() {
    if (!app.isPackaged) {
