...
export async function syncManagedSprintEngineMcpConfig(input, deps) {
    ...
    try {
        for (const syncInput of syncInputs) {
            ...
            if (!result.ok) {
                if (registeredRunId && registeredRunWasCreated)
                    await deps.sprintEngineMcpHub.unregisterRun(registeredRunId);
