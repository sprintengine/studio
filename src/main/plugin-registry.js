...
import { classifySignedManifestTrust } from './modules/module-signature';
import { validateManifestStructure } from './plugin-manifest-validate';
export function defaultUserPluginRoot() {
...
export function createPluginRegistry(options) {
    ...
    async function loadFromRoot(root, source) {
        const loaded = [];
        const loadedConversationProviders = [];
        const rejected = [];
        if (!existsSync(root))
    ...
    function loadFromRootSync(root, source) {
        const loaded = [];
        const loadedConversationProviders = [];
        const rejected = [];
        if (!existsSync(root))
    ...
    function mergeBundledAndUser(bundled, user) {
        plugins.clear();
        providers.clear();
        const report = {
            loaded: [],
        ...
        for (const plugin of user.loaded) {
            ...
            if (previous) {
                ...
                if (replacedIndex >= 0)
                    report.loaded.splice(replacedIndex, 1);
            }
            plugins.set(plugin.manifest.id, plugin);
            report.loaded.push(plugin);
        }
        ...
        for (const provider of user.loadedConversationProviders) {
            ...
            if (previous) {
                const replacedIndex = report.loadedConversationProviders.findIndex((p) => p.manifest.id === provider.manifest.id);
                if (replacedIndex >= 0)
                    report.loadedConversationProviders.splice(replacedIndex, 1);
            }
            providers.set(provider.manifest.id, provider);
            report.loadedConversationProviders.push(provider);
        }
...
async function classifyProviderAdapter(manifest, source, pluginRoot, options) {
    return classifyProviderAdapterWithEntryHash(manifest, source, options, await readExecutableAdapterEntryHash(manifest, pluginRoot));
}
function classifyProviderAdapterSync(manifest, source, pluginRoot, options) {
    return classifyProviderAdapterWithEntryHash(manifest, source, options, readExecutableAdapterEntryHashSync(manifest, pluginRoot));
}
function classifyProviderAdapterWithEntryHash(manifest, source, options, entryHash) {
    const adapter = manifest.adapter ?? { kind: 'declarative' };
...
async function readExecutableAdapterEntryHash(manifest, pluginRoot) {
...
function readExecutableAdapterEntryHashSync(manifest, pluginRoot) {
    if (manifest.adapter?.kind !== 'trusted-executable')
        return null;
    try {
...
function providerTrustError(status, productionMode) {
    if (status === 'invalid')
        return 'Executable provider adapter signature is invalid or the package was tampered with.';
    if (status === 'signed')
        return 'Executable provider adapter is signed but has not been trusted for execution.';
    if (status === 'unsigned')
...
export function manifestToListEntry(plugin) {
    ...
    // host, not hosted.
    const hostedVia = plugin.manifest.binary === 'claude' &&
...
export function providerManifestToListEntry(provider) {
    return {
