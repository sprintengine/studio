...
import { BACKLOG_DEPENDENCIES_PLANNED_KEY, backlogDependenciesPlannedFromFields, extractBacklogTitle, formatBacklogCsvList, isValidBacklogSlug, parseBacklogCsvList, parseBacklogFrontmatter, serializeBacklogFrontmatterFields, stripBacklogFrontmatter, } from '../shared/backlog/frontmatter';
import { deriveDefaultBacklogKey, formatBacklogNumericId, isValidBacklogKey, parseBacklogNumericId, planBacklogIdAllocation, } from '../shared/backlog/item-id';
...
const STORE_PATH = ['.multi-code', 'backlog', 'items.json'];
...
const CONFIG_PATH = ['.multi-code', 'backlog', 'config.json'];
...
const backlogMutationTails = new Map();
async function withBacklogMutationLock(workspace, operation) {
    ...
    try {
        ...
        release();
        if (backlogMutationTails.get(key) === tail)
            backlogMutationTails.delete(key);
    }
}
export async function readBacklogObjectStore(workspaceRoot) {
    try {
...
async function resolveBacklogWorkspaceKey(workspace) {
    const configPath = join(workspace.root, ...CONFIG_PATH);
    try {
        const raw = await readFile(configPath, 'utf-8');
        const parsed = JSON.parse(raw);
        if (isValidBacklogKey(parsed.key))
            return parsed.key;
    }
    catch (error) {
...
async function persistBacklogWorkspaceKey(workspace, key) {
    const target = resolve(join(workspace.root, ...CONFIG_PATH));
    if (!isPathInside(workspace.root, target))
        return;
    try {
...
export async function readBacklogWorkspaceKey(workspaceRoot) {
    try {
        const workspace = await validateWorkspaceRoot(workspaceRoot);
...
export async function ensureBacklogItemIds(input) {
    try {
...
const MIGRATABLE_FRONTMATTER_FIELDS = ['status', 'type', 'difficulty', 'criticality', 'risk'];
...
export function planBacklogStoreMigration(parsed) {
    ...
    for (const raw of items) {
        ...
        if (relativePath && Object.keys(updates).length > 0)
            migrations.push({ relativePath, updates });
    }
...
function sanitizeMigratedScalar(value) {
    return value.replace(/[\r\n]+/g, ' ').trim();
}
function recordRelativePath(raw) {
    if (!isPlainRecord(raw))
        return null;
    const source = raw.source;
    if (!isPlainRecord(source) || typeof source.relativePath !== 'string')
        return null;
...
async function loadMigratedStore(workspace) {
    let rawText;
    try {
    ...
    for (const record of plan.slimRecords) {
        ...
        if (relativePath && (await backlogFileExists(workspace, relativePath)))
...
export function readBacklogFrontmatterFields(content) {
...
export async function listBacklogItems(workspaceRoot) {
    try {
        const workspace = await validateWorkspaceRoot(workspaceRoot);
        const paths = [
...
export async function readBacklogItem(workspaceRoot, relativePath) {
    try {
        const workspace = await validateWorkspaceRoot(workspaceRoot);
        const normalized = validateBacklogRelativePath(relativePath);
        const target = resolve(join(workspace.root, normalized));
        if (!isPathInside(workspace.root, target))
            throw new Error('Backlog item path escaped the workspace root.');
        let raw;
        try {
...
async function listAllBacklogSourcePaths(workspace) {
    const paths = [];
...
async function nextBacklogNumericId(workspace) {
    let max = 0;
    for (const relativePath of await listAllBacklogSourcePaths(workspace)) {
        ...
        if (numericId !== undefined && numericId > max)
            max = numericId;
    }
    return max + 1;
}
...
async function peekBacklogWorkspaceKey(workspace) {
    try {
...
export async function createBacklogItem(input) {
    ...
    if (!title)
        return { ok: false, message: 'Enter a title for the new Backlog item.' };
    try {
        const workspace = await validateWorkspaceRoot(input.workspaceRoot);
        return await withBacklogMutationLock(workspace, async () => {
            const store = await loadStore(workspace);
...
export async function repairBacklogIntegrity(input) {
    try {
        const workspace = await validateWorkspaceRoot(input.workspaceRoot);
        ...
        return await withBacklogMutationLock(workspace, async () => {
            ...
            for (const candidatePath of paths) {
                ...
                if (parseBacklogNumericId(parsed.fields.id) === previousNumericId)
                    matches += 1;
            }
            if (matches < 2) {
...
export async function createBacklogEpic(input) {
    const title = input.title.trim();
    if (!title)
        return { ok: false, message: 'Enter a title for the new epic.' };
    try {
        const workspace = await validateWorkspaceRoot(input.workspaceRoot);
        const store = await loadStore(workspace);
        const existingLower = new Set(store.items.map((record) => record.source.relativePath.toLowerCase()));
...
export async function materializeProxyBacklogItem(input) {
    const externalId = input.externalId.trim();
    const connectionId = input.connectionId.trim();
    if (!externalId || !connectionId)
        return { ok: false, message: 'A tracker proxy item needs a connection and external id.' };
    try {
        const workspace = await validateWorkspaceRoot(input.workspaceRoot);
...
export async function markProxyBacklogItemUnavailable(input) {
    const externalId = input.externalId.trim();
    const connectionId = input.connectionId.trim();
    if (!externalId || !connectionId)
        return { ok: false, message: 'A tracker proxy lookup needs a connection and external id.' };
    try {
        const workspace = await validateWorkspaceRoot(input.workspaceRoot);
        const existing = await findProxyBacklogItemPath(workspace, connectionId, externalId);
        if (!existing)
...
async function findProxyBacklogItemPath(workspace, connectionId, externalId) {
    ...
    for (const relativePath of relativePaths) {
        ...
        if (fields.external_connection === connectionId && fields.external_id === externalId)
            return relativePath;
    }
    return null;
}
...
export async function ensureBacklogObjectRecords(workspaceRoot, items) {
    return mutateStore(workspaceRoot, () => null, (store, now) => {
        ...
        for (const item of items) {
            ...
            if (nextItems.some((record) => record.source.relativePath.toLowerCase() === pathKey))
                continue;
...
export async function updateBacklogStatus(input) {
    if (!isBacklogStatus(input.status))
...
export async function updateBacklogType(input) {
...
export async function updateBacklogTriage(input) {
    ...
    if ('criticality' in input)
        updates.criticality = input.criticality ?? null;
    if ('risk' in input)
        updates.risk = input.risk ?? null;
...
export async function updateBacklogEpic(input) {
...
export async function updateBacklogEpicColor(input) {
...
export async function updateBacklogDependencies(input) {
...
export async function updateBacklogDependenciesPlanned(input) {
...
export async function updateBacklogMockups(input) {
...
export async function updateBacklogHighlight(input) {
    if (typeof input.starred !== 'boolean')
        return { ok: false, message: 'Enter a valid Backlog highlight star value.' };
    if (input.color !== null && !isBacklogHighlightColor(input.color)) {
...
export async function addOrUpdateBacklogLink(input) {
    const link = normalizeBacklogLink(input.link);
    if (!link)
        return { ok: false, message: 'Enter a valid Backlog item link.' };
    if (link.target.path) {
    ...
    if (!linked.ok || input.status === undefined)
        return linked;
...
export async function removeBacklogLink(input) {
    const linkId = input.linkId.trim();
    if (!linkId)
        return { ok: false, message: 'Choose a Backlog link to remove.' };
    return mutateItem(input.workspaceRoot, input.relativePath, (record, now) => ({
        ...record,
...
async function clearBacklogSidecarStatus(workspaceRoot, relativePath) {
    try {
        ...
        const items = store.items.map((record) => {
            if (record.source.relativePath.toLowerCase() !== pathKey || record.status === undefined)
                return record;
...
export async function updateBacklogModuleMetadata(input) {
    const moduleId = input.moduleId.trim();
    if (!moduleId)
        return { ok: false, message: 'Backlog module metadata requires a module id.' };
    return mutateItem(input.workspaceRoot, input.relativePath, (record, now) => ({
        ...record,
        metadata: {
            ...(record.metadata ?? {}),
...
export async function moveBacklogObjectSource(input) {
...
export async function removeBacklogObjectRecord(input) {
...
async function writeBacklogFrontmatter(workspaceRoot, relativePath, updates) {
    let normalizedPath;
    try {
...
async function mutateItem(workspaceRoot, relativePath, update) {
...
async function mutateStore(workspaceRoot, validate, update) {
    try {
        validate();
        const workspace = await validateWorkspaceRoot(workspaceRoot);
        const store = await loadStore(workspace);
...
async function validateWorkspaceRoot(workspaceRoot) {
    if (!isAbsolute(workspaceRoot))
        throw new Error('Backlog workspace root must be an absolute path.');
    ...
    if (!rootStat.isDirectory())
        throw new Error('Backlog workspace root must be an existing folder.');
...
async function saveStore(workspace, store) {
    ...
    if (!isPathInside(workspace.root, target))
        throw new Error('Backlog metadata path escaped the workspace root.');
    try {
        await mkdir(dirname(target), { recursive: true });
...
function validateBacklogRelativePath(value) {
...
async function uniqueBacklogFilePath(workspace, title, existingLower) {
...
async function uniqueBacklogFilePathFromBase(workspace, base, existingLower) {
...
async function uniqueEpicFilePath(workspace, slug, existingLower) {
...
async function backlogFileExists(workspace, relativePath) {
    try {
...
function normalizeStore(value) {
    ...
    const items = Array.isArray(raw.items)
        ? raw.items.map(normalizeRecord).filter((record) => Boolean(record))
        : [];
...
function normalizeRecord(value) {
    if (!value || typeof value !== 'object')
        return null;
    const raw = value;
    if (!raw.source || typeof raw.source.relativePath !== 'string')
        return null;
    let relativePath;
    try {
    ...
    return {
        ...
        links: Array.isArray(raw.links) ? raw.links.map(normalizeBacklogLink).filter((link) => Boolean(link)) : [],
        createdAt: typeof raw.createdAt === 'string' ? raw.createdAt : undefined,
        updatedAt: typeof raw.updatedAt === 'string' ? raw.updatedAt : undefined,
    };
}
function normalizeBacklogHighlight(value) {
    if (!isPlainRecord(value))
        return undefined;
    const starred = value.starred === true;
    const color = isBacklogHighlightColor(value.color) ? value.color : null;
    if (!starred && color === null)
        return undefined;
    return { starred, color };
}
function normalizeBacklogLink(value) {
    if (!value || typeof value !== 'object')
        return null;
    const raw = value;
...
function isAbsolutePathInput(path) {
...
function isValidEpicSlug(value) {
    return typeof value === 'string' && /^[A-Za-z0-9._-]+$/.test(value) && value !== '.' && value !== '..';
}
function isBacklogHighlightColor(value) {
