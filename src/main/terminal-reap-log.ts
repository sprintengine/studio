...
const reapEvents: TerminalReapEvent[] = []

export function recordReapEvent(event: TerminalReapEvent): void {
...
export function listRecentReapEvents(): TerminalReapEvent[] {
  return [...reapEvents].reverse()
}

// Test-only: reset the buffer between cases.
export function clearReapEvents(): void {
  reapEvents.length = 0
}
