...
// A live terminal session reduced to what attribution needs: the pty root pid
// plus the metadata the panel shows. Built in terminal-runtime where the
// sessions live.
...
export function sumSubtreeRssBytes(rootPid: number, rows: readonly PsProcessRow[]): number {
  const childrenByParent = new Map<number, PsProcessRow[]>()
  const byPid = new Map<number, PsProcessRow>()
  ...
  if (root) totalKb += Math.max(0, root.rssKb)
...
export function rollupWorkspaceMemory(
  ...
  for (const root of roots) {
    if (root.workspaceId === null) continue
    const memoryBytes = sumSubtreeRssBytes(root.rootPid, rows)
    const terminal: WorkspaceTerminalMemorySample = {
    ...
    if (root.processAlive) {
      if (root.kind === 'agent') sample.resident = true
      sample.becameLiveAt =
