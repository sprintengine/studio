...
} from './mobile-swarm-activity'
import type { MobileControlSnapshot, MobileSwarmSnapshot } from './mobile-swarm-snapshot'
...
async function assertRevokedRegistrationsReceiveNoNotifications(): Promise<void> {
  ...
  assert.equal(deliveries[0].event.payload.category, 'swarm.complete')
}
...
function swarm(
  ...
  return {
    ...
    statePath: `/private/workspace/swarm/${swarmId}/state.yaml`,
    planPath: `/private/workspace/swarm/${swarmId}/plan.md`,
...
function artifact(
  ...
  return {
    ...
    status,
    taskId: 'T1',
    path: `/private/workspace/swarm/team/documents/${artifactId}.md`,
  }
}
...
function commandAudit(): MobileSwarmCommandAuditEntry {
  return {
    ...
    message: 'Swarm tool failed without exposing output.',
    recordedAt: now.toISOString(),
    statePath: '/private/workspace/swarm/team/state.yaml',
    artifactId: 'A1',
