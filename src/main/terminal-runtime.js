...
import { deriveActivityFromPhase, evaluateAgentStall, isAtRestAgentPhase, resolveAgentStateEvent, selectAgentStateTarget } from './agent-state';
import { cleanupTerminalStartupScript, applyAgentIdentityEnv, getPlainShellLaunchConfig, getShellLaunchConfig, getTerminalEnv, } from './terminal-launch';
...
import { agentCliLaunchFailureResult, preflightAgentCliLaunch, } from './cli-availability';
...
import { appendTerminalOutput, clearAgentStallTimer, clearTerminalIdleTimer, createFailedTerminalSession, createHeadlessTerminalSender, createInitialAgentState, createInitialTerminalActivity, createSuspendedPlaceholderSession, getTerminalLastSeenAt, getTerminalSize, getTerminalSnapshot, getTerminalIdleTimeoutMs, isTerminalProcessAlive, isTerminalSessionStale, materializeTerminalReplay, recordTerminalInput, recordTerminalVisibility, transitionTerminalActivity, } from './terminal-session';
import { buildReplaySnapshot } from './terminal-replay-snapshot';
import { TERMINAL_REMOTE_PENDING_LIMIT_BYTES, TERMINAL_REMOTE_TRANSPORT_HIGH_WATER_BYTES, } from './terminal-remote-attach';
import { recordSprintSessionForTokenLedger, sampleSprintSessionTokenUsage, } from './sprintengine-token-sampling';
import { killCliSessionSurvivors, probeSubtreesForLiveWork } from './terminal-subtree-probe';
...
import { explainSessionReapDecision, selectReapableSessions, clampSuspendIdleAfterMs, clampKeepRecentTerminalsAlive, DEFAULT_SUSPEND_IDLE_AFTER_MS, DEFAULT_KEEP_RECENT_TERMINALS_ALIVE, } from './terminal-reap-policy';
import { recordReapEvent } from './terminal-reap-log';
let requireAuthenticatedUser = (_message) => { };
let terminalDiagnostics = createTerminalDiagnostics({
    enabled: false,
    logMainPerfEvent: () => { },
});
let logMainPerfEvent = () => { };
const agentSessionExitListeners = new Set();
const agentPhaseListeners = new Set();
let syncMcpConfig;
let releaseManagedSprintEngineRun;
let callManagedSprintEngineTool;
let ensureBuiltinSkillInstalled;
let excludeWorktreeMcpConfig;
let prepareAgentStateHook;
let snapshotSidecars;
let logReapDiagnostic;
let setSprintEngineAutomationModeAdapter;
let resolveAutomationsFrontDoorAdapter;
...
// no frames.
function agentStateSupportsCli(cli) {
    if (!cli)
        return false;
...
function agentStateSpecForSession(session) {
...
export function cliResumeCapabilities(cli) {
...
function cliResumesWithCallerSessionId(cli) {
    return cliResumeCapabilities(cli).sessionIdFromCaller;
}
const sprintEngineMcpRunRefCounts = new Map();
const sprintEngineMcpWorkspaceRefCounts = new Map();
const pendingSprintEngineMcpRunReleases = new Set();
const sprintEngineAgentHeartbeatTimers = new Map();
const pendingSprintEngineLifecycleCalls = new Set();
const pendingSprintEngineTerminalTeardowns = new Map();
 ...
 * worktree config.
 */
export function mcpSettingsForManagedSprintEngineLaunch(settings, cli) {
 ...
 */
export function deriveSprintEngineRegistrationRoot(statePath, launchCwd) {
    let current = dirname(statePath);
    while (true) {
        if (basename(current) === '.multi-code')
...
export function buildManagedSprintEngineSyncInputForLaunch(statePath, launchCwd, launch) {
...
export function sprintEngineRegistryRootsForLaunch() {
    try {
...
function sprintEngineRoleForLaunch(role, agentId) {
    if (role?.trim())
        return role.trim();
...
const pendingAgentListenerRecords = new Set();
function trackAgentListenerRecord(record) {
    pendingAgentListenerRecords.add(record);
    void record.finally(() => {
        pendingAgentListenerRecords.delete(record);
    });
}
function descriptorPromptInput(prompt) {
    if (!prompt)
        return undefined;
    return process.platform === 'win32' ? `${prompt}\r\n\x1a\r\n` : `${prompt}\n\x04`;
}
function descriptorPromptKeystrokes(prompt) {
    if (!prompt)
        return undefined;
...
function installAgentLifecycleWatcher(terminalSession, descriptor, injectionMode) {
    ...
    if (!needsReadinessWatch && !needsCompletionWatch)
        return;
    let readinessPattern;
    if (needsReadinessWatch && descriptor.injection?.readiness) {
        try {
            ...
            console.error(`Invalid readiness regex from agent descriptor ${descriptor.executionId}: ${String(err)}`);
        }
    }
    const watcher = createAgentStreamWatcher({
        readinessPattern,
        completionSentinel: completionMode === 'output-sentinel' ? descriptor.completion?.sentinel : undefined,
    });
    ...
    if (needsReadinessWatch && descriptor.injection?.readiness?.timeoutMs) {
        readinessTimer = setTimeout(() => {
            ...
            if (!terminalSession.isDisposed) {
                ...
                if (fallback)
                    terminalSession.process.write(fallback);
            }
...
function sendTerminalEvent(sender, channel, payload) {
    if (!sender.isDestroyed()) {
        sender.send(channel, payload);
    }
}
const remoteTerminalViewers = new Map();
function remoteTerminalSinks(sessionId) {
    const viewers = remoteTerminalViewers.get(sessionId);
    if (!viewers)
        return [];
    return [...viewers.values()].map((viewer) => remoteTerminalSink(sessionId, viewer));
}
function remoteTerminalSink(sessionId, viewer) {
    return {
...
function forwardToRemoteViewer(sessionId, viewer, data) {
...
function endRemoteTerminalViewers(sessionId, reason) {
    const viewers = remoteTerminalViewers.get(sessionId);
    if (!viewers)
        return;
    ...
    for (const viewer of viewers.values()) {
        ...
        if (viewer.transport.isOpen())
            viewer.transport.send({ type: 'ended', reason });
    }
}
function notifyRemoteTerminalExit(sessionId, exitCode) {
    const viewers = remoteTerminalViewers.get(sessionId);
    if (!viewers)
        return;
    for (const viewer of viewers.values()) {
        if (viewer.transport.isOpen())
            viewer.transport.send({ type: 'exit', exitCode });
    }
}
function attachRemoteTerminalViewer(input) {
    ...
    const viewer = { transport: input.transport, scope: input.scope, desynced: false };
    const viewers = remoteTerminalViewers.get(input.sessionId) ?? new Map();
    ...
    const detach = () => {
        ...
        if (current.size === 0)
            remoteTerminalViewers.delete(input.sessionId);
    };
    const requireControl = (verb) => input.scope === 'control'
        ? null
        : {
            ok: false,
 ...
 */
export function resolveSpawnEventSink() {
    return BrowserWindow.getAllWindows()
        .find((win) => !win.isDestroyed() && !win.webContents.isDestroyed())
        ?.webContents
        ?? createHeadlessTerminalSender();
}
function broadcastTerminalSessionsChanged() {
...
function setTerminalVisible(sessionId, visible, sender) {
    const session = terminals.get(sessionId);
    if (!session || session.isDisposed)
        return;
...
function safeResizeTerminal(sessionId, cols, rows) {
    const session = terminals.get(sessionId);
    ...
    if (session.appliedCols === size.cols && session.appliedRows === size.rows)
        return;
    try {
...
function flushPendingTerminalResize(sessionId, session) {
...
export function suspendTerminal(sessionId) {
    const session = terminals.get(sessionId);
    if (!session || session.isDisposed || session.suspended || session.suspending)
        return;
    ...
    if (session.cliSessionId)
        void killCliSessionSurvivors(session.cliSessionId);
...
function writeTerminalSnapshotSidecar(session, payload) {
    if (!snapshotSidecars)
        return;
    ...
    if (session.kind !== 'agent')
        return;
    if (!payload.snapshot && !payload.rawReplay)
        return;
...
async function rehydrateSuspendedTerminalFromSidecar(sessionId, sender) {
    if (!snapshotSidecars)
        return undefined;
...
async function resumeTerminal(sender, payload) {
...
async function waitForTerminalExit(session, timeoutMs) {
    ...
    if (session.hasExited || session.isDisposed || session.suspended)
        return true;
...
export function setIdleSuspendThresholdMs(value) {
    configuredSuspendIdleAfterMs = clampSuspendIdleAfterMs(value);
}
export function getIdleSuspendThresholdMs() {
...
export function setKeepRecentTerminalsAlive(value) {
    configuredKeepRecentAliveCount = clampKeepRecentTerminalsAlive(value);
}
export function getKeepRecentTerminalsAlive() {
    return configuredKeepRecentAliveCount;
}
let agentCliPreflightOverrides = {};
export function __setAgentCliPreflightForTest(overrides) {
...
export function setTerminalReapExempt(sessionId, exempt) {
    const session = terminals.get(sessionId);
    if (!session || session.isDisposed)
        return;
    const next = exempt === true;
    if ((session.reapExempt === true) === next)
        return;
...
let activeSprintRunStatePaths = new Set();
export function setActiveSprintRunStatePaths(value) {
    activeSprintRunStatePaths = new Set(Array.isArray(value) ? value.filter((entry) => typeof entry === 'string') : []);
}
let staleTerminalSweepTimer;
function startStaleTerminalSweep() {
    if (staleTerminalSweepTimer)
...
function stopStaleTerminalSweep() {
    if (!staleTerminalSweepTimer)
        return;
    clearInterval(staleTerminalSweepTimer);
...
export function reapStaleTerminals(now = Date.now(), guardHolds, probedSessionIds) {
...
const reapSkipLogState = new Map();
...
// incident lacked.
function recordGuardHoldSkip(sessionId, guardHolds, now, sweep) {
...
function terminalLastInteractionAt(session) {
...
export function runIdleAgentReapSweep(now = Date.now(), guardHolds, probedSessionIds) {
    ...
    for (const staleKey of reapSkipLogState.keys()) {
        if (!terminals.has(staleKey))
            reapSkipLogState.delete(staleKey);
    }
    if (logReapDiagnostic) {
        for (const candidate of candidates) {
            ...
            if (explanation.verdict !== 'held')
                continue;
            if (explanation.hold === 'resting_recently' || explanation.hold === 'dead_process')
                continue;
            if (explanation.restingForMs <= configuredSuspendIdleAfterMs)
                continue;
...
export async function runGuardedTerminalReapSweeps(now = Date.now(), deps = {}) {
...
// retained buffers.
async function buildReapGuardHolds(targetSessionIds, deps) {
    const holds = new Map();
    const subtreeTargets = [];
    for (const sessionId of targetSessionIds) {
        const session = terminals.get(sessionId);
        if (!session || session.isDisposed || !isTerminalProcessAlive(session))
            continue;
        const pid = session.process.pid;
        if (typeof pid === 'number' && pid > 0)
            subtreeTargets.push({ sessionId, pid });
    }
    if (subtreeTargets.length > 0) {
        const verdicts = await probeSubtreesForLiveWork(subtreeTargets.map((target) => target.pid), deps.subtree);
        for (const target of subtreeTargets) {
            const reason = verdicts.get(target.pid);
            if (reason === undefined)
                holds.set(target.sessionId, 'subtree_undetermined');
            else if (reason !== null)
                holds.set(target.sessionId, `live_subtree_${reason}`);
        }
    }
    return holds;
}
...
export function listTerminalRoots() {
    return [...terminals.values()]
...
async function shutdownTerminalRuntime() {
...
function getLiveAgentExecutionIds() {
    return [...terminals.values()]
...
function resolveAgentExecutionId(input) {
 ...
 */
function adoptSprintRunSessionWorkspaceId(input) {
    let moved = 0;
    for (const session of terminals.values()) {
        if (session.isDisposed)
            continue;
        if (!samePath(session.sprintEngineStatePath ?? null, input.statePath))
            continue;
...
function disposeOtherAgentSessions(sessionId, workspaceId, agentId, sprintEngineStatePath) {
...
function scheduleTerminalIdleTransition(session) {
    clearTerminalIdleTimer(session);
    if (!isTerminalProcessAlive(session))
        return;
...
function setTerminalActivity(session, nextActivity, options = {}) {
...
function resolveSessionForAgentStateFrame(frame) {
...
function scheduleAgentStallCheck(session) {
    clearAgentStallTimer(session);
    if (!isTerminalProcessAlive(session))
        return;
    ...
    if (state.phase !== 'starting' && state.phase !== 'thinking' && state.phase !== 'tool_use')
        return;
    if (state.source !== 'hook' && state.phase !== 'starting')
        return;
    session.agentStallTimer = setTimeout(() => runAgentStallCheck(session), AGENT_STALL_THRESHOLD_MS);
}
function runAgentStallCheck(session) {
    session.agentStallTimer = undefined;
    if (!isTerminalProcessAlive(session) || !session.agentState)
        return;
    ...
    if (derived)
        setTerminalActivity(session, derived, { broadcast: false });
    if (terminals.get(session.sessionId) === session)
        broadcastTerminalSessionsChanged();
}
function ingestAgentStateFrame(frame) {
    ...
    if (resolution.phase === 'starting')
        session.pendingWakeupAt = null;
    if (frame.wakeup) {
    ...
    // the transition.
    if (session.sprintEngineStatePath
    ...
    const bridged = derived && derived.kind === 'working' && session.activity.kind === 'working'
        ? { kind: 'working', since: session.activity.since }
        : derived;
    ...
    if ((activityChanged || attentionChanged || cliSessionIdChanged || promptChanged)
...
function notifyAgentPhaseListeners(session, frame, resolution, previousPhase) {
    if (agentPhaseListeners.size === 0)
        return;
    const phaseEvent = {
        workspaceId: session.workspaceId ?? null,
...
function releaseSprintEngineMcpRun(session) {
...
function retainSprintEngineMcpRunRef(runId, workspaceRoot) {
    if (!runId)
        return;
...
function releaseSprintEngineMcpRunRef(runId, context) {
...
function releaseUnusedSprintEngineMcpRun(runId, context) {
    if (!runId || (sprintEngineMcpRunRefCounts.get(runId) ?? 0) > 0)
        return;
...
function queueSprintEngineMcpRunRelease(runId, context) {
    const workspaceRoot = context?.workspaceRoot?.trim();
    const cleanupMcpConfig = Boolean(workspaceRoot && context?.cleanupMcpConfig === true);
    const clients = cleanupMcpConfig
        ? ['codex', 'claude-code']
        : context?.clients?.length ? context.clients : ['codex', 'claude-code'];
...
function sprintEngineLifecycleCallInput(session, toolName, reason) {
    if (!session.sprintEngineMcpRunId || !session.sprintEngineStatePath || !session.agentId)
        return null;
    return {
...
function queueSprintEngineLifecycleCall(session, toolName, reason) {
    const input = sprintEngineLifecycleCallInput(session, toolName, reason);
    if (!input || !callManagedSprintEngineTool)
        return null;
...
function startSprintEngineAgentHeartbeat(session) {
    if (sprintEngineAgentHeartbeatTimers.has(session.sessionId))
        return;
    if (!sprintEngineLifecycleCallInput(session, 'sprintengine.agent.heartbeat'))
        return;
...
function stopSprintEngineAgentHeartbeat(sessionId) {
...
export async function sendSprintEngineAgentHeartbeats() {
    const sent = [];
    for (const session of terminals.values()) {
        if (!isTerminalProcessAlive(session))
            continue;
        ...
        if (session.agentId)
            sent.push(session.agentId);
    }
    return sent;
}
function recordSprintEngineAgentLeave(session, reason) {
    stopSprintEngineAgentHeartbeat(session.sessionId);
    return queueSprintEngineLifecycleCall(session, 'sprintengine.agent.leave', reason);
}
function queueSprintEngineTerminalTeardown(session, reason) {
...
function materializeAgentSessionIdentity(sessionId, workspaceId, agentSession) {
    if (!agentSession)
        return undefined;
    return {
...
function attachTerminalSession(sessionId, terminalSession, initialInput) {
...
async function spawnAgentSessionFromDescriptor(input) {
    ...
    try {
        ...
        const descriptorEnv = applyAgentIdentityEnv({ ...getTerminalEnv(), ...(input.descriptor.env ?? {}) }, {
            workspaceId: input.workspaceId,
            agentId: input.descriptor.executionId,
...
function createMobileCommandService() {
...
async function spawnMobileAgentTerminal(input) {
    ...
    disposeTerminal(input.sessionId);
    let sprintEngineMcpRunId;
    let sprintEngineMcpEnv;
    ...
    try {
        ...
        if (mobileBlock)
            return { ok: false, message: mobileBlock.message };
        const { command, args, cwd: launchCwd, pathStyle, initialInput, env, startupScriptPath } = getShellLaunchConfig(input.cwd, input.sessionId, false, input.sprintEngineStatePath, input.cli, input.initialPrompt, undefined, 'auto_workspace', undefined, undefined, undefined, sprintEngineMcpEnv, false, mobileCliAuthToken);
...
async function spawnTerminalFromIpc(sender, { sessionId, cols, rows, cwd, resume, sprintEngineStatePath, cli, initialPrompt, cliRuntimes, shellOnly, cliSessionId, kind, workspaceId, agentId, agentName, terminalId, executionMode, worktreeId, worktreePath, cliPermissionPreset = 'default', debugMode = false, cliModel, cliReasoning, memoryRootPath, memoryRelativeRoot, agentSession, visible = true, mcpSettings, connectorLaunch, connectorSkillId, spawnSkillId, agentRecord, }) {
    ...
    try {
        ...
        if (!shellOnly) {
            ...
            if (preflight.status === 'resolved')
                resolvedBinaryPath = preflight.binaryPath;
        }
        ...
        const skillIdsToInstall = [...new Set([connectorSkillId, spawnSkillId].filter((id) => Boolean(id)))];
        ...
        if (!shellOnly && cliAuthSecret) {
            ...
            if (block)
                return { ok: false, sessionId, message: block.message, exitCode: 1 };
        }
        const { command, args, cwd: launchCwd, pathStyle, initialInput, env, startupScriptPath } = shellOnly
            ...
            : getShellLaunchConfig(workingDirectory, launchSessionId, resume, sprintEngineStatePath, agentCli, initialPrompt, cliRuntimes, cliPermissionPreset, cliModel, memoryRootPath, memoryRelativeRoot, sprintEngineMcpEnv, debugMode, cliAuthToken, cliReasoning, resolvedBinaryPath);
...
function readTerminalOutput(sessionId) {
    const session = terminals.get(sessionId);
    if (!session || session.isDisposed)
        return undefined;
    return materializeTerminalReplay(session);
}
function writeTerminalInput(sessionId, data) {
    const startedAt = Date.now();
    const session = terminals.get(sessionId);
    if (!session || !isTerminalProcessAlive(session))
        return;
