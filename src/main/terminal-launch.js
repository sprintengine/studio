...
function agentStateSocketPathForLaunch() {
    try {
...
const AGENT_IDENTITY_ENV_KEYS = ['MULTICODE_WORKSPACE_ID', 'MULTICODE_AGENT_ID', 'MULTICODE_AGENT_NAME', 'MULTICODE_AGENT_STATE_SOCKET'];
...
export function applyAgentIdentityEnv(baseEnv, input) {
    const next = { ...baseEnv };
    for (const key of AGENT_IDENTITY_ENV_KEYS)
        delete next[key];
...
const PROTECTED_LAUNCH_ENV_KEYS = new Set([
...
function redirectsAnthropicEndpoint(providerEnv) {
...
// `launch.env`.
export function mergeProviderLaunchEnv(base, providerEnv) {
    ...
    for (const [key, value] of Object.entries(providerEnv)) {
        if (PROTECTED_LAUNCH_ENV_KEYS.has(key))
            continue;
        next[key] = value;
    }
    return next;
...
function sprintEngineRegistryRootsEnvValue() {
    const roots = getPluginSprintEngineRegistryRoots();
    return roots.length ? JSON.stringify(roots) : null;
}
function withSprintEngineEnv(env, cwd, sprintEngineStatePath, memoryRootPath, memoryRelativeRoot, managedMcpEnv) {
    ...
    const managedPythonEnv = managedPython.source === 'bundled' || managedPython.source === 'override'
    ...
    if (!registryRootsEnv)
        delete nextEnv.MULTICODE_SPRINTENGINE_REGISTRY_ROOTS;
...
function ensurePosixToolShimDirectory() {
    try {
        ...
        writeFileSync(sprintEngineShimPath, [
            '#!/usr/bin/env bash',
            'set -euo pipefail',
            'tool_path=""',
...
function ensureWindowsSprintEngineShimDirectory() {
    if (process.platform !== 'win32')
        return null;
    try {
...
function normalizeInitialPromptPaths(initialPrompt, target, paths) {
    if (!initialPrompt)
...
function getCliRuntimeSettings(cli, cliRuntimes) {
...
function getBundledSprintEngineToolPath() {
    if (app.isPackaged) {
...
function buildSprintEngineShellBootstrap(sprintEngineStatePath, memoryRootPath, memoryRelativeRoot, managedMcpEnv, providerLaunchEnv) {
    const shellStatePath = sprintEngineStatePath && process.platform === 'win32' ? toWslPath(sprintEngineStatePath) : sprintEngineStatePath;
    const shellMemoryRootPath = memoryRootPath && process.platform === 'win32' ? toWslPath(memoryRootPath) : memoryRootPath;
    const bundledToolPath = getBundledSprintEngineToolPath();
    const soulsRoot = getBundledSoulsRoot();
    const posixShimDirectory = ensurePosixToolShimDirectory();
    const shellBundledToolPath = bundledToolPath && process.platform === 'win32' ? toWslPath(bundledToolPath) : bundledToolPath;
    const shellSoulsRoot = soulsRoot && process.platform === 'win32' ? toWslPath(soulsRoot) : soulsRoot;
    const shellPosixShimDirectory = posixShimDirectory && process.platform === 'win32' ? toWslPath(posixShimDirectory) : posixShimDirectory;
    const lines = [
    ...
    lines.push([
        ...
        '}',
    ].join(' '), 'export -f sprintengine >/dev/null 2>&1 || true', [
        'souls() {',
        'local python_exe="python3";',
        'if [ -n "${MULTICODE_PYTHON:-}" ] && [ -x "$MULTICODE_PYTHON" ]; then python_exe="$MULTICODE_PYTHON";',
        'elif [ -x "$PWD/.venv/bin/python" ]; then python_exe="$PWD/.venv/bin/python";',
        ...
        '}',
    ].join(' '), 'export -f souls >/dev/null 2>&1 || true');
    return lines.join('; ');
}
function buildUserShellStartup() {
    return [
...
function isLoginShell(shellName) {
 ...
 */
export function resolveDefaultShellName() {
    if (process.platform === 'win32')
        return 'powershell';
    const path = getPosixShellPath();
    return path.split('/').pop() || path;
}
function getPosixShellPath() {
...
function buildInteractiveShellExec(shellPath, shellName) {
    const loginArg = isLoginShell(shellName) ? ' -l' : '';
    return `exec ${quotePosixCommand(shellPath)}${loginArg}`;
}
function createTerminalStartupScript(sessionId, extension, content) {
...
export function cleanupTerminalStartupScript(scriptPath) {
    if (!scriptPath)
        return;
    void unlink(scriptPath).catch(() => { });
}
function buildWslShellScript(cwd, sessionId, resume = false, sprintEngineStatePath, cli = 'codex', initialPrompt, cliRuntime, cliPermissionPreset = 'default', cliModel, memoryRootPath, memoryRelativeRoot, managedMcpEnv, debugMode = false, providerLaunchEnv, cliReasoning) {
...
export function getShellLaunchConfig(cwd, sessionId, resume = false, sprintEngineStatePath, cli = 'codex', initialPrompt, cliRuntimes, cliPermissionPreset = 'default', cliModel, memoryRootPath, memoryRelativeRoot, managedMcpEnv, debugMode = false, cliAuthToken, cliReasoning, 
...
resolvedBinaryPath) {
    ...
    if (process.platform === 'win32' && !cliRuntime.useWsl) {
        ...
        if (!isNativeWindowsPath(windowsCwd)) {
            throw new Error(`Workspace path "${cwd}" is not available as a Windows path. Turn on "Run through WSL" for ${cli}.`);
        }
        const startupScriptPath = createTerminalStartupScript(sessionId, 'ps1', buildNativeAgentLaunchPowerShellScript(cli, sessionId, resume, windowsCwd, shellInitialPrompt, cliRuntime, cliPermissionPreset, cliModel, debugMode, cliReasoning));
        return {
            command: 'powershell.exe',
            args: ['-NoLogo', '-NoExit', '-ExecutionPolicy', 'Bypass', '-File', startupScriptPath],
            env: mergeProviderLaunchEnv(withSprintEngineEnv(getTerminalEnv(), windowsCwd, windowsStatePath, windowsMemoryRootPath, memoryRelativeRoot, managedMcpEnv), providerLaunchEnv),
            cwd: windowsCwd,
            pathStyle: 'windows',
            startupScriptPath,
        };
    ...
    if (process.platform === 'win32') {
        const startupScriptPath = createTerminalStartupScript(sessionId, 'sh', buildWslShellScript(cwd, sessionId, resume, sprintEngineStatePath, cli, initialPrompt, cliRuntime, cliPermissionPreset, cliModel, memoryRootPath, memoryRelativeRoot, managedMcpEnv, debugMode, providerLaunchEnv, cliReasoning));
        return {
            command: 'wsl.exe',
            args: [
                '-e',
    ...
    return {
        ...
        cwd,
        env: mergeProviderLaunchEnv(withSprintEngineEnv(getTerminalEnv(), cwd, sprintEngineStatePath, memoryRootPath, memoryRelativeRoot, managedMcpEnv), providerLaunchEnv),
        pathStyle: 'posix',
        startupScriptPath,
    };
}
export function getPlainShellLaunchConfig(cwd, sprintEngineStatePath, sessionId = 'plain-terminal') {
    assertExistingDirectory(cwd);
    if (process.platform === 'win32') {
        const windowsCwd = toWindowsPath(cwd);
        const windowsStatePath = sprintEngineStatePath ? toWindowsPath(sprintEngineStatePath) : undefined;
        ...
        const startupScriptPath = createTerminalStartupScript(sessionId, 'sh', [
            buildUserShellStartup(),
            `cd ${quotePosix(toWslPath(cwd))}`,
...
function buildNativeAgentLaunchPowerShellScript(cli, sessionId, resume, cwd, initialPrompt, cliRuntime, cliPermissionPreset = 'default', cliModel, debugMode = false, cliReasoning) {
    ...
    if (pluginIdForCli(cli) === 'codex') {
        return buildCodexLegacyNativeAgentLaunchPowerShellScript(sessionId, resume, cwd, initialPrompt, cliRuntime, cliPermissionPreset, cliModel, debugMode, cliReasoning);
    }
...
export function buildCodexLegacyNativeAgentLaunchPowerShellScript(sessionId, resume, cwd, initialPrompt, cliRuntime, cliPermissionPreset = 'default', cliModel, debugMode = false, cliReasoning) {
...
function buildAgentLaunchCommand(cli, sessionId, resume = false, initialPrompt, cliRuntime, cliPermissionPreset = 'default', cliModel, debugMode = false, cliReasoning, resolvedBinaryPath) {
