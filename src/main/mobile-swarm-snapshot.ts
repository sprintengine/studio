...
export async function readSwarmSnapshot(statePathInput: string): Promise<MobileSwarmSnapshot> {
  ...
  if (basename(statePath) !== 'state.yaml') {
    throw new Error('Swarm snapshot state path must point to a state.yaml file.')
  }
  ...
  const swarm = parsed.swarm && typeof parsed.swarm === 'object' ? parsed.swarm : {}
  const updatedAt = isoStringOrNull(swarm.updatedAt) ?? stateStats.mtime.toISOString()
  ...
  return {
    swarmId,
    name: stringOrFallback(swarm.name, swarmId),
    workspacePath,
    statePath,
