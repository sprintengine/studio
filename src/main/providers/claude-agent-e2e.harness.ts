  ...
  return createClaudeAgentProvider({
    ...
    buildEnv: (input) => {
      ...
      for (const [key, value] of Object.entries(process.env)) {
        if (typeof value === 'string' && key !== 'ELECTRON_RUN_AS_NODE') passthrough[key] = value
      }
...
function createRuntime() {
  return new ConversationRuntime({
    ...
    secretStore: { getStatus: async () => ({ ok: false, message: 'no secrets in harness' }) } as never,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const orphans = execSync(
      `ps ax -o pid=,command= | grep -F 'MULTICODE_CONVERSATION_SESSION_ID' | grep -v grep || true`
    ).toString().trim()
