...
export type SystemMemoryDeps = {
  ...
  totalBytes?: number
  // macOS: `vm_stat` output. Linux: `/proc/meminfo` contents. Injectable for tests.
...
export function parseDarwinVmStat(
  ...
  if (free === 0 && inactive === 0 && compressed === 0) return null
...
function parseSwapUsedBytes(swapUsage: string): number {
...
export function parseLinuxMemInfo(memInfo: string, totalBytes: number): SystemMemorySample | null {
...
function osMemorySample(totalBytes: number): SystemMemorySample {
...
export async function sampleSystemMemory(deps: SystemMemoryDeps = {}): Promise<SystemMemorySample> {
