...
import { addOrUpdateBacklogLink, listBacklogItems, readBacklogItem, repairBacklogIntegrity, updateBacklogDependenciesPlanned, updateBacklogEpic, updateBacklogStatus, updateBacklogTriage, updateBacklogType, } from './backlog-service';
...
import { mobileControlProtocolVersion, mobileSnapshotCollections, } from '../shared/mobile-control/protocol';
...
import { createAgentCapabilityService, createFsSkillDirectoryReader, createWorkspaceSkillsService, } from './workspace-skills-service';
...
import { createSprintRuntime } from './sprint-runtime';
...
import { SPRINT_RUNS_CHANGED_CHANNEL } from '../shared/sprintengine/runSummary';
import { invalidateSprintRunSummary, listSprintRuns, readSprintRunVcs, watchSprintRunProjections, } from './sprintengine-run-index';
...
export function createAppServices(diagnosticsEnabled) {
    ...
    const resolveStudioMcpBridgeScriptPath = () => app.isPackaged
    ...
    const agentStateService = createAgentStateService({
        ...
        resolveAgentStateSpec: (cli) => getPluginRegistry()
            .loaded()
    ...
    let resolveAutomationsAppFrontDoor = () => null;
    ...
    let resolveRoadmapAppFrontDoor = () => null;
    ...
    let resolveModuleEnabled = () => false;
    ...
    const notifySprintRunsChanged = (statePath) => {
    ...
    const sprintRuntime = createSprintRuntime({
        ...
        artifacts: {
            readProjection: (input) => sprintEngineArtifacts.readProjection(input),
            autoApproveArtifact: ({ statePath, artifactId }) => sprintEngineArtifacts.reviewArtifact({ statePath, artifactId }, 'approve', 'auto-run'),
        ...
        broadcastOp: (op) => {
            ...
            if (op.statePath)
                trackerWriteBack.notifyRunActivity(op.statePath);
            ...
            if (op.statePath)
                notifySprintRunsChanged(op.statePath);
        },
    ...
    // with the process.
    const sprintRunnerLog = createSprintEngineRunnerLog((workspaceId) => sprintRuntimeRef?.resolveRunnerLogTarget(workspaceId) ?? null);
    ...
    const sprintCreateService = createSprintCreateService({
        ...
        listLaunchableClis: () => getPluginRegistry()
            .loaded()
        ...
        setCliPermissionPreset: (input) => sprintEngineAutomation.setCliPermissionPreset({ ...input, actor: 'automation' }),
    ...
    const skillsService = createSkillsService(app.getPath('userData'), {
        ...
        listWorkspaceRoots: () => workspaceSyncService
            .getSnapshot()
    ...
    let tailnetToolsFrontDoor = null;
        ...
        resolveGatewayTools: createStudioGatewayTools({
            ...
            appTools: createAutomationTools({
                ...
                mobileControl: (() => {
                    ...
                    const gatewayCommands = ['snapshot.request', 'backlog.update', 'sprintengine.create'];
                    const workspaceRoots = () => uniqueResolvedRoots(listKnownWorkspaceRoots(workspaceSyncService.getSnapshot()));
                    return {
                        async readSnapshot(input) {
                            const roots = workspaceRoots();
                            const include = input.include?.filter((entry) => mobileSnapshotCollections.includes(entry));
                            ...
                            if (input.knownSnapshotVersion && input.knownSnapshotVersion === safe.snapshotVersion) {
                                return { unchanged: true, snapshotVersion: safe.snapshotVersion };
                            }
                            return { unchanged: false, snapshot: safe };
                        },
                        async dispatchCommand(input) {
                            ...
                            const allowedWorkspaceRoots = input.type === 'sprintengine.create' ? stateRoots : [...stateRoots, ...workspaceRoots()];
                ...
                resolveSprintTaskInput: async (payload) => {
                    ...
                    if (result.ok)
                        sprintRuntime.resumeIfBlocked(payload.statePath);
                    return result;
                },
                ...
                createAgentWorktree: async ({ workspaceRoot, name }) => {
                    ...
                    if (!paths)
                        return { error: `"${name}" does not reduce to a usable worktree name.` };
                    ...
                    if (!created.ok)
                        return { error: created.message ?? 'Git worktree creation failed.' };
                ...
                listModuleContributedTools: () => resolveModuleMcpTools().map((tool) => ({ moduleId: tool.moduleId, toolName: tool.registration.name })),
    ...
    return {
        ...
        setAutomationsAppFrontDoorResolver(resolver) {
        ...
        getAutomationsAppFrontDoor: () => resolveAutomationsAppFrontDoor(),
        setRoadmapAppFrontDoorResolver(resolver) {
            resolveRoadmapAppFrontDoor = resolver;
        },
        setModuleEnabledResolver(resolver) {
            resolveModuleEnabled = resolver;
        },
        setModuleMcpToolsResolver(resolver) {
...
function getBundledHookReporterPath(filename) {
    if (app.isPackaged) {
...
function getBundledAgentStateReporterPath() {
...
function getBundledAgentStateReporterTemplatePath(template) {
    if (!template || template.includes('/') || template.includes('\\') || template.includes('..'))
        return null;
