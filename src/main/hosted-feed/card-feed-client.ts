// Fetches the hosted card feed (src/shared/hosted-card-feed.ts): HTTPS only,
// ETag/304, a cache file under userData, a bundled seed for the first boot and
// for offline, and honest source/state reporting. sources-feed-client.ts
// fetches its feed the same way. Deliberately a sibling and not a shared
// abstraction — the feeds have different schemas and must be free to drift. The
// rules are restated here rather than imported for the same reason, and two of
// them are honoured more strictly here than a background-only feed would need;
// both are marked below and say why the card feed cannot afford the looser
// version.
//
...
// parsed feed — the second of the stricter rules, and the one that
...
export class HostedCardFeedClient {
  ...
  // of the stricter rules. The in-flight guard hands back whatever
  ...
  // for an answer that was sitting on disk. A feed only a background poller
  // reads can live with that, because nothing draws on the result; the
  ...
  //
  // A seed regenerated from the hosted file it fetches never leads the remote,
  // and a feed read only by a background poller can afford a wasted round trip,
  // so neither would need this. This one is authored ahead of the hosted file
  // and read every time the home page opens.
