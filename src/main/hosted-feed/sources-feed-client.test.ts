...
async function main(): Promise<void> {
  ...
  // ── The de-duplication ────────────────────────────────────────────────────
  {
    ...
    assert.deepEqual(
      recommendedSourcesToAdd(list, [...ALWAYS_PRESENT_SKILL_SOURCE_IDS, 'github:wshobson/agents']).map((s) => s.repo),
      [],
    )
  ...
  await withDir(async (dir) => {
    ...
    const client = new HostedSourcesFeedClient({ feedUrl: FEED_URL, cachePath: join(dir, 'cache.json'), fetcher, now })
  ...
  await withDir(async (dir) => {
    ...
    for (const body of ['{ not json', JSON.stringify({ ...feed('2026-09-09T00:00:00Z', ['a/b']), schemaVersion: 7 })]) {
