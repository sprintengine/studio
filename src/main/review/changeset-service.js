...
export function registerReviewSourceProvider(kind, provider) {
...
export class ReviewChangeSetService {
    ...
    async detect(input) {
        ...
        if (!provider)
            return { ok: false, error: providerMissingMessage(input.kind) };
        try {
    ...
    async build(input) {
        const provider = providers.get(input.kind);
        if (!provider)
            throw new Error(providerMissingMessage(input.kind));
    ...
    async read(targetDir) {
        ...
        if (!validation.ok)
            return { ok: false, error: `Stored change set is invalid: ${validation.errors[0]}` };
        return { ok: true, changeset: validation.value };
    }
}
export function createReviewChangeSetService() {
...
export function reviewChangeSetDir(workspaceRoot, workspaceId) {
    if (!workspaceRoot)
        throw new Error('Review change set requires a workspace root.');
...
function assembleChangeSet(build) {
    ...
    if (build.description !== undefined)
        changeset.description = build.description;
    if (build.baseSha !== undefined)
        changeset.baseSha = build.baseSha;
    if (build.headRef !== undefined)
        changeset.headRef = build.headRef;
    if (build.headSha !== undefined)
        changeset.headSha = build.headSha;
    return changeset;
}
...
const branchProvider = {
    async build(input) {
        if (input.kind !== 'branch')
            throw new Error('Branch provider received a non-branch source.');
...
const patchProvider = {
    async build(input) {
        if (input.kind !== 'patch')
            throw new Error('Patch provider received a non-patch source.');
        const text = input.text;
        if (text.trim().length === 0)
            throw new Error('The patch is empty.');
...
function providerMissingMessage(kind) {
    if (kind === 'pull-request')
        return PULL_REQUEST_NOT_INSTALLED;
