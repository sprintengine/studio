...
import { createDefaultGhRunner, defaultResolveToken } from './github-pr-provider';
...
export async function postReview(changeset, comments, deps) {
    ...
    for (const comment of postable) {
        ...
        if (resolved.ok)
            batch.push(resolved.value);
        else
            outcomes.push({ id: comment.id, sync: { state: 'pending' }, anchorStatus: MOVED });
    }
    if (batch.length === 0) {
        ...
        for (const hunk of file.hunks) {
            ...
            const flush = () => {
                if (removed > 0 || added > 0)
                    deltas.push({ start: runStart, removed, added });
                removed = 0;
                added = 0;
            };
            for (const { kind } of hunk.lines) {
...
async function fetchHeadSha(pr, deps) {
    ...
    if (!res.ok)
        throw new HttpError(`GitHub request failed (HTTP ${res.status}).`, res.status);
    const head = readHeadSha(await res.json());
    if (!head)
        throw new Error('GitHub returned an unexpected pull request payload.');
    return head;
}
...
async function createReview(pr, commitId, batch, deps) {
    ...
    if (review === undefined) {
        ...
        if (!res.ok)
            throw new HttpError(reviewPostMessage(res.status), res.status);
        review = await res.json();
    }
    const reviewId = readReviewId(review);
    const reviewUrl = readString(review, 'html_url') ?? prWebUrl(pr);
    const commentUrls = reviewId !== undefined ? await fetchReviewCommentUrls(pr, reviewId, deps).catch(() => new Map()) : new Map();
...
async function fetchReviewCommentUrls(pr, reviewId, deps) {
    ...
    if (await deps.gh.available()) {
        ...
        if (result.code === 0)
            payload = safeJson(result.stdout);
    }
...
async function ghPostJson(apiPath, body, pr, deps) {
    ...
    try {
        ...
        if (result.code !== 0)
            throw new HttpError(ghErrorMessage(result.stderr), ghStatus(result.stderr));
...
function authAwareMessage(error, pr) {
    if (error instanceof HttpError) {
        ...
        if (error.status === 401)
            return `GitHub rejected the credentials (401). ${setup}`;
        if (error.status === 403)
            return `GitHub denied the request (403) — rate limit or missing scope. ${setup}`;
        if (error.status === 404)
            return `Pull request not found, or your credentials cannot access ${pr.owner}/${pr.repo} (404). ${setup}`;
...
function reviewPostMessage(status) {
    ...
    if (status === 422)
        return 'GitHub could not anchor one of the comments (422) — the pull request may have changed. Refresh the review and try again.';
    return `Posting the review failed (HTTP ${status}).`;
}
function authSetupHint(provider) {
...
function readString(json, key) {
    if (isRecord(json) && typeof json[key] === 'string' && json[key].length > 0)
        return json[key];
    return undefined;
}
...
// a real clock.
export function defaultReviewSyncDeps() {
    return {
