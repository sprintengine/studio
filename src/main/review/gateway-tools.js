...
export function createReviewGatewayTools(backends) {
    ...
    // machine's layout.
    function resolveReviewDir(args) {
    ...
    const reviewListPending = {
        ...
        description: 'List the reviews across the projects open in this app. Each entry is addressed by {reviewId, projectRoot} and '
    ...
    const reviewGetChangeset = {
        ...
        description: 'Read the normalised change set for a review: files, per-file hunks, base/head refs and SHAs, and the source '
        ...
        handler: async (args) => {
            ...
            if (!read.ok)
                return toolError('changeset_unreadable', 'The stored change set could not be read or is not valid.');
            if (!read.changeset)
                return toolError('no_changeset', 'No change set has been ingested for this review yet.');
    ...
    const reviewGetBrief = {
        ...
        description: 'Read the current walkthrough (brief) for a review, or null when none exists yet or the stored one is unusable. '
    ...
    const reviewSubmitBrief = {
        ...
        description: 'Persist a walkthrough (brief) for a review. Pass the brief as an object (never a JSON string). It is validated '
        ...
        handler: async (args) => {
            ...
            if (!shape.ok)
                return reviewInvalid('brief_invalid', 'The brief failed schema validation.', shape.errors);
            const read = await changeSets.read(target.reviewDir);
            if (!read.ok)
                return toolError('changeset_unreadable', 'The stored change set could not be read or is not valid.');
            if (!read.changeset)
            ...
            if (!match.ok)
                return reviewInvalid('brief_mismatch', 'The brief does not match the change set.', match.errors);
            const leak = homePathLeak(shape.value, backends.homeDir());
            if (leak)
                return reviewInvalid('brief_path_leak', 'The brief contains an absolute machine path.', [leak]);
...
function redactChangeSetForExport(changeset) {
    const source = changeset.source.kind === 'branch'
