...
export async function enumerateReviews(roots: string[]): Promise<ReviewIndexEntry[]> {
...
async function readReviewEntry(workspaceRoot: string, reviewId: string): Promise<ReviewIndexEntry | null> {
  ...
  const comments: ReviewComment[] = state?.comments ?? []
...
async function readJsonValidated<T>(
  filePath: string,
  validate: (input: unknown) => { ok: true; value: T } | { ok: false; errors: string[] },
): Promise<T | null> {
  let raw: string
  try {
