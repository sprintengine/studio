...
export async function readReviewState(reviewDir: string): Promise<ReviewStateReadResult> {
  let raw: string
  try {
    raw = await readFile(join(reviewDir, STATE_FILE), 'utf-8')
  } catch (error) {
    if ((error as NodeJS.ErrnoException)?.code === 'ENOENT') return { ok: true, state: null }
...
export async function writeReviewState(reviewDir: string, state: ReviewWorkspaceState): Promise<void> {
