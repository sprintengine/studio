 ...
 */
export async function listGitHubRepos(tokenStore) {
    ...
    const repos = [];
    let nextUrl = 'https://api.github.com/user/repos?per_page=100&sort=pushed&affiliation=owner,collaborator,organization_member';
    try {
...
function toRepoSummary(record) {
    const repo = record;
    if (typeof repo.full_name !== 'string' || typeof repo.name !== 'string' || typeof repo.clone_url !== 'string') {
        return null;
    }
    const owner = repo.owner && typeof repo.owner === 'object' && typeof repo.owner.login === 'string'
        ? repo.owner.login
