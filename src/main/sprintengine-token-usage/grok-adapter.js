...
const sessionDirBySession = new Map();
const parsedBySessionDir = new Map();
...
async function findSessionDir(sessionsDir, sessionId) {
    const memoKey = `${sessionsDir} ${sessionId}`;
    const memoized = sessionDirBySession.get(memoKey);
    if (memoized && (await pathExists(path.join(memoized, 'updates.jsonl'))))
        return memoized;
    let entries;
    try {
...
function extractTotalTokens(row) {
...
async function readUpdatesTotals(updatesFile) {
    let maxTotal = 0;
    let model = null;
...
function effectiveTotalFromSignals(signals) {
...
async function readSignals(sessionDir) {
    try {
...
async function statKeyFor(files) {
    const parts = [];
...
export async function readGrokUsage(cliSessionId, homeDir, env) {
    if (!SESSION_ID_PATTERN.test(cliSessionId))
        return null;
