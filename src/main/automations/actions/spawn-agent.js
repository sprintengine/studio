...
const PERMISSION_PRESETS = ['default', 'auto_workspace', 'bypass_all'];
export function createSpawnAgentActionProvider() {
    return {
        kind: 'spawn-agent',
        configSchema: {
            type: 'object',
...
export async function runSpawnAgentAction(config, runtime) {
    ...
    const isolation = launched.worktreePath ? 'worktree' : 'workspace-checkout';
...
export function parseSpawnAgentConfig(config) {
    if (!isRecord(config))
        throw new Error('spawn-agent config must be an object.');
    const prompt = optionalString(config.prompt);
    if (!prompt)
        throw new Error('spawn-agent config requires a prompt.');
    const requiredIntegrations = config.requiredIntegrations;
    if (requiredIntegrations !== undefined
    ...
    if (permissionPreset !== undefined && !PERMISSION_PRESETS.includes(permissionPreset)) {
...
function triggerContextBlock(includeTriggerContext, triggerPayload) {
    if (includeTriggerContext !== true)
        return [];
...
function cappedTriggerJson(payload) {
    ...
    if (Buffer.byteLength(json, 'utf8') <= TRIGGER_CONTEXT_MAX_BYTES)
        return json;
