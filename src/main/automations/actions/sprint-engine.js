...
const SPRINT_ENGINE_AUTOMATION_ROLES = [
    'architect',
    'product',
    'developer',
    'frontend',
...
export function createSprintEngineRunActionProvider(frontDoors) {
    return {
        ...
        run: async (config, ctx) => {
            ...
            if (!runner.ok)
                return failedRun(runner.message);
            return {
                status: 'completed',
                summary: parsed.role
 ...
 */
export function createSprintEngineStartActionProvider(deps) {
    ...
    return {
        ...
        run: async (config, ctx) => {
            ...
            if (!created.ok)
                return failedRun(created.message);
            return {
                status: 'completed',
...
function parseSprintEngineStartConfig(config) {
    if (!isRecord(config))
        throw new Error('sprint-engine-start config must be an object.');
    const backlogItem = optionalString(config.backlogItem)?.replace(/\\/g, '/');
    if (!backlogItem)
        throw new Error('sprint-engine-start config requires a backlog item.');
    ...
    const roster = optionalString(config.roster) ?? optionalString(config.team);
    return {
        backlogItem,
        ...(roster ? { roster } : {}),
...
async function defaultResolveBaseStartPoint(workspaceRoot) {
    let upstream;
    try {
        upstream = (await execFileAsync('git', ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{upstream}'], { cwd: workspaceRoot, timeout: GIT_TIMEOUT_MS })).stdout.trim();
    }
    catch {
    ...
    try {
        ...
        throw new Error(`Could not refresh "${remote}" before starting the chained sprint (the new sprint must base on `
            + `the refreshed ${upstream}, not a stale local branch): ${detail}`);
    }
    return upstream;
}
function parseSprintEngineRunConfig(config) {
    if (!isRecord(config))
        throw new Error('sprint-engine-run config must be an object.');
    const team = optionalString(config.team);
    if (!team)
        throw new Error('sprint-engine-run config requires a team.');
...
function isSprintEngineAutomationRole(value) {
    return SPRINT_ENGINE_AUTOMATION_ROLES.includes(value);
}
