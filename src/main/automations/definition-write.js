...
import { automationProviderBlockedReason, } from './provider-registry';
...
export function createDefinitionWriteCore(deps) {
    async function notifyDefinitionsChanged(workspaceRoot) {
        try {
    ...
    async function finishWrite(workspaceRoot, value) {
    ...
    async function createDefinition(workspaceRoot, draft) {
        ...
        const prepared = prepareDefinitionForWrite(definition, deps.getTriggerProviderRegistrations(), deps.getActionProviderRegistrations(), deps.checkProviderPermission, deps.now());
        if (!prepared.ok)
            return prepared;
        const store = deps.createStore(workspaceRoot);
        const created = await store.createDefinition(prepared.value);
        if (!created.ok)
            return storeError(created.error);
        const state = await writeNextRunCache(store, prepared.value.id, prepared.value.nextRunAt);
        if (!state.ok)
            return storeError(state.error);
    ...
    return {
        ...
        async installFromCatalogue(workspaceRoot, input) {
            ...
            if (!existing.ok)
                return storeError(existing.errors[0]);
            const added = existing.values.find((definition) => definition.sourceCatalogueId === sourceCatalogueId);
            if (added)
                return { ok: true, value: { definition: added, alreadyAdded: true } };
            ...
            if (!created.ok)
                return created;
            const value = { definition: created.value, alreadyAdded: false };
        ...
        async update(workspaceRoot, automationId, patch, precondition) {
            ...
            const admitted = precondition?.(existing.value) ?? { ok: true };
            if (!admitted.ok)
                return admitted;
            const timestamp = new Date(deps.now()).toISOString();
            const updated = {
            ...
            const prepared = prepareDefinitionForWrite(updated, deps.getTriggerProviderRegistrations(), deps.getActionProviderRegistrations(), deps.checkProviderPermission, deps.now());
            if (!prepared.ok)
                return prepared;
            const written = await store.updateDefinition(prepared.value);
            if (!written.ok)
                return storeError(written.error);
            const state = await writeNextRunCache(store, prepared.value.id, prepared.value.nextRunAt);
            if (!state.ok)
                return storeError(state.error);
        ...
        async remove(workspaceRoot, automationId, precondition) {
            ...
            if (!deleted.ok)
                return storeError(deleted.error);
 ...
 * path on success.
 */
export function validateKnownWorkspaceRoot(workspaceRoot, getWorkspaceSyncSnapshot) {
    ...
    try {
        ...
        return fail('workspace_snapshot_unavailable', error instanceof Error ? error.message : 'Unable to verify workspaceRoot against the workspace snapshot.');
    }
}
function normalizeWorkspaceRoot(workspaceRoot) {
    return resolve(workspaceRoot).replace(/\\/g, '/').replace(/\/+$/u, '').toLowerCase();
}
export function buildDefinitionForCreate(draft, timestamp, createAutomationId) {
    return {
...
export function prepareDefinitionForWrite(definition, triggerProviders, actionProviders, checkProviderPermission, after) {
    ...
    const triggerBlockedReason = automationProviderBlockedReason(triggerRegistration, checkProviderPermission(triggerRegistration));
    if (triggerBlockedReason)
        return fail('provider_blocked', triggerBlockedReason);
    ...
    const actionBlockedReason = automationProviderBlockedReason(actionRegistration, checkProviderPermission(actionRegistration));
    if (actionBlockedReason)
        return fail('provider_blocked', actionBlockedReason);
    ...
    if (definition.trigger.kind === 'schedule') {
        ...
        if (!validation.ok)
            return fail('invalid_schedule', validation.error);
        ...
        if (nextRunAt === null) {
            ...
            if (scheduleCadenceCanExhaust(validation.value))
                return ok({ ...definition, nextRunAt: null });
    ...
    if (!validation.ok)
        return fail('invalid_trigger_config', validation.error);
    if (definition.status !== 'enabled') {
        return ok({ ...definition, nextRunAt: null });
    }
    if (!triggerProvider.computeNextRun)
        return ok({ ...definition, nextRunAt: null });
...
async function writeNextRunCache(store, automationId, nextRunAt, remove = false) {
 ...
 */
function catalogueDraftInput(payload, hostTimeZone) {
    if (!isRecord(payload))
        return payload;
 ...
 */
function localiseCatalogueSchedule(trigger, hostTimeZone) {
    if (!hostTimeZone || !isRecord(trigger) || trigger.kind !== SCHEDULE_TRIGGER_KIND)
        return trigger;
 ...
 */
function resolveHostTimeZone(hostTimeZone) {
    try {
...
export function parseDefinitionDraft(input) {
    if (!isRecord(input))
        return fail('invalid_input', 'Automation definition must be an object.');
    const name = trimmedString(input.name);
    if (!name)
        return fail('invalid_input', 'Automation definition name is required.');
    if (!isAutomationStatus(input.status))
        return fail('invalid_input', 'Automation definition status is invalid.');
    ...
    const runInWorktree = input.runInWorktree;
    ...
    const disableAfterRun = input.disableAfterRun;
    ...
    const effectiveDisableAfterRun = disableAfterRun === undefined
...
export function parseDefinitionPatch(input) {
    if (!isRecord(input))
        return fail('invalid_input', 'Automation update patch must be an object.');
    const patch = {};
...
export function parseKindConfig(input, label) {
...
function defaultAutomationId(name) {
    const slug = name
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9._-]+/g, '-')
