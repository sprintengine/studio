...
export async function enqueueTriggerEventRun(input) {
    ...
    try {
        ...
        if (!started.ok)
            return problem(input, storeProblem(workspaceRoot, started.error, definition.id));
        try {
...
export function clearTriggerBlockedReasonState(state, automationId) {
    if (!state.triggerBlockedReasonByAutomationId)
        return;
    delete state.triggerBlockedReasonByAutomationId[automationId];
}
export function triggerEventInFlightKey(workspaceRoot, automationId) {
    return `${workspaceRoot}\u0000${automationId}`;
}
async function completeTriggerEventRun(input, eventId, finalRun, completedAt) {
    const { definition, projectFolder, store } = input;
    const workspaceRoot = projectFolder.folderPath;
    const completed = await store.recordRun(finalRun);
    if (!completed.ok)
        return problem(input, storeProblem(workspaceRoot, completed.error, definition.id));
    ...
    if (!updated) {
        ...
        if (existingProblem)
            return { status: 'problem', problem: existingProblem };
...
async function updateDefinitionAfterTriggerRun(input, run, updatedAt, mutateState) {
...
function runRecord(input, dueAt, fields) {
    return {
...
function markTriggerEventSeen(state, automationId, eventId, seenAt) {
...
function pruneTriggerEventDedup(events) {
    const entries = Object.entries(events);
    if (entries.length <= TRIGGER_EVENT_DEDUP_RETENTION_LIMIT)
        return;
    const retainedEventIds = new Set(entries
...
function compareTriggerEventDedupEntriesNewestFirst([leftEventId, leftSeenAt], [rightEventId, rightSeenAt]) {
...
function triggerEventDedupTimestamp(value) {
...
function problem(input, value) {
    pushProblem(input, value);
    return { status: 'problem', problem: value };
}
function pushProblem(input, value) {
