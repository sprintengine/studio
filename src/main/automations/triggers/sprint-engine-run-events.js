...
import { SPRINT_ENGINE_RUN_COMPLETED_TRIGGER_KIND, SPRINT_ENGINE_RUN_NEEDS_INPUT_TRIGGER_KIND, } from '../../../shared/automations/contracts';
import { isCanceledSprintEngineRun, isCompletedSprintEngineRun, normalizeSprintEngineProjection, sprintEngineHumanInputTasks, } from '../../../shared/sprintengine/state';
 ...
 */
export function sprintEngineRunFingerprint(state) {
 ...
 */
export function createSprintEngineRunCompletedTriggerProvider(frontDoors) {
    ...
    const completedObservedAt = new Map();
    return createRunEventTriggerProvider({
        ...
        toEvents: ({ state, team, statePath, now }) => {
            ...
            if (now - observed.since < COMPLETED_CONFIRMATION_MS)
                return [];
            return [{
 ...
 */
export function createSprintEngineRunNeedsInputTriggerProvider(frontDoors) {
    return createRunEventTriggerProvider({
...
function createRunEventTriggerProvider(options) {
    return {
        kind: options.kind,
...
function validateRunEventTriggerConfig(config, kind) {
    ...
    return { ok: true, value: { kind, team, ...(label ? { label } : {}) } };
}
