...
export function createWebhookTriggerProvider() {
    return {
...
export function validateWebhookTriggerConfig(config) {
    if (!isRecord(config))
        return invalid('Webhook trigger config must be an object.');
    if (config.kind !== WEBHOOK_TRIGGER_KIND)
        return invalid('Webhook trigger kind must be "webhook".');
    ...
    if (enabled) {
        if (port.value === undefined)
            return invalid('Webhook trigger port is required when enabled is true.');
        if (!path.value)
            return invalid('Webhook trigger path is required when enabled is true.');
        if (!secret.value)
            return invalid('Webhook trigger secret is required when enabled is true.');
    }
    return {
        ok: true,
        value: {
...
export function activeWebhookTriggerConfig(config) {
    const validation = validateWebhookTriggerConfig(config);
    if (!validation.ok || validation.value.enabled !== true)
        return null;
    if (validation.value.port === undefined || !validation.value.path || !validation.value.secret)
        return null;
    return validation.value;
}
export function webhookEndpointPath(path) {
...
export function normalizeWebhookDeliveryId(value) {
...
export function webhookPayloadMatchesConfig(payload, config) {
    if (!config.eventType)
        return true;
...
function normalizeSecret(value) {
    ...
    if (/[\r\n]/u.test(secret))
        return { ok: false, error: 'Webhook trigger secret cannot contain newlines.' };
