...
export const scheduleTriggerProvider = {
    kind: 'schedule',
    configSchema: {
        type: 'object',
...
export function validateScheduleTriggerConfig(config) {
    if (!isRecord(config))
        return invalid('Schedule trigger config must be an object.');
    if (config.kind !== 'schedule')
        return invalid('Schedule trigger kind must be "schedule".');
    if (typeof config.timezone !== 'string' || config.timezone.trim() === '') {
        return invalid('Schedule trigger timezone is required.');
    }
    if (!isValidTimeZone(config.timezone))
        return invalid(`Schedule trigger timezone "${config.timezone}" is invalid.`);
    if (!isRecord(config.cadence))
        return invalid('Schedule trigger cadence must be an object.');
    switch (config.cadence.type) {
        ...
        case 'daily': {
            if (!isValidLocalTime(config.cadence.timeLocal))
                return invalid('Daily cadence timeLocal must use HH:mm.');
            return { ok: true, value: config };
        }
        case 'weekly': {
            if (!isValidLocalTime(config.cadence.timeLocal))
                return invalid('Weekly cadence timeLocal must use HH:mm.');
 ...
 */
export function scheduleCadenceCanExhaust(config) {
    return config.cadence.type === 'at';
}
export function computeNextRun(config, after) {
    const validation = validateScheduleTriggerConfig(config);
    if (!validation.ok || !Number.isFinite(after))
        return null;
    switch (config.cadence.type) {
        case 'interval':
...
// daily/weekly use.
function computeAtRun(timeZone, datetime, after) {
...
function parseAtDatetime(value) {
    ...
    if (probe.getUTCFullYear() !== target.year
...
function computeNextDailyRun(timeZone, timeLocal, after) {
...
function computeNextWeeklyRun(timeZone, timeLocal, daysOfWeek, after) {
...
function instantForLocalDateTime(target, timeZone) {
    ...
    for (let instant = start; instant <= end; instant += MINUTE_MS) {
        ...
        if (sameLocalDateTime(current, target))
            return instant;
...
function localDateTimeFromInstant(instant, timeZone) {
