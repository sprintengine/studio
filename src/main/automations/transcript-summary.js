 ...
 */
export async function readTranscriptSummary(transcriptPath) {
    if (!isAcceptableTranscriptPath(transcriptPath))
        return undefined;
    try {
    ...
    try {
        await forEachJsonlRow(transcriptPath, (row) => {
            const texts = assistantTextBlocks(row);
            if (!texts)
                return;
...
function isAcceptableTranscriptPath(transcriptPath) {
 ...
 */
function assistantTextBlocks(row) {
