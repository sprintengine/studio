...
export async function openAutomationRunPullRequest(input, deps = defaultPullRequestDeps) {
    ...
    if (!status.ok)
        return { ok: false, reason: commandReason('read git status', status) };
    ...
    if (hasWorkingDiff) {
        ...
        if (!add.ok)
            return { ok: false, reason: commandReason('stage changes', add) };
        ...
        if (!commit.ok)
            return { ok: false, reason: commandReason('commit changes', commit) };
    }
    // 2. Push the branch so a PR can reference it.
    const push = await deps.runGit(worktreePath, ['push', '-u', 'origin', branch]);
    if (!push.ok)
        return { ok: false, reason: commandReason('push the run branch', push) };
    ...
    if (!created.ok)
        return { ok: false, reason: commandReason('open a pull request', created) };
    ...
    if (view.ok && view.stdout.trim())
        return { ok: true, url: view.stdout.trim(), created: true };
    return { ok: false, reason: 'Opened a pull request but could not resolve its URL.' };
}
function commandReason(action, result) {
...
async function runGhCommand(cwd, args) {
    try {
...
export const defaultPullRequestDeps = {
