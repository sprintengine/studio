...
test('definition-write', async () => {
  ...
  function assertRetiredAutonomyFieldIsIgnoredNotRejected(): void {
    ...
    if (!result.ok) return
    assert.equal(
      Object.hasOwn(result.value, 'autonomyDefault'),
      false,
      'the retired field is not carried into the draft',
    )
    ...
    assert.equal(
      Object.hasOwn(patch.value, 'autonomyDefault'),
      false,
      'the retired field is not carried into the patch',
    )
  }
  ...
  async function assertInstallCreatesOneEnabledDefinitionWithProvenance(): Promise<void> {
    await withProjectRoots(1, async ([root]) => {
      ...
      assert.equal(
        ...
        true,
        'the definition is scheduled at install, not at the next restart',
      )
