...
export async function evaluatePollingTriggerDefinition(input) {
    ...
    try {
        ...
        await recordBlockedTriggerRun(input, error instanceof Error ? error.message : `Automation trigger "${definition.trigger.kind}" failed while polling.`);
        return;
    }
    if (!pollResult.ok) {
...
async function recordBlockedTriggerRun(input, blockedReason) {
    const { definition, projectFolder, result, state, store } = input;
    const workspaceRoot = projectFolder.folderPath;
    if (state.triggerBlockedReasonByAutomationId?.[definition.id] === blockedReason)
        return;
    ...
    try {
        ...
        if (updated)
            result.fired.push({ workspaceRoot, automationId: definition.id, runId: run.id, status: run.status });
    }
    finally {
        input.inFlight.delete(inFlightKey);
    }
}
async function clearTriggerBlockedReason(input) {
    const { definition, projectFolder, result, state, store } = input;
    if (!state.triggerBlockedReasonByAutomationId?.[definition.id])
        return;
    clearTriggerBlockedReasonState(state, definition.id);
    const written = await store.writeState(state);
    if (!written.ok)
        result.problems.push(storeProblem(projectFolder.folderPath, written.error, definition.id));
}
