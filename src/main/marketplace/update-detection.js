...
// never "up to date".
import { marketplaceUpdateAvailability } from '../../shared/marketplace';
...
export async function readMarketplaceUpdateStates(services, input = {}) {
    const receiptsRead = await readMarketplacePluginInstallReceipts(services.receiptStorePath);
    if (!receiptsRead.ok)
        return { ok: false, message: receiptsRead.message };
    ...
    // component's own id.
    const receiptOwnedModuleIds = new Set();
    for (const receipt of receiptsRead.receipts) {
        ...
        for (const component of receipt.components) {
            if (component.kind === 'module')
                receiptOwnedModuleIds.add(component.id);
        }
    }
    ...
    for (const module of discovered.modules) {
        ...
        if (claimedIds.has(id) || receiptOwnedModuleIds.has(id))
            continue;
        claimedIds.add(id);
