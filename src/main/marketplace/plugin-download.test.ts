...
test('plugin-download', async () => {
  const SOURCE_URL = 'https://github.com/sprintengine/studio-releases/tree/main/plugins/downloaded-plugin'
  const API_ROOT =
    'https://api.github.com/repos/sprintengine/studio-releases/contents/plugins/downloaded-plugin?ref=main'
  const API_MCP =
    ...
    return async (url) => {
      ...
      if (url === API_AUTOMATION) {
        return jsonResponse([
          {
            type: 'file',
            path: `plugins/downloaded-plugin/${AUTOMATION_COMPONENT_PATH}`,
            download_url: RAW_AUTOMATION,
          },
        ])
      }
      ...
      const fixture = createUnsignedFixture({
        ...
        components: {
          mcp: {
            ...
            files: [{ path: 'mcp/server.json', sha256: sha256Hex(mcpComponentSource()) }],
          },
          module: { path: 'module' },
        },
      })
  ...
  async function testNonCanonicalOwnerGithubSourceDownloads(): Promise<void> {
    await withTempDir(async (dir) => {
      ...
      const apiMcp =
        'https://api.github.com/repos/community-org/registry/contents/plugins/downloaded-plugin/mcp?ref=main'
      const fetcher: MarketplacePluginDownloadFetch = async (url) => {
        if (url === apiRoot) {
          return jsonResponse([
            { type: 'file', path: 'plugins/downloaded-plugin/plugin.json', download_url: RAW_PLUGIN },
