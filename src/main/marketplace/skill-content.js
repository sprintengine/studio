 ...
 */
export function skillContentDigest(files) {
 ...
 * `contentDigest`.
 */
export async function verifyBundledSkillFolder(folderPath, files, contentDigest) {
    if (files.length === 0)
        return { ok: false, message: 'Digest listing is empty.' };
    ...
    const walk = async (dir, relPrefix) => {
        ...
        for (const name of names) {
            ...
            if (!stats.isFile())
                return `Bundled entry ${rel} is not a regular file.`;
    ...
    if (walkProblem !== undefined)
        return { ok: false, message: walkProblem };
