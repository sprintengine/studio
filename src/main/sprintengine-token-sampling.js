...
import { appendTokenLedgerRecord, readSessionTokenUsage, } from './sprintengine-token-usage';
...
export function sprintTokenUsageDeps() {
    return {
...
function sprintTokenLedgerKey(session) {
...
const recordedSessionIdentities = new Set();
export function recordSprintSessionForTokenLedger(session) {
    const key = sprintTokenLedgerKey(session);
    if (!key)
        return;
    const identity = `${key.statePath}|${key.agentId}|${key.cli}|${key.cliSessionId}`;
    if (recordedSessionIdentities.has(identity))
        return;
...
export function sampleSprintSessionTokenUsage(session, reason) {
    const key = sprintTokenLedgerKey(session);
    if (!key)
        return Promise.resolve();
    return readSessionTokenUsage(key.cli, key.cliSessionId, sprintTokenUsageDeps())
        .then((usage) => appendTokenLedgerRecord(key.statePath, {
        kind: 'sample',
        agentId: key.agentId,
        cli: key.cli,
        cliSessionId: key.cliSessionId,
