...
test('canvas-service', async () => {
      ...
      await harness.service.commitScene(
        {
          ...ref,
          baseRevision: 0,
          elements: [element('X', { version: 5, versionNonce: 100 })],
          appState: {},
          files: {},
        },
        1,
  ...
  async function assertChangesSinceLastRead(): Promise<void> {
    ...
    assert.deepEqual(
      harness.service.changesSinceLastRead(ref, 'agent-1'),
      [],
      'the first look has nothing to compare to',
    )
  ...
  async function assertHousekeeping(): Promise<void> {
    ...
    assert.ok(
      harness.fs.files.has(`${ROOT}/diagrams/canvas.excalidraw.in-flight.tmp`),
      'a write in flight is left alone',
    )
  ...
  function assertDiskMerge(): void {
    ...
    const disk = [
      element('same', { version: 4, x: 900 }),
      element('older', { version: 5, x: 50 }),
      element('disk-only'),
    ]
