...
import { parseWorkspaceRegistryFile, serializeWorkspaceRegistryFile, workspaceRegistryContentEqual, } from '../shared/workspace-registry';
export const WORKSPACE_REGISTRY_FILE_NAME = 'workspace-registry.json';
const DEFAULT_PERSIST_DEBOUNCE_MS = 250;
export function createWorkspaceRegistryStore(deps) {
     ...
     */
    function write(file) {
        if (lastPersisted && workspaceRegistryContentEqual(lastPersisted, file))
            return;
        pendingFile = file;
        if (pendingTimer)
 ...
 */
export function createInMemoryWorkspaceRegistryStore(initial) {
    let current = initial ?? null;
    return {
        read: () => current ? { status: 'loaded', file: current, droppedRecords: [] } : { status: 'missing' },
