...
export async function importGitHubIssuesIntoWatchtower(input: {
  ...
  try {
    ...
    if (!repo) return { ok: false, provider: 'github', message: 'No GitHub origin remote was found for this repository.', unavailable: true }
...
export function githubIssueToImportItem(repo: GitHubRepoRef, issue: GitHubIssueApiRecord): SwitchboardImportItem {
...
async function fetchOpenGitHubIssues(repo: GitHubRepoRef, token: string): Promise<GitHubIssueApiRecord[]> {
  ...
  let nextUrl: string | null = `https://api.github.com/repos/${encodeURIComponent(repo.owner)}/${encodeURIComponent(repo.repo)}/issues?state=open&per_page=100`
  const records: GitHubIssueApiRecord[] = []
  for (let page = 0; nextUrl && page < 50; page += 1) {
    ...
    if (!Array.isArray(payload)) throw new Error('GitHub import returned an unexpected response shape.')
    records.push(...payload.filter(isImportableGitHubIssue))
    nextUrl = nextGitHubPageUrl(response.headers.get('link'))
  }
  if (nextUrl) throw new Error('GitHub import stopped after 50 pages to avoid an unbounded import.')
  return records
}

function isImportableGitHubIssue(record: unknown): record is GitHubIssueApiRecord {
