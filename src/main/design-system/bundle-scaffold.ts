 ...
 */
async function stampBundleInto(
  input: ScaffoldDesignSystemBundleInput & { bundleDir: string },
): Promise<DesignSystemScaffoldResult> {
...
export async function seedDesignSystemBundle(input: {
  ...
  if (!input.targetDir.trim()) return { ok: false, message: 'No folder chosen.' }
  if (!input.name.trim()) return { ok: false, message: 'A design system needs a name.' }
  ...
  try {
    if (!input.sourceDir) {
      ...
      if (!stamped.ok) throw new Error(stamped.message ?? 'scaffold failed')
      return stamped
    }
    ...
    if (!targetPreexisted) {
      for (const path of createdPaths) await rm(path, { recursive: true, force: true }).catch(() => undefined)
    }
    return {
      ok: false,
 ...
 * not have.
 */
async function restampManifest(bundleDir: string, name: string, summary: string): Promise<void> {
