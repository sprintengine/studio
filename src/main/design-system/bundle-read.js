...
import { emitTokensCss, parseTokenDocument, resolveAccentColor, resolveFontFamilies, resolveTokenValue, } from '../../shared/design-system/tokens-css';
import { countDocListItems, extractBodyHtml, extractStages, extractStyleBlocks, parseComponentDoc, sentenceCaseLabel, } from '../../shared/design-system/bundle-parse';
...
function failure(reason, path, message) {
 ...
 */
async function inlineCssUrls(css, fromDir, inliner, unresolved) {
...
async function inlineMarkupUrls(html, fromDir, inliner, unresolved) {
 ...
 */
export async function readDesignSystemBundle(bundleDir) {
    ...
    try {
        ...
        const code = codeOf(error);
        if (code === 'ENOENT') {
            return failure('no-manifest', bundleDir, `That folder has no ${DESIGN_SYSTEM_MANIFEST_FILENAME}, so it is not a design system.`);
        }
        return failure('unreadable', bundleDir, `Could not read ${DESIGN_SYSTEM_MANIFEST_FILENAME} in ${bundleDir}: ${describe(error)}`);
    }
...
async function readTokenDocument(bundleDir) {
...
function buildSpecimen(document) {
    if (!document) {
...
function buildRamp(document) {
    ...
    const walk = (node, path) => {
        ...
        if ('$value' in record) {
            ...
            if (light)
                swatches.push({ path: dotted, light, dark: dark ?? light });
            return;
        }
 ...
 */
function declaredGroups(manifest) {
    const groups = [];
...
async function readComponents(bundleDir, manifest, inliner) {
    ...
    const declared = Array.isArray(manifest.contents.components)
        ? manifest.contents.components.filter((name) => typeof name === 'string')
        : [];
...
async function readPatterns(bundleDir, manifest, inliner) {
    const dir = join(bundleDir, 'patterns');
    const declared = Array.isArray(manifest.contents.patterns)
        ? manifest.contents.patterns.filter((name) => typeof name === 'string')
        : [];
    const views = [];
    for (const name of declared) {
...
async function readGlyphs(bundleDir, manifest) {
    const dir = join(bundleDir, 'glyphs');
    const declared = Array.isArray(manifest.contents.glyphs)
        ? manifest.contents.glyphs.filter((name) => typeof name === 'string')
        : [];
    const views = [];
    for (const name of declared) {
