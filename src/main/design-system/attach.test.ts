...
// A library entry is now a REGISTERED PATH (item 2004), so the fixture points a
// registry at a bundle folder rather than copying one into a store. Registration
// itself is covered by library-registry.test.ts.
/**
 * The two paths the library needs, for a fixture root.
 *
 * `registryPath` sits beside the root exactly as it does in production, and
 * `legacyRoot` points at the root itself so the fixture's `<name>/<version>/`
 * copy is ADOPTED — which is how a pre-registry machine's systems reach the
 * library at all.
 */
function libraryPathsFor(libraryRoot: string): LibraryPaths {
  return { registryPath: `${libraryRoot}.json`, legacyRoot: libraryRoot }
}
...
run('library attach lands the full bundle at design-system/ with provenance stamped, source untouched', async () => {
  ...
  try {
    ...
    const result = await attachDesignSystemBundle(
      { kind: 'library', id: designSystemRegistrationId(join(libraryRoot, 'example', '1.2.3')) },
      workspace,
