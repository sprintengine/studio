...
async function resolveAttachSource(source, libraryPaths) {
    if (source.kind === 'library') {
        ...
        return {
            ...
            libraryCoords: read.entry.name && read.entry.version
    ...
    try {
        ...
        return {
            failure: {
                ok: false,
                stage: 'source',
                message: `The folder is not a design-system bundle (${DESIGN_SYSTEM_MANIFEST_FILENAME} is missing or invalid): ${error instanceof Error ? error.message : String(error)}`,
            },
        };
    }
 ...
 */
export async function attachDesignSystemBundle(source, workspaceRoot, libraryPaths) {
 ...
 */
export async function detachDesignSystemBundle(workspaceRoot) {
    if ((await statKind(workspaceRoot)) !== 'dir') {
    ...
    try {
        ...
        return {
            ok: false,
            message: `Could not remove ${DESIGN_SYSTEM_ATTACH_DIRNAME}/: ${error instanceof Error ? error.message : String(error)}`,
        };
    }
}
