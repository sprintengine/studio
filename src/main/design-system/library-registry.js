...
import { designSystemRegistrationId, normaliseRegistryPath, DESIGN_SYSTEM_REGISTRY_SCHEMA_VERSION, } from '../../shared/design-system/library';
...
export function defaultDesignSystemRegistryPath() {
...
export function defaultDesignSystemLibraryRoot() {
 ...
 */
export async function readDesignSystemRegistry(registryPath) {
    let raw;
    try {
 ...
 */
async function writeDesignSystemRegistry(registryPath, registry) {
...
function toEntry(registration, probed) {
 ...
 */
async function adoptLegacyCopies(legacyRoot, existing) {
    ...
    for (const name of nameDirs.sort((a, b) => a.localeCompare(b))) {
        ...
        for (const version of versionDirs.sort((a, b) => a.localeCompare(b))) {
            ...
            if (known.has(normaliseRegistryPath(path)))
                continue;
 ...
 */
async function ensureAdopted(paths) {
    const registry = await readDesignSystemRegistry(paths.registryPath);
    if (registry.adoptedLegacyCopies)
        return registry;
 ...
 */
export async function listDesignSystemLibrary(paths) {
...
export async function readDesignSystemLibraryEntry(paths, id) {
 ...
 */
export async function registerDesignSystemFolder(paths, folderPath) {
 ...
 */
export async function forgetDesignSystemFolder(paths, id) {
    const registry = await readDesignSystemRegistry(paths.registryPath);
