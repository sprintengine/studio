 ...
 */
export async function findEscapingSymlink(bundleDir: string): Promise<string | null> {
  const base = await realpath(bundleDir)
  return walkForEscapingSymlink(base, base)
}

async function walkForEscapingSymlink(base: string, dir: string): Promise<string | null> {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) {
      ...
      if (isAbsolute(linkTarget)) return bundleRelative
      try {
        const resolved = await realpath(entryPath)
        if (resolved !== base && !resolved.startsWith(base + sep)) return bundleRelative
      } catch {
        return bundleRelative
      }
