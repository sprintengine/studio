...
export function createWorkspaceSyncService(options) {
    const registry = options.registry;
    const maxReplayEvents = Math.max(1, Math.floor(options.maxReplayEvents ?? MAX_REPLAY_EVENTS));
    const now = options.now ?? Date.now;
    const resolveResumeCapabilities = options.resolveResumeCapabilities ?? (() => ({ resumeSession: false, sessionIdFromCaller: false }));
    ...
    function getEventsAfter(sequence) {
     ...
     */
    function createWorkspace(input, actor) {
        ...
        const emitted = prepared.reused
            ...
            }, prepared.windowId, actor);
        if (!emitted.ok)
            return { ok: false, reason: emitted.reason, message: emitted.message };
     ...
     * write it.
     */
    function adoptWorkspace(workspace, windowId, folderPath, actor) {
        return emit({
            type: 'workspace.created',
            payload: {
    ...
    function removeWorkspace(workspaceId, actor) {
        if (!registry.getRecord(workspaceId)) {
            return failure('unknown_workspace', `Workspace "${workspaceId}" is not in the registry.`);
        }
        return emit({ type: 'workspace.remove', payload: { workspaceId } }, registry.getState().primaryWorkspaceWindowId, actor);
    }
    /**
     ...
     */
    function updateWorkspaceFields(workspaceId, patch, actor) {
        return emit({ type: 'workspace.update_fields', payload: { workspaceId, patch, editedAt: now() } }, registry.getState().primaryWorkspaceWindowId, actor);
    }
    function updateWorkspaceAgent(workspaceId, agentId, patch, actor) {
        return emit({ type: 'workspace.update_agent', payload: { workspaceId, agentId, patch, configEditedAt: now() } }, registry.getState().primaryWorkspaceWindowId, actor);
    }
    /**
     ...
     */
    function reuseExisting(prepared, actor) {
        ...
        if (existing.folderMissing) {
            const cleared = emit({ type: 'workspace.update_fields', payload: { workspaceId: existing.id, patch: { folderMissing: false }, editedAt: now() } }, prepared.windowId, actor);
            if (!cleared.ok)
                return cleared;
        }
        if (alreadyInWindow) {
    ...
    function emit(command, sourceWindowId, actor, announce = true) {
        const event = {
        ...
        if (events.length > maxReplayEvents)
            events.splice(0, events.length - maxReplayEvents);
        if (announce) {
...
function validateCommand(input, state, sourceWindowId) {
...
function validateSetActive(payload, state, sourceWindowId) {
    const windowId = normalizeId(payload.windowId);
    if (!windowId)
        return reject('invalid_window_id', 'Active workspace commands require a window id.');
...
function validateMoveToWindow(payload, state, sourceWindowId) {
...
function validatePlacement(payload, state, sourceWindowId) {
    const windowId = normalizeId(payload.windowId);
    if (!windowId)
        return reject('invalid_window_id', 'Placement commands require a window id.');
    if (windowId !== sourceWindowId) {
...
function validateWindowClose(payload, state, sourceWindowId) {
    const windowId = normalizeId(payload.windowId);
...
function validateWorkspaceCreated(payload, state, sourceWindowId) {
...
function validateRename(payload) {
    const workspaceId = normalizeId(payload.workspaceId);
    if (!workspaceId)
        return reject('invalid_workspace_id', 'Rename commands require a workspace id.');
    ...
    if (editedAt === null)
        return reject('invalid_edited_at', 'Rename commands require a numeric editedAt stamp.');
...
function validateUpdateLayout(payload) {
    const workspaceId = normalizeId(payload.workspaceId);
    if (!workspaceId)
        return reject('invalid_workspace_id', 'Layout commands require a workspace id.');
    if (!isRecord(payload.layoutModel)) {
        return reject('invalid_layout_model', 'Layout commands require a FlexLayout model object.');
    }
    const editedAt = editedAtOf(payload.editedAt);
    if (editedAt === null)
        return reject('invalid_edited_at', 'Layout commands require a numeric editedAt stamp.');
    return {
        ok: true,
        command: {
            ...
            payload: {
                workspaceId,
                layoutModel: clone(payload.layoutModel),
                editedAt,
            },
        },
    };
...
function validateUpdateFields(payload) {
    const workspaceId = normalizeId(payload.workspaceId);
    if (!workspaceId)
        return reject('invalid_workspace_id', 'Field commands require a workspace id.');
    if (!isRecord(payload.patch))
        return reject('invalid_patch', 'Field commands require a patch object.');
    const editedAt = editedAtOf(payload.editedAt);
    if (editedAt === null)
        return reject('invalid_edited_at', 'Field commands require a numeric editedAt stamp.');
    const patch = {};
...
function validateUpdateAgent(payload) {
    const workspaceId = normalizeId(payload.workspaceId);
    ...
    return {
        ...
        command: {
            ...
            payload: {
                workspaceId,
                agentId,
                patch: payload.patch === null ? null : clone(payload.patch),
                configEditedAt,
            },
        },
    };
}
function validateRemove(payload) {
    const workspaceId = normalizeId(payload.workspaceId);
    if (!workspaceId)
        return reject('invalid_workspace_id', 'Remove commands require a workspace id.');
    return { ok: true, command: { type: 'workspace.remove', payload: { workspaceId } } };
}
function validateAssignSession(payload, state, sourceWindowId) {
    const workspaceId = normalizeId(payload.workspaceId);
    const agentId = normalizeId(payload.agentId);
...
function validateLaunchState(payload, state, sourceWindowId) {
    const workspaceId = normalizeId(payload.workspaceId);
    const agentId = normalizeId(payload.agentId);
    if (!workspaceId || !agentId) {
    ...
    for (const key of ['cliStartRequested', 'cliHasLaunched', 'cliOnboardingPromptSent', 'cliResumeAvailable']) {
...
function validateSourceWindowOwnsWorkspace(state, sourceWindowId, workspaceId) {
    if (!state.workspaces.some((workspace) => workspace.id === workspaceId)) {
        return reject('unknown_workspace', `Workspace "${workspaceId}" is not known to workspace sync.`);
    }
    const sourceWindow = state.workspaceWindows.find((windowState) => windowState.id === sourceWindowId);
...
function eventPayloadForCommand(command, state, resolveResumeCapabilities, now) {
