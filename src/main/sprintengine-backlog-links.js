...
import { buildSprintEnginePullRequestLink, buildSprintEngineRunLink, isSprintEngineChildRunLink, runRelativePathForStatePath, safeProjectRelativeRunPath, sprintEnginePullRequestLinksOf, teamSlugFromStatePath, } from '../shared/backlog/sprintengine-links';
 ...
 */
async function resolveRunRelativePath(workspaceRoot, statePath) {
...
export async function recordSprintEngineExecutionLink(input) {
    ...
    const isEpicSource = (input.childRelativePaths?.length ?? 0) > 0 || isBacklogEpicRelativePath(input.relativePath);
    const linked = await addOrUpdateBacklogLink({
        workspaceRoot: input.workspaceRoot,
        relativePath: input.relativePath,
        link: buildSprintEngineRunLink({ teamSlug, runRelativePath }),
        ...(isEpicSource ? {} : { status: 'in_progress' }),
    });
    if (!linked.ok)
        return linked;
...
export async function attachSprintEnginePullRequestLink(input) {
    ...
    if (!pullRequests.length)
        return { ok: false, message: 'A pull request link needs a URL.' };
    ...
    for (const record of store.store.items) {
        const matched = (record.links ?? []).some((link) => {
            ...
            if (link.type !== 'execution' || isSprintEngineChildRunLink(link))
                return false;
        ...
        if (!matched)
            continue;
        const existing = new Map(sprintEnginePullRequestLinksOf(record.links ?? []).map((link) => [link.id, link.target.url]));
        for (const pullRequest of pullRequests) {
            ...
            if (existing.get(link.id) === pullRequest.url)
                continue;
