...
function maybeRefreshThreadCounts(pids) {
    if (threadCountSampleInFlight)
        return;
    if (Date.now() - threadCountSampledAt < THREAD_SAMPLE_THROTTLE_MS)
        return;
...
function maybeRefreshSystemMemory() {
    if (systemMemorySampleInFlight)
        return;
    if (Date.now() - systemMemorySampledAt < SYSTEM_MEMORY_SAMPLE_THROTTLE_MS)
        return;
...
// spawns/suspends.
function maybeRefreshWorkspaceMemory() {
    if (workspaceMemorySampleInFlight)
        return;
    if (Date.now() - workspaceMemorySampledAt < WORKSPACE_MEMORY_SAMPLE_THROTTLE_MS)
        return;
...
function maybeRefreshChildProcessMetrics(pids) {
    if (childProcessMetricSampleInFlight)
        return;
    if (Date.now() - childProcessMetricSampledAt < CHILD_PROCESS_SAMPLE_THROTTLE_MS)
        return;
...
let listConversationRootsDep;
function collectAttributionRoots() {
...
export function registerDiagnosticsIpc(ipcMain, deps) {
    listConversationRootsDep = deps.listConversationRoots;
    ipcMain.handle('diagnostics:log', async (_, input) => {
    ...
    ipcMain.handle('diagnostics:get-process-metrics', () => {
        ...
        const snapshot = collectProcessMetrics(() => app.getAppMetrics(), Date.now(), { pid: process.pid, heapUsedBytes: heap.heapUsed, heapTotalBytes: heap.heapTotal }, threadCountCache, childProcessMetricCache);
