...
const AUX_WINDOW_KINDS = ['diff', 'file'];
...
function getWindowPlacement(win) {
...
export function sendWindowState(win) {
    if (win.isDestroyed())
        return;
    win.webContents.send('window:state-changed', getWindowState(win));
}
export function sendWindowPlacement(win) {
    if (win.isDestroyed())
        return;
...
function getWorkspaceWindowId(win) {
    try {
...
export function registerWindowIpc(ipcMain, options) {
    ...
    ipcMain.handle('window:confirm-close', (event) => {
        const win = getRequestWindow(event);
        if (win)
            options.confirmWindowClose(win);
    });
    ...
    ipcMain.handle('window:open-aux-window', (_event, input) => {
        ...
        if (!kind)
            return { ok: false, message: 'invalid_aux_window_kind' };
        const singletonKey = typeof input?.singletonKey === 'string' ? input.singletonKey.trim() : '';
        if (!singletonKey)
            return { ok: false, message: 'missing_singleton_key' };
    ...
    ipcMain.handle('window:create-workspace-window', (_event, input) => {
        ...
        if (!windowId)
            return { ok: false, message: 'missing_window_id' };
        try {
...
function normalizeStringParams(input) {
