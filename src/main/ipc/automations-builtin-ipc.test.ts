...
async function testBuiltinListAndAddGoThroughTheCatalogueWrite(): Promise<void> {
  ...
  if (!unknown.ok) assert.equal(unknown.code, 'not_found', 'an id this build does not ship is refused, not improvised')
  ...
  const installedTrigger = added.value.definition.trigger.config as { timezone: string; cadence: { timeLocal: string } }
