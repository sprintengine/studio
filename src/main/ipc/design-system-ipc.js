...
import { scaffoldDesignSystemBundle, seedDesignSystemBundle, } from '../design-system/bundle-scaffold';
...
import { defaultDesignSystemLibraryRoot, defaultDesignSystemRegistryPath, forgetDesignSystemFolder, listDesignSystemLibrary, readDesignSystemLibraryEntry, registerDesignSystemFolder, } from '../design-system/library-registry';
...
function parseAttachSource(value) {
...
export function registerDesignSystemIpc(ipcMain) {
    ipcMain.handle('design-system:regenerate-derived', (_event, rootDir) => {
    ...
    ipcMain.handle('design-system:scaffold-bundle', (_event, workspaceRoot, name, summary) => {
    ...
    ipcMain.handle('design-system:seed-bundle', (_event, sourceDir, targetDir, name, summary) => seedDesignSystemBundle({
    ...
    ipcMain.handle('design-system:resolve-brand-demo-seed', () => resolveDesignSystemBrandDemoSeedDir());
    ...
    ipcMain.handle('design-system:lint-bundle', (_event, bundleDir) => {
    ...
    ipcMain.handle('design-system:read-bundle', (_event, bundleDir) => readDesignSystemBundle(typeof bundleDir === 'string' ? bundleDir : ''));
    ...
    ipcMain.handle('design-system:library-read', (_event, id) => {
    ...
    ipcMain.handle('design-system:library-register', (_event, folderPath) => registerDesignSystemFolder(libraryPaths(), typeof folderPath === 'string' ? folderPath : ''));
    ipcMain.handle('design-system:library-forget', (_event, id) => forgetDesignSystemFolder(libraryPaths(), typeof id === 'string' ? id : ''));
    ...
    ipcMain.handle('design-system:attach', (_event, source, workspaceRoot) => {
    ...
    ipcMain.handle('design-system:detach', (_event, workspaceRoot) => {
        if (typeof workspaceRoot !== 'string' || workspaceRoot.trim().length === 0) {
            return Promise.resolve({ ok: false, message: 'No workspace root provided.' });
        }
