...
export function registerMemoryActivityIpc(ipcMain: IpcMain): void {
  ipcMain.handle('memory-activity:install', async (_, input: EnableInput): Promise<ActivityInstallResult> => {
  ...
  ipcMain.handle('memory-activity:uninstall', async (_, input: WorkspaceInput): Promise<ActivityUninstallResult> => {
  ...
  ipcMain.handle('memory-activity:start-watching', async (_, input: EnableInput): Promise<{ ok: true }> => {
    if (input?.workspaceRoot && input.memoryRelativeRoot) {
      await startMemoryActivityWatcher(input.workspaceRoot, input.memoryRelativeRoot)
    }
    return { ok: true }
  ...
  ipcMain.handle('memory-activity:get-status', async (_, input: WorkspaceInput): Promise<ActivityStatus> => {
  ...
  ipcMain.handle('memory-activity:is-installed', async (_, input: WorkspaceInput): Promise<boolean> => {
