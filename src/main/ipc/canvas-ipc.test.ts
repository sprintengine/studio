...
test('canvas-ipc', async () => {
  ...
  function createService(): { service: CanvasServiceInternal; recorded: Recorded[] } {
    ...
    const service = {
      ...
      openBoard: record(
        'openBoard',
        Promise.resolve(
          canvasOk({ path: 'diagrams/a.excalidraw', revision: 1, elements: [], appState: {}, files: {} }),
        ),
      ),
  ...
  async function main(): Promise<void> {
    ...
    assert.match(
      unmergeable.error.message,
      /versionNonce/,
      'the merge needs both tie-breaks, and says which is missing',
    )
