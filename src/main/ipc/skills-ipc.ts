...
export function registerSkillsIpc(ipcMain: IpcMain, service: SkillsService): void {
  ...
  ipcMain.handle('skills:add-source', (_, input: SkillAddSourceInput): Promise<SkillAddSourceResult> =>
    service.addSource(input),
  )
  ipcMain.handle('skills:add-local-source', (_, input: SkillAddLocalSourceInput): Promise<SkillAddSourceResult> =>
    service.addLocalSource(input),
  )
  ipcMain.handle('skills:remove-source', (_, input: SkillRemoveSourceInput): Promise<SkillRemoveSourceResult> =>
    service.removeSource(input),
  )
  ...
  ipcMain.handle('skills:install', (_, input: SkillInstallInput): Promise<SkillInstallOutcome> =>
    service.install(input),
  )
  ipcMain.handle('skills:uninstall', (_, input: SkillUninstallInput): Promise<SkillUninstallOutcome> =>
    service.uninstall(input),
  )
  ipcMain.handle('skills:sync-source', (_, input: SkillSyncSourceInput): Promise<SkillSyncSourceOutcome> =>
    service.syncSource(input),
  )
  ...
  ipcMain.handle('skills:install-plugin', (_, input: SkillPluginInstallInput): Promise<SkillPluginInstallOutcome> =>
    service.installPlugin(input),
  )
  ipcMain.handle(
