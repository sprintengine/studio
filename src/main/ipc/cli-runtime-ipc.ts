...
export function registerCliRuntimeIpc(ipcMain: IpcMain): void {
  ...
  ipcMain.handle('cli-runtime:install', (event, input: InstallIpcInput): Promise<CliInstallResult> => {
    const channel = `cli-runtime:install-output:${input.cli}`
    return installCli({ cli: input.cli, methodId: input.methodId }, input.runtime, (chunk) => {
