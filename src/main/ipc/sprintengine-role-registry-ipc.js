...
import { defaultUserRoleRegistryRoot, deleteUserRole, getUserRole, installRoleFolder, loadUserRoleManifests, saveUserRole, } from '../sprintengine-role-registry';
...
async function withRoleCatalogInvalidation(write) {
...
function bundledSpecialistPackDir() {
    return app.isPackaged
        ? join(process.resourcesPath, 'resources', 'specialist-pack')
        : join(app.getAppPath(), 'resources', 'specialist-pack');
}
export function registerSprintEngineRoleRegistryIpc(ipcMain) {
    ipcMain.handle('sprintengine:user-roles:install-folder', (_event, srcDir) => {
    ...
    ipcMain.handle('sprintengine:user-roles:save', (_event, input) => {
        ...
        return withRoleCatalogInvalidation(saveUserRole(input, defaultUserRoleRegistryRoot()));
    });
    ipcMain.handle('sprintengine:user-roles:delete', (_event, id) => {
        if (typeof id !== 'string')
            return Promise.resolve({ ok: false });
        return withRoleCatalogInvalidation(deleteUserRole(id, defaultUserRoleRegistryRoot()));
    });
    ipcMain.handle('sprintengine:user-roles:get', (_event, id) => {
        if (typeof id !== 'string')
            return Promise.resolve({ ok: false });
