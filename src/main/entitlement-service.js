...
export class EntitlementService {
    ...
    async refreshFeature(featureKey) {
    ...
    async checkAccess(request) {
        ...
        if (cacheStatus === 'expired') {
            return denied(request.featureKey, value, 'expired', 'Multicode premium access needs a fresh entitlement check.', limit);
        }
        ...
        if (cacheStatus === 'offline_grace' && request.hostedCost) {
            return denied(request.featureKey, value, 'offline_grace', 'This premium action needs online entitlement verification.', limit, graceExpiresAt);
        }
        ...
        return denied(request.featureKey, value, 'missing', 'Upgrade this organization or switch to one with Multicode premium access.', limit);
    }
...
export function isEntitlementSnapshotFresh(snapshot) {
...
export function entitlementGraceExpiresAt(cache, graceMs = ENTITLEMENT_GRACE_MS) {
    const snapshotExpiresAt = Date.parse(cache.snapshot.expiresAt);
    if (!Number.isFinite(snapshotExpiresAt))
        return null;
...
export function isEntitlementCacheTooStale(cache, maxCacheAgeMs = ENTITLEMENT_MAX_CACHE_AGE_MS) {
...
export function entitlementCacheStatus(cache, graceMs = ENTITLEMENT_GRACE_MS, maxCacheAgeMs = ENTITLEMENT_MAX_CACHE_AGE_MS) {
    ...
    if (isEntitlementCacheTooStale(cache, maxCacheAgeMs))
        return 'expired';
    if (isEntitlementSnapshotFresh(cache.snapshot))
        return 'fresh';
...
export function offlineGraceMessage(graceExpiresAt) {
...
export function isEntitlementSnapshot(input, product) {
...
// file exists to stop.
function entitlementValue(snapshot, featureKey) {
    if (featureKey in snapshot.features)
        return snapshot.features[featureKey];
    if (featureKey in snapshot.limits)
        return snapshot.limits[featureKey];
    return undefined;
}
function allowed(featureKey, value, status, graceExpiresAt, limit) {
    return {
        allowed: true,
        featureKey,
        value,
...
function denied(featureKey, value, status, message, limit, graceExpiresAt) {
    return {
        allowed: false,
        featureKey,
        value,
