...
import { sprintEngineAutomationShouldRun, transitionSprintEngineAutomation, } from '../shared/sprintengine/automation-lifecycle';
import { buildSprintEngineAgentRosterForState, isCanceledSprintEngineRun, isCompletedSprintEngineRun, normalizeSprintEngineProjection, resolveSprintEngineAgentRuntime, } from '../shared/sprintengine/state';
import { createSprintEngineAutoRunCycleState, reconcileWorkspaceSessions, superviseWorkspace, } from '../shared/sprintengine/auto-run-cycle';
import { setSprintEngineAutoRunPerfLogger } from '../shared/sprintengine/auto-run';
import { samePath } from '../shared/paths';
import { agentCliSupportsConversationResume, agentCliUsesStableSessionIdForResume, resumeCapabilitiesForCli, } from '../shared/agent-cli-resume';
...
function synthesizeAgent(id, name) {
    return {
        id,
        name,
        status: 'idle',
 ...
 */
function reconcileViewAgents(view, state) {
    if (!state)
        return;
    const next = {};
    ...
    for (const [agentId, agent] of Object.entries(view.agents)) {
        if (!next[agentId] && (agent.cliStartRequested || agent.cliSessionId))
            next[agentId] = agent;
    }
    view.agents = next;
}
export function createSprintRuntime(deps) {
    ...
    function applyAutomationEventToView(entry, event) {
        entry.view.sprintEngineAutoState = transitionSprintEngineAutomation(currentAutoState(entry), event, now());
        reconcilePower(entry);
    }
    function currentAutoState(entry) {
     ...
     * renderer normalizer.
     */
    function adoptDesiredMode(target, record) {
        const current = currentAutoState(target);
        if (current.desiredMode === record.desiredMode)
            return;
        ...
        if (sprintEngineAutomationShouldRun(target.view.sprintEngineAutoState))
            void tick();
    }
    function persistResidue(entry) {
    ...
    function automationEventForStopReason(reason, context) {
        switch (reason) {
    ...
    function buildPorts() {
        return {
            ...
            ...(deps.terminal.sendPrompt
                ? { terminalSendPrompt: (sessionId, text) => deps.terminal.sendPrompt(sessionId, text) }
                : {}),
            ...
            autoApproveSprintEngineArtifact: (statePath, artifactId) => deps.artifacts.autoApproveArtifact({ statePath, artifactId }),
            memoryResolveRoot: ({ workspaceRoot, relativeRoot }) => deps.resolveMemoryRoot(workspaceRoot, relativeRoot),
            ...
            isAgentTabVisible: (workspaceId, agentId) => {
                ...
                return deps.terminal.list().some((session) => session.kind === 'agent'
            ...
            updateAgent: (workspaceId, agentId, update) => {
                ...
                const touchesConfig = 'cliRuntimeOverride' in update || 'name' in update || 'cliStartupPrompt' in update;
            ...
            applyAutomationStopReason: (workspaceId, reason, context) => {
                ...
                if (!target)
                    return;
                const event = automationEventForStopReason(reason, context);
                if (event)
                    applyAutomationEventToView(target, event);
                deps.broadcastOp({
                    kind: 'stop_reason',
                    statePath: target.statePath,
                    reason: reason,
            ...
            // Projection refresh ---------------------------------------------------
            refreshWorkspaceProjection: async ({ workspace, tokens, force }) => {
                ...
                if (result.unchanged)
                    return { status: 'unchanged', state: target.view.sprintEngineState ?? null };
                if (result.token)
                    tokens.set(workspace.id, result.token);
                const normalized = normalizeSprintEngineProjection(result.data, target.workspaceName);
                if (!normalized)
                    return { status: 'error', message: 'Projection could not be normalized.' };
                ...
                const autoState = currentAutoState(target);
                if (autoState.completionTeardownAt !== undefined
            ...
            tearDownDepartedTaskScopedWorker: async (workspaceId, agentId, preloadedSessions) => {
                const target = entryForWorkspaceId(workspaceId);
                if (!target)
                    return { recorded: false, removedAgent: false, removedTab: false, closedSessionId: null };
     ...
     * main's copy alone.
     */
    function mergeAgentConfigs(entry, agentConfigsInput) {
        ...
        let changed = false;
        const applyConfig = (agent, config) => ({
            ...agent,
        ...
        for (const [agentId, config] of Object.entries(agentConfigs)) {
            ...
            if (agent) {
                ...
                if (merged.cliRuntimeOverride !== agent.cliRuntimeOverride
     ...
     */
    function syncAgentSessionIdentity(entry) {
        for (const session of deps.terminal.list()) {
            if (session.kind !== 'agent' || session.sprintEngineStatePath !== entry.statePath)
                continue;
            const agentId = session.agentId;
            if (!agentId)
                continue;
            const agent = entry.view.agents[agentId];
            if (!agent || agent.cliSessionId !== session.sessionId)
                continue;
     ...
     */
    function performTerminalTeardown(target) {
        if (currentAutoState(target).completionTeardownAt !== undefined)
            return;
        const at = now();
        target.view.sprintEngineAutoState = {
        ...
        const agentIdsToRetire = new Set();
        for (const session of sessions) {
            if ((session.processAlive || session.suspended)
                && session.kind === 'agent'
        ...
        for (const [agentId, agent] of Object.entries(target.view.agents)) {
            if (agent.cli && agent.cliSessionId)
                agentIdsToRetire.add(agentId);
        }
     ...
     */
    function enterTerminalDormancy(target, opts) {
        ...
        const targetRuntimeState = canceled ? 'canceled' : 'complete';
        if (currentAutoState(target).runtimeState !== targetRuntimeState) {
            applyAutomationEventToView(target, canceled ? { type: 'runner_canceled' } : { type: 'runner_complete' });
            if (!canceled) {
                deps.broadcastOp({
                    kind: 'stop_reason',
                    statePath: target.statePath,
     ...
     */
    function retireWorkerSession(target, agentId, sessions) {
        const agent = target.view.agents[agentId];
        ...
        const liveSession = sessions.find((session) => (session.processAlive || session.suspended)
            && session.kind === 'agent'
            && ((samePath(session.sprintEngineStatePath ?? null, target.statePath)
    ...
    function rekeyInFlightSpawns(entry, fromWorkspaceId, toWorkspaceId) {
        const inFlight = entry.refs.inFlightSpawns.current;
        for (const key of [...inFlight]) {
            if (!key.startsWith(`${fromWorkspaceId}:`))
                continue;
            inFlight.delete(key);
            ...
            for (const entry of [...runsByStatePath.values()]) {
                ...
                const entryWork = async () => {
                    ...
                    if (!spawnDelayElapsed)
                        return;
                    await superviseWorkspace(ports, entry.cycleState, entry.view, deps.getLaunchSettings().cliRuntimes, deps.getLaunchSettings().mcp, entry.refs.inFlightSpawns, entry.refs.sentArtifactApprovalMessages, entry.refs.autoApprovalDiagnostics, entry.refs.sentContinuationMessages, entry.refs.sentDispatchMessages, entry.refs.sentArchitectTriageMessages, entry.refs.sentAgentNotificationEvents, entry.refs.projectionTokensByWorkspace, entry.refs.idleClockByAgent, entry.refs.retirementCooldownByAgent);
                };
                ...
                try {
                    ...
                    await Promise.race([
                        ...
                        new Promise((_, reject) => {
                            setTimeout(() => reject(new Error(`Scheduler tick exceeded ${SPRINT_RUNTIME_TICK_WATCHDOG_MS}ms`)), SPRINT_RUNTIME_TICK_WATCHDOG_MS).unref?.();
                        }),
                    ]);
                }
                catch (error) {
    ...
    return {
         ...
         */
        resolveRunnerLogTarget(workspaceId) {
            for (const entry of runsByStatePath.values()) {
         ...
         */
        adoptAutomationRecord(statePath, record) {
            const entry = runsByStatePath.get(statePath);
            if (!entry)
                return;
         ...
         */
        resumeIfBlocked(statePath) {
            const entry = runsByStatePath.get(statePath);
            if (!entry)
                return;
            if (currentAutoState(entry).runtimeState !== 'blocked')
                return;
            applyAutomationEventToView(entry, { type: 'runner_started' });
         ...
         */
        notifyAutomationChanged(statePath, record) {
            const entry = runsByStatePath.get(statePath);
            if (!entry)
                return;
