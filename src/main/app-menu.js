...
const menuAcceleratorOverrides = new Map();
...
function zoomFocusedWindowIn(win) {
    if (!win || win.isDestroyed())
        return;
    const browserWindow = BrowserWindow.fromId(win.id);
    if (!browserWindow || browserWindow.isDestroyed())
...
function menuAccelerator(commandId, fallback) {
    if (menuAcceleratorOverrides.has(commandId)) {
        return menuAcceleratorOverrides.get(commandId) ?? undefined;
    }
    return fallback;
}
function sanitizeAcceleratorUpdate(input) {
    if (!input || typeof input !== 'object')
        return null;
    const record = input;
    if (typeof record.commandId !== 'string')
        return null;
    if (!MENU_ACCELERATOR_COMMAND_IDS.has(record.commandId))
        return null;
    if (record.accelerator !== null && typeof record.accelerator !== 'string')
        return null;
    if (typeof record.accelerator === 'string' && record.accelerator.length > 80)
        return null;
    return {
...
export function registerAppMenuIpc(ipcMain) {
    ipcMain.handle('app-menu:update-accelerators', (_event, input) => {
                    ...
                    ? [
                        {
                            ...
                            click: (_, win) => sendMenuCommand(win ?? BrowserWindow.getFocusedWindow(), 'diagnostics.open'),
                        },
                        { type: 'separator' },
                    ]
                    : []),
