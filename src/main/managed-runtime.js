...
function venvRelativePath(platform) {
    return platform === 'win32'
 ...
 */
function runtimeResourceRoots(env) {
    const roots = [];
    if (env.resourcesPath)
        roots.push(env.resourcesPath);
    if (!env.isPackaged)
        roots.push(join(env.cwd, 'resources'));
    return roots;
}
/** Absolute path to the bundled CPython interpreter, or null if not vendored. */
export function bundledPythonPath(env) {
...
export function bundledNpmCliPath(env) {
    for (const root of runtimeResourceRoots(env)) {
 ...
 */
export function resolveManagedPython(env, repoRoot) {
 ...
 */
export function reportManagedPythonResolution(resolved, env, logger = console) {
    ...
    if (env.isPackaged && resolved.source !== 'bundled' && resolved.source !== 'override') {
        logger.warn(`${summary} — expected the bundled CPython in a packaged build. ` +
 ...
 */
export function managedNodeBinary(env) {
    return env.execPath;
}
/**
 ...
 * source env.
 */
export function managedNodeEnv(base = {}) {
 ...
 * left in place.
 */
export function withManagedRuntimePath(env, shimDir, platform) {
...
export function currentRuntimeEnv(overrides = {}) {
...
export function getManagedPython(repoRoot) {
 ...
 */
export function managedPythonSpawnEnv(base, source) {
    if (source !== 'bundled')
        return base;
    const next = { ...base };
...
export function getManagedNpmPrefixDir(platform = process.platform) {
...
export function getManagedRuntimeShimDir(platform = process.platform) {
    if (platform === 'win32') {
 ...
 */
export function ensureManagedRuntimeShims(env = currentRuntimeEnv()) {
    ...
    try {
        ...
        if (env.platform === 'win32') {
            writeFileSync(join(shimDir, 'node.cmd'), ['@echo off', 'set ELECTRON_RUN_AS_NODE=1', `"${node}" %*`, ''].join('\r\n'), 'utf8');
            ...
            writeFileSync(nodeShim, ['#!/usr/bin/env bash', `exec env ELECTRON_RUN_AS_NODE=1 ${shellQuote(node)} "$@"`, ''].join('\n'), { encoding: 'utf8', mode: 0o755 });
