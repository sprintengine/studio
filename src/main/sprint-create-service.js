import { DEFAULT_SPRINT_ENGINE_ROLE_CLI_DEFAULTS, DEFAULT_SPRINT_ENGINE_ROLE_COUNTS, NO_ROLES_ROSTER_ID, activeSprintEngineRoleIds, findSavedSprintEngineRoster, isNoRolesRosterRef, resolveInitialSprintEngineRoster, sprintEngineLaunchRoleCounts, } from '../shared/sprintengine/saved-rosters';
import { SPRINT_ENGINE_ROLELESS_KEY } from '../shared/sprintengine/state';
import { deriveSprintEngineAutomationDesiredMode, normalizeSprintEngineAutoState, sprintEngineAutomationInitialStateForMode, sprintEngineAutomationModeForRunOptions, } from '../shared/sprintengine/automation-lifecycle';
import { SPRINT_ENGINE_DEFAULT_MAX_PARALLEL_AGENTS, SprintEngineNewTeamCreationError, clampSprintEngineMaxParallelAgents, runSprintEngineNewTeamCreation, } from '../shared/sprintengine/new-team-creation';
...
import { scanBacklog } from '../shared/backlog/scan';
...
export function createSprintCreateService(deps) {
    // -------------------------------------------------------------------------
     ...
     */
    function resolveRequestedRoster(request) {
        ...
        if (explicit && Object.keys(explicit).length > 0) {
            ...
            const roleCounts = Object.fromEntries(Object.keys(DEFAULT_SPRINT_ENGINE_ROLE_COUNTS).map((role) => [role, 0]));
            ...
            const roleCliDefaults = { ...DEFAULT_SPRINT_ENGINE_ROLE_CLI_DEFAULTS };
            for (const role of Object.keys(roleCounts)) {
                if (!roleCliDefaults[role])
                    roleCliDefaults[role] = 'claude-code';
            }
            return {
                ok: true,
                roster: {
    ...
    function builtInRoster(savedRosters, explicitRosterRef) {
     ...
     */
    function resolveRequestedRuntime(request, roster, launchRoleCounts) {
        ...
        const requestedClis = [request.runtime?.cli, ...Object.values(request.roleClis ?? {})]
            .filter((cli) => Boolean(cli));
        if (requestedClis.length > 0) {
            const unknown = requestedClis.find((cli) => !launchable.has(cli));
            if (unknown) {
                return {
                    ok: false,
                    response: failure('sprint_unknown_runtime', `Agent CLI "${unknown}" is not installed, not known, or cannot report agent status, so this sprint cannot start on it. Install a hook-capable CLI, or pick another agent.`),
                };
            }
        }
        const roleCliDefaults = { ...roster.roleCliDefaults };
        const roleModelOverrides = { ...roster.roleModelOverrides };
        const roleReasoningOverrides = {};
        ...
        for (const [field, map] of [
            ...
            for (const role of Object.keys(map ?? {})) {
                if (addressable.includes(role))
                    continue;
                return {
                    ok: false,
                    response: failure('sprint_unknown_role', staffed.length > 0
        ...
        for (const role of addressable) {
            if (request.runtime?.cli)
                roleCliDefaults[role] = request.runtime.cli;
            if (request.runtime?.model)
                roleModelOverrides[role] = request.runtime.model;
            if (request.runtime?.effort)
                roleReasoningOverrides[role] = request.runtime.effort;
        }
        ...
        const unlaunchable = Object.entries(roleCliDefaults).find(([, cli]) => typeof cli === 'string' && cli.length > 0 && !launchable.has(cli));
        if (unlaunchable) {
            return {
                ok: false,
                response: failure('sprint_unknown_runtime', `The saved roster pins role "${unlaunchable[0]}" to agent CLI "${unlaunchable[1]}", which is not installed, not known, or cannot report agent status. Update the roster, or pick another agent.`),
            };
        }
     ...
     */
    function requestedMaxConcurrentAgents(request) {
    ...
    function workspacePort() {
        const port = {
            ...
            addWorkspace(registration) {
                ...
                const adopted = deps.adoptWorkspace(workspace, registration.windowId?.trim() || deps.primaryWorkspaceWindowId(), registration.folderPath);
                if (!adopted.ok) {
     ...
     */
    async function startRun(workspace, statePath) {
        ...
        if (hydrated.ok) {
            ...
            deps.logDiagnostic?.({
                ...
                message: `The sprint "${workspace.name}" was created, but its automation mode could not be written, so the `
     ...
     */
    async function buildEpicSourcePlan(folderPath, sourceRelativePath) {
        if (!isBacklogEpicPath(sourceRelativePath))
            return null;
        try {
            ...
            if (!plan || plan.sourcePlanKind !== 'epic')
                return null;
            return {
     ...
     */
    function bundleChildLinks(bundle, scannedItems) {
     ...
     */
    async function buildSelectionSourcePlan(folderPath, refs) {
        let scanned;
        try {
    ...
    // -------------------------------------------------------------------------
    async function createGoalSourcedSprint(request) {
        ...
        try {
            ...
            if (error instanceof SprintEngineNewTeamCreationError) {
                return failure(`sprint_${error.code.replace(/-/g, '_')}`, error.message === error.code ? `Sprint run creation failed: ${error.code}.` : error.message);
            }
            throw error;
        }
     ...
     */
    async function createPlanSourcedSprint(request, sourceRelativePath, selectionRefs) {
        ...
        if (selectionRefs) {
            ...
            if (!selection.ok)
                return failure(selection.code, selection.message);
        }
        let sourceContent;
        if (selection?.ok) {
            ...
            try {
                ...
                return failure('sprint_source_unreadable', messageOf(error, `Sprint source "${normalizedSourcePath}" could not be read.`));
            }
        }
        ...
        if (!launch.ok)
            return failure(launch.code, launch.message);
        const { result } = launch;
        ...
        if (normalizedSourcePath.startsWith('backlog/')) {
            ...
            try {
                await deps.addBacklogLink({
                    ...
                    ...(isBacklogEpicPath(normalizedSourcePath) ? {} : { status: 'in_progress' }),
                });
        ...
        if (port.record)
            await startRun(port.record, result.sprintEngineContext.statePath);
