...
// real asset set on disk against APP_THEMES: every concrete (resolved) theme must
// ship both a workspace and a chat plate, and there must be no orphan plates for
// ids that aren't themes. This is what guarantees backdropFor never silently
// returns null for a live theme.
...
const files = new Set(readdirSync(backdropsDir).filter((name) => name.endsWith('.jpg')))

const resolvedThemes = APP_THEMES
...
for (const theme of resolvedThemes) {
  for (const surface of SURFACES) {
    ...
    assert.ok(files.has(file), `missing backdrop plate: ${file}`)
  }
}
...
assert.deepEqual(orphans, [], `orphan backdrop plates not mapped to any theme: ${orphans.join(', ')}`)

assert.equal(files.size, resolvedThemes.length * SURFACES.length)

console.log(`backdropRegistry: ${resolvedThemes.length} themes × ${SURFACES.length} plates verified`)
