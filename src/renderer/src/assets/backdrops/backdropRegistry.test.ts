...
const standardFiles = new Set(
  readdirSync(backdropsDir).filter((name) => name.endsWith(".jpg")),
);
const fourKFiles = new Set(
  readdirSync(fourKDir).filter((name) => name.endsWith(".jpg")),
);
...
for (const theme of resolvedThemes) {
  for (const surface of SURFACES) {
    ...
    assert.ok(
      standardFiles.has(file),
      `missing standard backdrop plate: ${file}`,
    );
...
const expected = new Set(
  resolvedThemes.flatMap((theme) =>
    SURFACES.map((surface) => `backdrop-${theme}-${surface}.jpg`),
  ),
);
const standardOrphans = [...standardFiles].filter(
...
assert.deepEqual(
  fourKOrphans,
  [],
  `orphan 4K backdrop plates not mapped to any theme: ${fourKOrphans.join(", ")}`,
);
...
console.log(
  `backdropRegistry: ${resolvedThemes.length} themes × ${SURFACES.length} plates × 2 resolutions verified`,
);
