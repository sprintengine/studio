...
export function resolveAgentBacklogLink(
  ...
  // Resolve by the live location of the agent, not the (possibly stale) workspace
  // id in the link — the agent may have been moved to another workspace.
  if (!findWorkspaceIdForAgent(input.workspaces, parsed.agentId)) {
...
export type AgentBacklogLinkOpenPorts = {
  // Activate the agent's *current* workspace and focus (or add) its terminal
  // tab, resolving the workspace live by agent id. Returns false when the agent
  // is not open anywhere so the caller can surface a diagnostic. The port owns
  // the workspace lookup and the mounted-model vs persisted-layout fallback.
  focusAgent(input: {
    agentId: string
    agentName: string
