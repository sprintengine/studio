...
// Strip-less navigational rail components. Files / Git / Backlog / Sprint
// Engines are single-instance navigational surfaces, so they share ONE
// left-docked pane whose FlexLayout tab strip is hidden and the PanelRail
// buttons act as exclusive switches into it — exactly one shows at a time.
// Knowledge Graph keeps the same nav-pane semantics but is reached through the
// command palette / View menu rather than a rail glyph. The Editor is
