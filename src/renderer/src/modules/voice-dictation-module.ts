...
const VoiceDictationTopBarItem = React.lazy(
  () => import('./voice-dictation/VoiceDictationTopBarItem')
)
const VoiceDictationSettingsSection = React.lazy(
  () => import('./voice-dictation/VoiceDictationSettingsSection')
)
...
function VoiceDictationSettingsIcon({ className }: { className?: string }) {
  return React.createElement(
    ...
    React.createElement('rect', {
      x: 9.5, y: 3, width: 5, height: 10, rx: 2.5,
...
export const voiceDictationRendererModule: RendererModule = {
  ...
  registerRenderer(host) {
    ...
    host.registerCommand({
      ...
      run: () =>
        import('./voice-dictation/voiceDictationController').then(
          ({ voiceDictationController }) => voiceDictationController.toggle()
        ),
    })
