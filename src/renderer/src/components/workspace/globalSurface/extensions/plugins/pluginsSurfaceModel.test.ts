...
test('pluginsSurfaceModel', async () => {
  ...
  run('installed state comes from our receipt, or from a hand-enabled Claude key', () => {
    ...
    assert.equal(
      derivePluginInstallState([foreign], SOURCE.id, p, 'claude-plugins-official', 'ffffff').kind,
      'installed',
    )
  ...
  run('what an install did is one line in the installer’s words', () => {
    ...
    assert.equal(
      summarizePluginInstall({ harnesses: [], mcpServers: [], warnings: ['x failed'] }),
      'Installed. x failed',
    )
  })
  ...
  run('a version bump of an installed skill pack is an available update, never applied silently', () => {
    ...
    assert.equal(
      derivePluginInstallState([installed], studio.id, plugin('studio-skills'), 'sprintengine-studio', 'oldcommit')
        .kind,
      'installed',
