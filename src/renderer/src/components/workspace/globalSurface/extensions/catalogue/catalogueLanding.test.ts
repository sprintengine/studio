...
test('catalogueLanding', async () => {
  ...
  run('a plugin the scan does not hold, or a scan that failed, is said in the source’s name', () => {
    ...
    assert.deepEqual(failed, {
      status: 'missed',
      notice: 'acme/hub could not be read, so read-one could not be opened.',
    })
  })
  ...
  run('with no source named, the plugin is looked for in the scans already in hand and no read is started', () => {
    ...
    assert.equal(
      resolve({ sourceId: '', itemId: null }, {}).status,
      'missed',
      'a link that names nothing opens nothing',
    )
  })
