...
// expect the skip affordance to disappear until it is answered.
//
// The historic 'mode' pivot step is gone: the hub's rail IS the type choice, and
// picking a type is never a step. The sprint flow is gone too (MC-2062): sprint
// creation is the New sprint dialog, so no flow here carries a sprint step.
export type StepId =
  | 'workspace'
  | 'mcp-servers'
  | 'knowledge'
