...
//
// The row list matches the "Final sweeps" panel — the wizard's other "pick zero or
// more, default none" control — so the two cannot drift: a bordered list that
// scrolls inside itself, one toggle row per option, selection carried by the
// accent rail rather than a badge.
function AlsoChangesProjectsPanel({
  projects,
  selectedProjectIds,
  onToggleProject,
  disabled,
...
export function RosterAndRunSettings({
  ...
  // "Also changes these projects" (MC-1613): the sibling projects on disk this run
  // may also change, none selected by default. Omitted by surfaces without run
  // options, and by a workspace with no sibling project to offer.
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="overflow-hidden rounded-md border border-[color:var(--border-default)] bg-[color:var(--bg-surface)]">
            ...
            <AlsoChangesProjectsPanel
              ...
              useWorktrees={useWorktrees ?? false}
            />
          </div>
        </div>
  ) : null
