...
async function main(): Promise<void> {
  ...
  // area rather than shrinking its target. The chevron glyph stays icon.size.xs.
  {
    ...
    const chevron = host.querySelector('button[aria-expanded]') as HTMLButtonElement
    assert.match(chevron.className, /size-\[var\(--hit-target-min\)\]/, 'the chevron button fills the hit-target floor')
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    const { host, root, render } = mount(
      <ProviderRow {...BASE} name="Muse Code" recessed actions={install} stateLine="Not installed — no muse on PATH" />,
    )
  ...
  const onboarding = readFileSync(join(repoRoot, 'src/renderer/src/components/onboarding/FirstRunCliCard.tsx'), 'utf8')
