...
async function main(): Promise<void> {
  ...
  run('a macOS desktop is named like one', () => {
    for (const hostName of ['mac-mini', 'IMAC-OFFICE', 'Mac-Studio-01', 'mac-pro']) {
      assert.equal(
  ...
  run('windows and linux take the monitor, and "darwin" does not fall into "win"', () => {
    ...
    assert.equal(deviceGlyphFor({ os: 'Windows', hostName: 'BUILD-BOX' }), DeviceDesktopGlyph)
    assert.equal(deviceGlyphFor({ os: 'linux', hostName: 'build-box' }), DeviceDesktopGlyph)
    assert.notEqual(
      deviceGlyphFor({ os: 'darwin', hostName: 'mac-mini' }),
      DeviceDesktopGlyph,
  ...
  run('phones take the phone', () => {
    ...
    assert.equal(deviceGlyphFor({ os: 'iOS', hostName: 'conal-iphone' }), DevicePhoneGlyph)
  })
  ...
  run('an unknown machine still gets a mark, never an empty slot', () => {
    ...
    assert.equal(
      deviceGlyphFor({ os: 'macOS', hostName: 'dev-machine' }),
      RemoteMachineGlyph,
  ...
  run('it returns the COMPONENT, so the caller keeps size and ink', () => {
    const Glyph = deviceGlyphFor({ os: 'macOS', hostName: 'mac-mini' })
