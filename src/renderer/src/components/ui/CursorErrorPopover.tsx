...
/**
 * A small red error box anchored just above the pointer that raised it. Used
 * for transient "this click failed" feedback — a missing file link, a rejected
 * drop — where a corner toast is too heavy and too far from the user's eye.
 *
 * Collapsed it shows one clipped line and self-dismisses; clicking expands it to
 * the full message with copy, and hovering reveals copy without committing to
 * the expand. It carries no status dot — failure is conveyed by the red frame.
 */
export function CursorErrorPopover({
  ...
  // the cursor so the box grows away from the pointer instead of drifting.
  useLayoutEffect(() => {
    ...
    const compute = () => {
      ...
      if (placeAbove) {
        setPlacement({ left, bottom: vh - (anchor.y - CURSOR_GAP), maxHeight: spaceAbove })
      } else {
        setPlacement({ left, top: anchor.y + CURSOR_GAP, maxHeight: spaceBelow })
      }
    }
    compute()
  ...
  // Escape dismisses from anywhere while the box is open.
  useEffect(() => {
  ...
  return createPortal(
    <div
      ...
      style={{
        ...
        maxHeight: expanded ? expandedMaxHeight : COLLAPSED_MAX_HEIGHT,
        // Hidden until measured so the first paint never flashes at 0,0.
        visibility: placement ? 'visible' : 'hidden',
      }}
      className={[
        'cursor-error-popover z-50 chip-bg-error chip-border-error',
        ...
        expanded ? 'cursor-default overflow-y-auto' : 'cursor-pointer overflow-hidden',
      ].join(' ')}
    >
