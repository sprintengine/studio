...
test('AutomationEditor', async () => {
  ...
  assert.deepEqual(
    cronResolved,
    cronTrigger,
    'a loaded cron schedule round-trips verbatim, never converted to interval',
  )
  ...
  assert.deepEqual(
    editedSchedule.config,
    {
      ...
      cadence: { type: 'weekly', timeLocal: '08:30', daysOfWeek: [1, 3] },
    },
  ...
  assert.equal(
    resolveAutomationRuntimeCli(undefined, undefined, []),
    'claude-code',
    'and the picker is never left empty',
  )
