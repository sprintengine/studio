...
const agentRuntimeSource = readFileSync(join(process.cwd(), 'src/renderer/src/modules/agent-runtime-module.ts'), 'utf8')
...
run(
  'the detail header shows Hand to agent as its own primary and no module action as a button — modules cannot compete for the header',
  () => {
...
run('with four execute module actions the More menu and the row menu list them in order then label', () => {
  ...
  assert.deepEqual(menuLabels(fourExecuteActions), [
    'Publish notes',
    'Replay session',
    'Chart this item',
    'Forecast weather',
...
run('a module action offers itself in both menus and nowhere else as a button', () => {
  ...
  const item = overflow.find(
    (entry) => entry.kind !== 'separator' && 'label' in entry && entry.label === 'Chart this item',
  )
  ...
  assert.equal(handToAgentSource.includes('Chart this item'), false, 'and neither do the shell header controls')
})
