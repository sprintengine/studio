...
export function PeerPicker({ scan, scanning, connections, reachability, now, onScan, onConnect, 
/** Whether this machine's own listener is running, so the reverse offer can be made. */
canOfferReverse, busy, 
/** Set when a request is already waiting; Connect is dead meanwhile. */
waitingOn, intro, }) {
    const view = React.useMemo(() => peerPickerView({ scan, scanning, connections, reachability, now }), [scan, scanning, connections, reachability, now]);
    ...
    const reverseScopes = React.useMemo(() => PAIR_SCOPE_ROWS.filter((row) => row.defaultOn).flatMap((row) => row.scopes), []);
          ...
          {view.rows.map((row) => (<div key={row.peer.id} className="flex items-center gap-2 py-2 first:pt-0 last:pb-0" data-peer-state={row.state}>
              ...
              {row.state === 'connectable' ? (
                ...
                <Tooltip content={connectDirectionNote(row.peer.hostName, offerReverse)} placement="left">
                  <PrimaryButton size="xs" onClick={() => onConnect(row.endpoint, offerReverse ? reverseScopes : null)} disabled={busy || waitingOn !== null}>
                    Connect
                  </PrimaryButton>
                </Tooltip>) : row.state === 'paired' ? (<span className="rounded-[3px] border border-[color:var(--border-default)] px-1 text-micro text-[color:var(--text-muted)]">
                  Paired
                </span>) : null}
            </div>))}
        </div>)}
