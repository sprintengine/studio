...
test('focusedAgentSlice', async () => {
  ...
  // closed and its session gone.
  {
    ...
    assert.equal(
      resolveFocusedAgentId('ws', 'a1', new Set(['a1']), [session({ agentId: 'a2', lastOutputAt: 99 })]),
      'a1',
    )
  }
  ...
  // A removed agent falls back to the live agent that did something most recently.
  {
    ...
    assert.equal(
      resolveFocusedAgentId('ws', undefined, ['a2', 'a3'], sessions),
      'a3',
      'never focused: the same fallback',
    )
  }
