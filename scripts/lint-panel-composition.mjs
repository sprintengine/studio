...
const WORKSPACE_TOPBAR_PATH = resolve(process.cwd(), 'src/renderer/src/components/workspace/WorkspaceActions.tsx')
...
const ALLOW_LIST = new Map([
  [
    'BacklogCreateDialog.tsx',
    'Modal-based "New backlog item" capture dialog; chrome is owned by Modal + ModalHeader, not a PanelHeader. It is a dialog rendered by BacklogPanel, not an operational panel.',
  ],
  ['AgentPanel.tsx', 'Hosts an agent terminal (TerminalView); chrome is the terminal surround, not a PanelHeader.'],
  [
    'AgentChatView.tsx',
    'Agent-tab body for conversation agents (sibling of AgentPanel/TerminalView); the FlexLayout tab owns identity, so the surface uses a compact composer + session-status bar, not a PanelHeader.',
  ],
...
try {
  ...
  files = readdirSync(PANELS_DIR).filter((name) => name.endsWith('.tsx') && !name.endsWith('.test.tsx'))
} catch (error) {
  process.stdout.write(`Panel directory missing or unreadable: ${PANELS_DIR}\n${error.message}\n`)
  process.exit(2)
}
