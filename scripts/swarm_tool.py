...
def main(argv: list[str] | None = None) -> int:
    ...
    if _help_requested(cleaned_argv):
        print(f"[swarm] backend mode: {backend} (selected by {selected_by})", file=sys.stderr)
    ...
    print(
        f"[swarm] unsupported backend mode: {backend}. "
...
def _select_backend(argv: list[str]) -> tuple[str, list[str], str]:
    ...
    while index < len(argv):
        ...
        if value == "--backend":
            if index + 1 >= len(argv):
                print("[swarm] --backend requires a value: direct-core or mcp-local.", file=sys.stderr)
                raise SystemExit(2)
...
def _run_mcp_backend(argv: list[str], backend: str) -> int:
    ...
    if not getattr(args, "uses_state", True):
        print(
            f"[swarm] backend mode {backend} does not support '{args.group}'. "
    ...
    try:
        ...
        print(f"[swarm] backend mode {backend} cannot dispatch command: {message}", file=sys.stderr)
    ...
    try:
        ...
        print(
            f"[swarm] backend mode {backend} failed before dispatch: {exc}",
            file=sys.stderr,
        )
        return 1
    ...
    if not response.get("ok"):
        ...
        print(
            f"[swarm] backend mode {backend} returned {error.get('code', 'error')}: "
