...
protocol.registerSchemesAsPrivileged([
  {
    scheme: 'studio-module',
    privileges: { standard: true, secure: true, supportFetchAPI: true, stream: true, corsEnabled: true },
  },
])
const deadline = setTimeout(() => {
  console.error('Doom wrapper smoke timed out')
  app.exit(1)
}, 45000)
app.whenReady().then(async () => {
  ...
  try {
    await build({
      entryPoints: [path.join(root, 'src/main/modules/module-assets.ts')],
      bundle: true,
      platform: 'node',
      format: 'cjs',
      packages: 'external',
    ...
    session.defaultSession.webRequest.onBeforeRequest({ urls: ['studio-module://*/*'] }, (details, callback) =>
      callback({
        cancel: !isAllowedModuleAssetRequest(
          details,
          require('node:url').pathToFileURL(path.join(temp, 'index.html')).href,
        ),
      }),
    )
    ...
    const installed = {
      manifest: { id: 'studio-doom' },
      moduleRoot: path.join(workspaceRoot, 'module'),
    ...
    protocol.handle(
      ...
      createModuleAssetHandler({
        assetOrigin: () => origin,
        discoverModules: async () => ({ modules: [installed], rejected: [] }),
        isEnabled: () => true,
      }),
    )
    await build({
      stdin: {
        ...
        window.__root.render(React.createElement(Panel,{workspaceId:'electron-wrapper-smoke'}));`,
        resolveDir: root,
        loader: 'tsx',
      },
      bundle: true,
    ...
    const waitFor = (predicate) =>
      window.webContents.executeJavaScript(
        `new Promise((resolve,reject)=>{const end=Date.now()+20000;const check=()=>{if((${predicate})())return resolve(true);if(Date.now()>end)return reject(new Error('UI condition timed out: '+document.body.innerText));setTimeout(check,50)};check()})`,
      )
    const click = (label) =>
      window.webContents.executeJavaScript(
        `(()=>{const button=[...document.querySelectorAll('button')].find(b=>b.textContent.trim()===${JSON.stringify(label)});if(!button)throw new Error('Button not found');button.click()})()`,
      )
    ...
    await waitFor(
      `()=>document.body.innerText==='' || document.querySelector('.doom-overlay')?.textContent.includes('Doom paused')`,
    )
    await window.webContents.executeJavaScript(`document.querySelector('.doom-workspace').style.display=''`)
    await waitFor(`()=>document.body.innerText.includes('Doom paused')`)
    const result = await window.webContents.executeJavaScript(
      `({header:document.querySelector('header')?.textContent.includes('Doom')===true, iframe:document.querySelector('iframe').src, muted:document.body.innerText.includes('Unmute')})`,
    )
    ...
    if (await window.webContents.executeJavaScript('document.querySelectorAll("iframe").length'))
      throw new Error('Unmount retained iframe')
