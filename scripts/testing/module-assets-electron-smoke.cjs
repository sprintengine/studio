...
app.whenReady().then(async () => {
  try {
    ...
    await fs.writeFile(
      root + '/engine.js',
      `(async()=>{try { await WebAssembly.instantiateStreaming(fetch('engine.wasm')); const worker=await new Promise(r=>{const w=new Worker('worker.js');w.onmessage=e=>{w.terminate();r(e.data)}}); const db=await new Promise((r,j)=>{const q=indexedDB.open('doom-smoke',1);q.onsuccess=()=>r(q.result);q.onerror=()=>j(q.error)});db.close();parent.postMessage({ok:true,worker,origin:location.origin},'*')}catch(e){parent.postMessage({error:String(e)},'*')}})()`,
    )
    protocol.handle(
      'studio-module',
      createModuleAssetHandler({
        ...
        discoverModules: async () => ({
          modules: [{ manifest: { id: 'doom' }, moduleRoot: root, trust: { status: 'trusted' } }],
          rejected: [],
        }),
        isEnabled: () => true,
      }),
    ...
    const crossOriginBlocked = await win.webContents.executeJavaScript(
      `fetch('studio-module://646f6f6d/engine.wasm').then(()=>false,()=>true)`,
    )
    if (!crossOriginBlocked) throw new Error('Cross-origin asset reads must be blocked')
    const workerBlocked = await win.webContents.executeJavaScript(
      `new Promise(r=>{const url=URL.createObjectURL(new Blob(["fetch('studio-module://646f6f6d/engine.wasm').then(r=>postMessage(r.status===403),()=>postMessage(true))"],{type:'text/javascript'}));const w=new Worker(url);w.onmessage=e=>{w.terminate();URL.revokeObjectURL(url);r(e.data)}})`,
    )
    if (!workerBlocked) throw new Error('Worker guessed a private module origin')
    await win.loadFile(shellFile)
    const result = await win.webContents.executeJavaScript(
      `new Promise((resolve)=>{const timer=setTimeout(()=>resolve({error:'timeout'}),10000);window.addEventListener('message', e=>{clearTimeout(timer);resolve(e.data)},{once:true});const f=document.createElement('iframe');f.sandbox='allow-scripts allow-same-origin allow-pointer-lock';f.src=${JSON.stringify(origin + '/index.html')};document.body.append(f)})`,
    )
    console.log(JSON.stringify(result))
    if (!result.ok || result.worker !== 'worker-ok') throw new Error('Smoke failed')
