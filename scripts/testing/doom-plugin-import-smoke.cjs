...
async function main() {
  ...
  try {
    await build({
      ...
      outfile: path.join(temp, 'download.cjs'),
      bundle: true,
      platform: 'node',
      format: 'cjs',
      packages: 'external',
    ...
    const fetcher = async (url) => {
      ...
      if (tamper && relative === 'module/runtime/runtime.js')
    ...
    console.log(
      'PASS: signed Doom imports within production limits, requires community trust, and rejects tampered runtime bytes.',
    )
  } finally {
