...
A multi-step plan threads a roster from a `@roster=` annotation on one step,
...
def test_two_steps_with_different_rosters_produce_differently_staffed_runs(tmp_path) -> None:
    # The epic's acceptance, stated literally: "Two steps in one plan with
...
def test_a_run_with_no_roster_declares_no_configured_roles(tmp_path) -> None:
    # The un-staffed case the plan's built-in default produces. It must be an
