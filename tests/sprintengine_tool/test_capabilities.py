...
def test_worker_tool_listing_stays_under_byte_budget(tmp_path) -> None:
    ...
    # worker needs, nudging it to 27k. MC-2508 adds `roles.brief` beside `soul.get`
    # so every worker can fetch a role brief under the new name (~0.7k), nudging
    # it to 28k. It still guards against regression toward the old ~62k-char
    # full listing.
