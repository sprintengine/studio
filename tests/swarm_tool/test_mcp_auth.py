...
def test_mcp_call_requires_authenticated_mcp_authorized_actor(tmp_path) -> None:
    ...
    missing_actor = server.call_tool("swarm.task.next", payload)
...
def test_allowed_roots_reject_out_of_scope_and_platform_confused_paths(tmp_path) -> None:
    ...
    server = SwarmMcpServer(allowed_roots=[tmp_path / "allowed"])

    out_of_scope = server.call_tool("swarm.summary", {"statePath": str(fixture.state_path)}, actor("workspace-user"))
    platform_confused = server.call_tool("swarm.summary", {"statePath": r"C:\workspace\swarm\state.yaml"}, actor("workspace-user"))
...
def test_authenticated_mcp_user_can_invoke_artifact_review_with_payload_actor(tmp_path) -> None:
    artifact_file = tmp_path / "swarm" / "mcp-artifact-auth" / "review.md"
...
def test_authenticated_mcp_user_can_invoke_feedback_tool_without_swarm_role(tmp_path) -> None:
    ...
    response = server.call_tool(
        "swarm.feedback.summarize",
        {"statePath": str(fixture.state_path)},
        actor("workspace-user", "not-a-swarm-role"),
    )
