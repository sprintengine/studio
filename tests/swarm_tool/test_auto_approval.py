...
def test_renderer_auto_approval_policy_matches_approved_artifact_kinds() -> None:
    renderer_source = (Path(__file__).resolve().parents[2] / "src/renderer/src/utils/swarm.ts").read_text(encoding="utf-8")
...
def test_electron_auto_run_sends_approval_intent_instead_of_approving_directly() -> None:
    ...
    supervisor_source = (repo_root / "src/renderer/src/components/workspace/SwarmAutoRunSupervisor.tsx").read_text(
        encoding="utf-8"
    )
