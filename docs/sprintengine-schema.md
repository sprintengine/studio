...
per-task quality requirements, its role manifests, — pre-v3 — its persistent
`agents` map, and — pre-v4 — its single repo described by fields this build no
longer writes are all incompatible.
...
`roster_configured`, and `runner_policy_updated`.
