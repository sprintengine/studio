...
| Default folder  | None in the project. A bare name is a board in the app's own board store, `<userData>/canvas/<project key>/`, outside the project's working tree, so a board never dirties the repository. It used to be `diagrams/` at the workspace root (`folderPath`); boards already there keep listing, opening and saving in place, and a bare name finds one when the store has none of that name. Tools and the tab still accept any project-relative `*.excalidraw` path with a folder in it. A board reaches the repository through the tab's **Export to folder…** action (see 3.8). |
...
agent CLI ── stdio bridge ── MCP socket ── canvas-tools.ts
                    ...
                    (convert, bind, layout,          (the live editor: view +
                     ...
                     <userData>/canvas/<project key>/*.excalidraw   ← source of truth
                     (and boards an earlier build left in <workspace root>/diagrams/)
```
...
- **Person edit**: tab `onChange` (debounced ~400 ms, skipped when the scene
  ...
  writes, bumps `revision`, pushes to _other_ subscribers.
...
| `paths.ts`           | `CANVAS_LEGACY_FOLDER = 'diagrams'`, `canvasBoardIsInStore(path)`, `canvasLegacyPathFor(bareName)`, `CANVAS_FILE_EXTENSION = '.excalidraw'`, `CANVAS_LIST_MAX_BOARDS = 200`, `normalizeCanvasPath(input)` (posix; a bare name normalizes to a folderless file name, which is a store board, and anything with a folder is project-relative; rejects `..`, absolute paths and other extensions; the extension is accepted in ANY case and kept exactly as given, so a file already on disk is never renamed; `.git`/`node_modules` are refused case-insensitively on every platform), `canvasBoardName(path)`, `canvasPathIsCaseInsensitive(platform)`, `canvasBoardKeyPath(path, platform)`. |
...
type CanvasSkeleton = {
  ...
  endElementId?: string // arrows/lines; id or tempId
...
`board` (project-relative path or bare name, through `normalizeCanvasPath`; a
bare name resolves to the store, or to `diagrams/<name>.excalidraw` when only
that exists, through `boardExists`);
omitted, it targets the workspace's most recently CHANGED board (`listBoards`
sorted by `modifiedAt`), else the store board `canvas.excalidraw`. A mutating tool
...
### 3.8 Storage and export

Boards used to be created in the project's `diagrams/` folder, which put every
sketch an agent drew into `git status`. They now live outside the project
altogether, following the same rule as the app's hooks, skills and MCP gateway
(`docs/agent-launch-isolation.md`): nothing of the app's belongs in the
project's working tree unless the person puts it there.

- **Store.** `<userData>/canvas/<project key>/<name>.excalidraw`, resolved in
  main by `canvasBoardStoreDir` (`src/main/canvas/canvas-board-store.ts`) and
  injected into the service as `resolveBoardStore`. The agent-integration copy
  already lives under `userData`, and nothing under `~/.sprintengine` is keyed
  per project, so the store sits beside it. The project key is
  `deriveWorkspaceId(workspaceRoot)` — the one-way hash of the resolved project
  folder that the app already uses as a stable, path-free project key — so the
  boards follow the folder rather than a registry entry. The store is flat: a
  normalized path with no folder (`arch.excalidraw`) is a store board, and one
  with a folder is always a project file. Nothing is written into the project,
  so there is no ignore file to write. `listBoards` reads the store, then walks
  the project as before, leaving out boards at the project root: their path
  would have no folder, which names a store board.
- **Where an agent finds it.** It does not. Agents name boards; the `canvas.*`
  tools run in main and resolve the name, and no tool answers with a store path.
  An agent in a WSL shell on a Windows host therefore needs no path
  translation. The reveal action asks main for the path too
  (`canvas:reveal-board`), since the renderer cannot spell it either.
- **Legacy boards.** Nothing is moved. A board in `diagrams/` (or any other
  project folder) keeps its path: it lists, opens, saves in place, and a tab
  that already holds it keeps working. The tools resolve a bare name to the
  store first and to `diagrams/<name>.excalidraw` when only that exists; the
  picker refuses to create a store board whose name a legacy board already has,
  since a bare name would then be ambiguous.
- **Export.** The only way a board reaches the repository. The editor's menu
  has **Export to folder…**. The dialog always exports the board file and
  optionally a PNG and an SVG, rendered by the tab's own editor. Main shows the
  folder picker (`canvas:export-board`), so the destination is always one the
  person chose rather than a path from the renderer; `CanvasService.exportBoard`
  settles the disk, serializes the board it holds and writes each file through
  a temp and a rename, replacing an earlier export of the same name. The board
  in the store stays where it is.
