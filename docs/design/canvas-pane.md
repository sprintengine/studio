...
| Default folder  | `.sprintengine/canvas/` under the workspace root (`folderPath`, the same root the backlog and the browser sidecar resolve against): the app's own board store, which ignores itself so a board never dirties the repository. It used to be `diagrams/`; boards already there keep listing, opening and saving in place, and a bare name finds one when the store has none of that name. Tools and the tab still accept any project-relative `*.excalidraw` path. A board reaches the repository through the tab's **Export to folder…** action (see 3.8). |
...
agent CLI ── stdio bridge ── MCP socket ── canvas-tools.ts
                    ...
                    (convert, bind, layout,          (the live editor: view +
                     ...
                     <workspace root>/.sprintengine/canvas/*.excalidraw   ← source of truth
                     (and boards an earlier build left in diagrams/)
```
...
| `paths.ts`           | `CANVAS_DEFAULT_FOLDER = '.sprintengine/canvas'` (built with `sidecarRelativePath`), `CANVAS_LEGACY_FOLDER = 'diagrams'`, `canvasBoardIsInStore(path)`, `canvasLegacyPathFor(bareName)`, `CANVAS_FILE_EXTENSION = '.excalidraw'`, `CANVAS_LIST_MAX_BOARDS = 200`, `normalizeCanvasPath(input)` (posix, project-relative, rejects `..`, absolute paths and other extensions; a bare name lands in the default folder; the extension is accepted in ANY case and kept exactly as given, so a file already on disk is never renamed; `.git`/`node_modules` are refused case-insensitively on every platform), `canvasBoardName(path)`, `canvasPathIsCaseInsensitive(platform)`, `canvasBoardKeyPath(path, platform)`. |
...
sorted by `modifiedAt`), else `.sprintengine/canvas/canvas.excalidraw`. A mutating tool
...
sketch an agent drew into `git status`. They now live in the app's store under
the workspace sidecar, following the same rule as the app's hooks, skills and
MCP gateway (`docs/agent-launch-isolation.md`): nothing of the app's belongs in
the repository unless the person puts it there.

- **Store.** A bare name normalizes to `.sprintengine/canvas/<name>.excalidraw`.
  The service writes `.sprintengine/canvas/.gitignore` (`*`) the first time it
  writes a board there, never over an existing file — the same one-line
  self-ignore the browser captures and the backlog cache use. `listBoards` walks
  the store by name (it is a dot-folder, which the project walk skips) and then
...
- **Export.** The editor's menu has **Export to folder…**. The dialog always
  exports the board file and optionally a PNG and an SVG, rendered by the tab's
  own editor. Main shows the folder picker (`canvas:export-board`), so the
  destination is always one the person chose rather than a path from the
  renderer; `CanvasService.exportBoard` settles the disk, serializes the board
  it holds and writes each file through a temp and a rename, replacing an
  earlier export of the same name. The board in the store stays where it is.
...
2. A person can open a Canvas tab, draw, close the app, reopen and find the
   drawing; the file under `.sprintengine/canvas/` is valid and opens in the
