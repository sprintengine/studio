...
# Append-only time-series of per-session cumulative token usage, sampled by the
# host on each agent Stop/SessionEnd (when the CLI transcript is flushed) via
# `sprintengine.agent.sample_token_usage`. build_projection brackets these
# samples against task/gate-attempt lifecycle windows to attribute tokens to
# tasks and roles (Phase 2). See knowledge/multicode/sprint-engine.md.
TOKEN_SAMPLE_FILE = "metrics/agent-token-samples.jsonl"
TOKEN_SAMPLE_SCHEMA_VERSION = 1
...
# --- Phase 2: per-task / per-role token attribution -------------------------
#
# Primitive: tokens(window) = cumulative(session @ end) - cumulative(session @
# start), where cumulative comes from a host-sampled snapshot of the adapter's
# per-session reading. Samples are taken on Stop/SessionEnd (flush), so the
# snapshot at-or-before a window boundary is the cumulative as of that boundary.
# Windows are orchestrator-owned lifecycle spans (task implementation, each gate
# attempt). A window we cannot bracket with samples is flagged `partial`, never
# zeroed or guessed.
...
def record_agent_token_sample(
    ...
    *,
    ...
    path = team_dir / TOKEN_SAMPLE_FILE
    append_jsonl(path, record)
    with FolderLock(team_dir / READY_QUEUE_LOCK_FILE):
        write_projection_file(team_dir, {}, state_path=state_path)
    return path
...
def _sample_per_model(sample: dict[str, Any]) -> dict[str, dict[str, int]]:
    out: dict[str, dict[str, int]] = {}
    for model in sample.get("perModel") or []:
        if not isinstance(model, dict):
            continue
        name = str(model.get("model") or "unknown")
        bucket = out.setdefault(name, _zero_usage())
...
def _cumulative_at(samples: list[dict[str, Any]], boundary: str) -> dict[str, dict[str, int]] | None:
    """Per-model cumulative from the latest sample at-or-before `boundary`.

    Samples must be sorted by sampledAt. Returns None when the session produced
    no sample by the boundary (it had not been measured yet at that time).
    """
    chosen: dict[str, Any] | None = None
    for sample in samples:
        if str(sample.get("sampledAt") or "") <= boundary:
            chosen = sample
        else:
            break
    ...
    *,
    ...
    for samples in samples_by_session.values():
        ...
        for model, end_usage in end_cumulative.items():
            ...
            bucket = aggregate.setdefault(model, _zero_usage())
            for key in _USAGE_KEYS:
    ...
    if not measured_any:
        ...
        return {"perModel": [], "total": _zero_usage(), "partial": True, "reason": "no_sample"}
    total = _zero_usage()
...
def _sum_usage(parts: list[dict[str, Any]]) -> dict[str, Any]:
    aggregate: dict[str, dict[str, int]] = {}
    for part in parts:
        for model in part.get("perModel") or []:
            bucket = aggregate.setdefault(str(model.get("model") or "unknown"), _zero_usage())
            for key in _USAGE_KEYS:
                value = model.get(key)
    ...
    for usage in aggregate.values():
        for key in _USAGE_KEYS:
            total[key] += usage[key]
    return {"perModel": [{"model": model, **usage} for model, usage in aggregate.items()], "total": total}


def _samples_by_agent(samples: list[dict[str, Any]]) -> dict[str, dict[str, list[dict[str, Any]]]]:
    by_agent: dict[str, dict[str, list[dict[str, Any]]]] = {}
    for sample in samples:
        if not isinstance(sample, dict):
            continue
        agent_id = str(sample.get("agentId") or "").strip()
        session_id = str(sample.get("cliSessionId") or "").strip()
        if not agent_id or not session_id:
            continue
        by_agent.setdefault(agent_id, {}).setdefault(session_id, []).append(sample)
    for sessions in by_agent.values():
        for session_samples in sessions.values():
            session_samples.sort(key=lambda item: str(item.get("sampledAt") or ""))
    return by_agent


def attribute_task_token_usage(task: dict[str, Any], samples: list[dict[str, Any]]) -> None:
    """Attach per-attempt and per-task tokenUsage to a projection task in place.

    Developer (implementation) window: [startedAt, earliest gate-attempt start]
    keyed by the implementing agent — the work left the developer's hands when
    review began, so the review period is excluded. A task reworked across
    changes_requested loops cannot be isolated to a single span (the loops
    interleave with review and other work), so its developer window is reported
    but flagged partial. Each gate attempt is its own clean single-episode
    window keyed by attempt.claimedBy. The task total sums the developer window
    and every attempt.
    """
    by_agent = _samples_by_agent(samples)

    gates = task.get("qualityGates") if isinstance(task.get("qualityGates"), list) else []
    attempt_usages: list[dict[str, Any]] = []
    earliest_attempt_start: str | None = None
    reworked = False
    for gate in gates:
        ...
        for attempt in gate.get("attempts") or []:
            if not isinstance(attempt, dict):
                continue
            actor = str(attempt.get("claimedBy") or attempt.get("actor") or "").strip()
            started_at = attempt.get("startedAt")
            completed_at = attempt.get("completedAt")
            if isinstance(started_at, str) and started_at:
                if earliest_attempt_start is None or started_at < earliest_attempt_start:
    ...
    implementer = str(task.get("lastImplementedByAgentId") or task.get("ownerAgentId") or "").strip()
    dev_end = earliest_attempt_start or task.get("completedAt")
    developer = _window_usage(by_agent.get(implementer, {}), task.get("startedAt"), dev_end)
    if reworked and not developer.get("partial"):
        # Single-span developer math is only exact for a one-episode task; a
        # reworked task's span includes review/other-work gaps. Report it, flag it.
        developer = {**developer, "partial": True, "reason": "reworked_multi_episode"}
    developer["agentId"] = implementer

    combined = _sum_usage([developer, *attempt_usages])
    task["tokenUsage"] = {
        "perModel": combined["perModel"],
        "total": combined["total"],
        "developer": developer,
        "partial": bool(developer.get("partial")) or any(u.get("partial") for u in attempt_usages),
    }
...
def build_projection(
    ...
    *,
    ...
    if folder_store_ready:
        ...
        token_samples = read_jsonl_file(team_dir / TOKEN_SAMPLE_FILE)
    ...
    for task in tasks:
        ...
        attribute_task_token_usage(task, token_samples)
