...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = task_sub.add_parser("request-changes", help="Reviewer: return a target task to implementation and require your re-approval.")
    p.add_argument("--task-id", required=True, help="Target task requiring rework.")
    p.add_argument("--id", required=True, help="Requesting reviewer agent id.")
    p.add_argument("--source-task-id", help="Sweep/final-review task the requester owns; omit for an independent phase review of the target itself.")
    p.add_argument("--feedback", required=True, help="Concrete blocking feedback for the implementer.")
    p.add_argument("--path", action="append", default=[], help="Project-relative path supporting the finding.")
    p.add_argument("--finding-json", action="append", default=[], help="Optional structured finding JSON object.")
    p.set_defaults(handler=task_commands.request_changes)

    p = task_sub.add_parser("approve-rework", help="Sweep/final reviewer: approve rework you requested on another task.")
    p.add_argument("--task-id", required=True, help="Reworked target task.")
    p.add_argument("--source-task-id", required=True, help="Review source task still owned by the requester.")
    p.add_argument("--id", required=True, help="Original requesting reviewer id.")
    p.add_argument("--summary", required=True, help="Approval rationale.")
    p.set_defaults(handler=task_commands.approve_rework)

    p = task_sub.add_parser("reassign-review", help="Planner: explicitly replace an unrecoverable review requester.")
    p.add_argument("--task-id", required=True)
    p.add_argument("--id", required=True, help="Planner/operator actor id.")
    p.add_argument("--reviewer-id", required=True, help="Replacement reviewer agent id.")
    p.add_argument("--reviewer-role", required=True, help="Replacement reviewer role.")
    p.add_argument("--reviewer-cli", help="Replacement CLI; required for phase review unless the previous runtime is reusable.")
    p.add_argument("--reviewer-model", help="Optional replacement model.")
    p.add_argument("--reason", required=True, help="Why approval authority is being replaced.")
    p.set_defaults(handler=task_commands.reassign_review)
