...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--model", help="CLI model to record on the claimed task. Overrides SPRINTENGINE_AGENT_MODEL; the app injects that env at launch, so this is for headless CLI use.")
    p.add_argument("--cli", help="CLI the recorded model belongs to. Overrides SPRINTENGINE_AGENT_CLI.")
    ...
    p.add_argument("--needs-input-finding-id", help="Structured finding id this escalation is about. The reviewer task-filing channel: a finding too large to fix forward inside its review task escalates naming the finding, and the architect files a task carrying --from-finding-id.")
    ...
    p.add_argument("--needs-input-finding-id", help="Structured finding id this escalation is about (the reviewer task-filing channel).")
    ...
    p.add_argument("--source-doc", action="append", default=[], help="Project-root-relative canonical source document this task implements (e.g. the backlog item on a reference-sourced run). Repeatable. Workers are directed to read each in full on claim; keep the task card the delta, never a restatement.")
    p.add_argument("--task-note", action="append", default=[], help="Repeatable task note.")
    p.add_argument("--backlog-ref", help="Project-root-relative path to the backlog item this task delivers, e.g. backlog/2026-07-30-example.md. One item per task: a second task pointing at the same item is rejected.")
    p.add_argument("--backlog-key", help="Human key of the backlog item named by --backlog-ref, e.g. MC-1843. Requires --backlog-ref.")
    ...
    p.add_argument("--kind", choices=["work", "review", "integration_review"], help="Task charter marker. `review` = a planned review of other tasks' work; `integration_review` = the terminal task that proves the pieces work together (build, run the app, exercise the seams). Both run like any other task. Omit for ordinary work.")
    p.add_argument("--from-finding-task-id", help="Task whose structured finding this task was filed to answer (reviewer task-filing channel). Use with --from-finding-id.")
    p.add_argument("--from-finding-id", help="Structured finding id this task was filed to answer. Use with --from-finding-task-id.")
    ...
    p.add_argument("--path", action="append", help="Replace the task's owned modules with this repeatable list of directories.")
    ...
    p.add_argument("--source-doc", action="append", help="Replace canonical source documents with this repeatable list of project-root-relative paths.")
    p.add_argument("--clear-source-docs", action="store_true")
    ...
    p.add_argument("--backlog-ref", help="Point this task at the backlog item it delivers, by project-root-relative path. One item per task.")
    p.add_argument("--backlog-key", help="Human key of the backlog item named by --backlog-ref, e.g. MC-1843. Requires --backlog-ref.")
    p.add_argument("--clear-backlog-ref", action="store_true", help="Clear this task's backlog item pointer.")
    p.add_argument("--produces-implementation", action="store_true", help="Mark this task as implementation-producing even when its role is not developer/frontend.")
    p.add_argument("--kind", choices=["work", "review", "integration_review"], help="Set or clear the task's charter marker; `work` clears it back to an ordinary task.")
    p.add_argument("--product-facing", action="store_true", help="Mark this task as requiring product acceptance when product is rostered.")
    p.add_argument("--not-product-facing", action="store_true", help="Persist that this task should not receive product acceptance by default.")
